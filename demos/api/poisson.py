...
# This scripts assembles and solves a simple static Poisson problem
...
def create_geometry():
    model.add("poisson")
    surfs=[]
    surfs.append( (2,factory.addRectangle(0, 0, 0, 1, 1)) )
    surfs.append( (2,factory.addDisk(0.7, 0.5, 0, 0.1, 0.1)) )
    surfs.append( (2,factory.addRectangle(0.2, 0.4, 0, 0.2, 0.2)) )
    
    ov, ovv = factory.fragment(surfs, []) 

    factory.synchronize() # need to synchronize before fragmenting
    
    bnd = model.getBoundary(ov, combined=True, oriented=False, recursive=False)
    bnd = np.array(bnd)
    
    model.addPhysicalGroup( 2, [ov[0][1]] , 2)
    model.setPhysicalName(  2, 2, 'SourceCircle')
    model.addPhysicalGroup( 2, [ov[1][1]] , 3)
    model.setPhysicalName(  2, 3, 'SourceSquare')
    model.addPhysicalGroup( 2, [ov[2][1]] , 4)
...
def solve():
    ...
    printf('numMeshNodes =', numMeshNodes, ' maxNodeTag =', maxNodeTag)
    ...
    for iGroup in vGroups:
        ...
        for tagEntity in vEntities:
            ...
            for elementType in vElementTypes:
                ...
                printf('\nIn group', namGroup, ' with tag ', tagGroup, ', numElements = e =', numElements)
                ...
                if dimEntity==2 :
                    ...
                    localmat = np.einsum("egik,egjk,eg,g->eij", dsfdx, dsfdx, qdet, weights) # Gauss integration
    ...
    sol = scipy.sparse.linalg.spsolve(globalmat[:numUnknowns,:numUnknowns], globalrhs[:numUnknowns])
