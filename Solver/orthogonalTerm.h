...
public:
  ...
  orthogonalTerm(GModel *gm, int iField, simpleFunction<double> *k,
                 PView *pview)
    : helmholtzTerm<double>(gm, iField, iField, k, 0), _pview(pview),
      _dofView(0), withDof(false)
  {
  }
  orthogonalTerm(GModel *gm, int iField, simpleFunction<double> *k,
                 dofManager<double> *dofView)
    : helmholtzTerm<double>(gm, iField, iField, k, 0), _pview(0),
      _dofView(dofView), withDof(true)
  {
  }
  void elementVector(SElement *se, fullVector<double> &m) const
  {
    ...
    for(int i = 0; i < npts; i++) {
      ...
      for(int j = 0; j < nbSF; j++) {
        Grads[j] =
          SVector3(invjac[0][0] * grads[j][0] + invjac[0][1] * grads[j][1] +
    ...
    for(int j = 0; j < nbSF; j++)
      for(int k = 0; k < j; k++) mat(k, j) = -1. * mat(j, k);
    ...
    for(int i = 0; i < nbSF; i++) {
      std::map<MVertex *, double>::iterator it =
        _distance_map->find(e->getShapeFunctionNode(i));
      val(i) = it->second;
    }
    ...
    /*   /\* for (int i = 0; i < nbSF; i++){ *\/ */
    /*   /\* 	data->getValue(0, e->, e->getNum(),
     * e->getShapeFunctionNode(i)->getNum(), nod, 0, val(i)); *\/ */
    /*   /\* } *\/ */
    /* } */
