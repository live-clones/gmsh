...
//
// This file redefines the Gmsh C++ API in terms of the C API (v4.15.0).
//
...
namespace gmsh { // Top-level functions
  ...
  namespace model { // Model functions
    ...
    inline void getPhysicalGroupsEntities(gmsh::vectorpair & dimTags,
                                          std::vector<gmsh::vectorpair> & entities,
                                          const int dim = -1)
    {
      int ierr = 0;
      int *api_dimTags_; size_t api_dimTags_n_;
      int **api_entities_; size_t *api_entities_n_, api_entities_nn_;
      gmshModelGetPhysicalGroupsEntities(&api_dimTags_, &api_dimTags_n_, &api_entities_, &api_entities_n_, &api_entities_nn_, dim, &ierr);
      if(ierr) throwLastError();
      dimTags.resize(api_dimTags_n_ / 2); for(size_t i = 0; i < api_dimTags_n_ / 2; ++i){ dimTags[i].first = api_dimTags_[i * 2 + 0]; dimTags[i].second = api_dimTags_[i * 2 + 1]; } gmshFree(api_dimTags_);
      entities.resize(api_entities_nn_); for(size_t i = 0; i < api_entities_nn_; ++i){ entities[i].resize(api_entities_n_[i] / 2); for(size_t j = 0; j < api_entities_n_[i] / 2; ++j){ entities[i][j].first = api_entities_[i][j * 2 + 0]; entities[i][j].second = api_entities_[i][j * 2 + 1]; } gmshFree(api_entities_[i]); } gmshFree(api_entities_); gmshFree(api_entities_n_);
    }
    ...
    // model.
    inline int isEntityOrphan(const int dim,
                              const int tag)
    {
      int ierr = 0;
      int result_api_ = gmshModelIsEntityOrphan(dim, tag, &ierr);
      if(ierr) throwLastError();
      return result_api_;
    }
    ...
    inline void getEntityType(const int dim,
                              const int tag,
                              std::string & entityType)
    {
      int ierr = 0;
      char *api_entityType_;
      gmshModelGetEntityType(dim, tag, &api_entityType_, &ierr);
      if(ierr) throwLastError();
    ...
    // a sphere.
    inline void getEntityProperties(const int dim,
                                    const int tag,
                                    std::vector<int> & integers,
                                    std::vector<double> & reals)
    {
      int ierr = 0;
      int *api_integers_; size_t api_integers_n_;
      double *api_reals_; size_t api_reals_n_;
      gmshModelGetEntityProperties(dim, tag, &api_integers_, &api_integers_n_, &api_reals_, &api_reals_n_, &ierr);
      if(ierr) throwLastError();
      integers.assign(api_integers_, api_integers_ + api_integers_n_); gmshFree(api_integers_);
      reals.assign(api_reals_, api_reals_ + api_reals_n_); gmshFree(api_reals_);
    }
    ...
    namespace occ { // OpenCASCADE CAD kernel functions
      ...
      {
        int ierr = 0;
        int result_api_ = gmshModelOccFillet2D(edgeTag1, edgeTag2, radius, tag, pointTag, (int)reverse, &ierr);
        if(ierr) throwLastError();
        return result_api_;
      }
      ...
      inline void getClosestEntities(const double x,
                                     const double y,
                                     const double z,
                                     ...
                                     const int n = 1)
      {
        ...
        int *api_dimTags_; size_t api_dimTags_n_; vectorpair2intptr(dimTags, &api_dimTags_, &api_dimTags_n_);
        int *api_outDimTags_; size_t api_outDimTags_n_;
        double *api_distances_; size_t api_distances_n_;
        double *api_coord_; size_t api_coord_n_;
        gmshModelOccGetClosestEntities(x, y, z, api_dimTags_, api_dimTags_n_, &api_outDimTags_, &api_outDimTags_n_, &api_distances_, &api_distances_n_, &api_coord_, &api_coord_n_, n, &ierr);
        if(ierr) throwLastError();
        gmshFree(api_dimTags_);
        outDimTags.resize(api_outDimTags_n_ / 2); for(size_t i = 0; i < api_outDimTags_n_ / 2; ++i){ outDimTags[i].first = api_outDimTags_[i * 2 + 0]; outDimTags[i].second = api_outDimTags_[i * 2 + 1]; } gmshFree(api_outDimTags_);
        distances.assign(api_distances_, api_distances_ + api_distances_n_); gmshFree(api_distances_);
        coord.assign(api_coord_, api_coord_ + api_coord_n_); gmshFree(api_coord_);
      }
  ...
  namespace algorithm { // Raw algorithms
    ...
    inline void triangulate(const std::vector<double> & coordinates,
                            std::vector<std::size_t> & triangles,
                            const std::vector<std::size_t> & edges = std::vector<std::size_t>())
    {
      int ierr = 0;
      double *api_coordinates_; size_t api_coordinates_n_; vector2ptr(coordinates, &api_coordinates_, &api_coordinates_n_);
      size_t *api_triangles_; size_t api_triangles_n_;
      size_t *api_edges_; size_t api_edges_n_; vector2ptr(edges, &api_edges_, &api_edges_n_);
      gmshAlgorithmTriangulate(api_coordinates_, api_coordinates_n_, &api_triangles_, &api_triangles_n_, api_edges_, api_edges_n_, &ierr);
      if(ierr) throwLastError();
      gmshFree(api_coordinates_);
      triangles.assign(api_triangles_, api_triangles_ + api_triangles_n_); gmshFree(api_triangles_);
      gmshFree(api_edges_);
    }
    ...
    inline void tetrahedralize(const std::vector<double> & coordinates,
                               ...
                               const std::vector<std::size_t> & triangles = std::vector<std::size_t>())
    {
      int ierr = 0;
      double *api_coordinates_; size_t api_coordinates_n_; vector2ptr(coordinates, &api_coordinates_, &api_coordinates_n_);
      size_t *api_tetrahedra_; size_t api_tetrahedra_n_;
      double *api_steiner_; size_t api_steiner_n_;
      size_t *api_triangles_; size_t api_triangles_n_; vector2ptr(triangles, &api_triangles_, &api_triangles_n_);
      gmshAlgorithmTetrahedralize(api_coordinates_, api_coordinates_n_, &api_tetrahedra_, &api_tetrahedra_n_, &api_steiner_, &api_steiner_n_, api_triangles_, api_triangles_n_, &ierr);
      if(ierr) throwLastError();
      gmshFree(api_coordinates_);
      tetrahedra.assign(api_tetrahedra_, api_tetrahedra_ + api_tetrahedra_n_); gmshFree(api_tetrahedra_);
      steiner.assign(api_steiner_, api_steiner_ + api_steiner_n_); gmshFree(api_steiner_);
      gmshFree(api_triangles_);
    }
  ...
  namespace fltk { // FLTK graphical user interface functions
    ...
    inline void run(const std::string & optionFileName = "")
    {
      int ierr = 0;
      gmshFltkRun(optionFileName.c_str(), &ierr);
      if(ierr) throwLastError();
    }
