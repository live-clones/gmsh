...
namespace gmsh { // Top-level functions
  ...
  namespace model { // Per-model functions
    ...
    namespace mesh { // Per-model meshing functions
      ...
      // the number C of components of a basis function. Each physical mesh edge
      // (or Face) will  be assigned a unique orientation,and all edges (or Faces)
      // of physical mesh will be equipped with an orientation tag , indicating
      // whether the image of the corresponding edge (or Face) of the reference
      // domain through the reference map has the same or opposite orientation.The
      // global edge orientation always pointing from the vertex with the lower
      // global vertex number to the one with the higher one.
