...
# This file defines the Gmsh Julia API (v4.8.1).
#
...
"""
    gmsh.model.mesh.getElementQualities(elementTags, qualityName = "minSJ", task = 0, numTasks = 1)

Get the quality of elements in `elementTags`. `elementTags` is a vector
containing the tags (unique, strictly positive identifiers) of the elements.
`qualityName` is the quality mesure requested ("minSJ" for the minimal scaled
jacobien, "minSICN" for the minimal signed inverted condition number and
"minSIGE" for the signed inverted gradient error. Default is "minSJ").
`elementQuality` is a vector containing qualities of elements in `elementTags`,
in the same order as `elementTags`. If `numTasks` > 1, only compute and return
the part of the data indexed by `task`.

Return `elementQualities`.
"""
function getElementQualities(elementTags, qualityName = "minSJ", task = 0, numTasks = 1)
    api_elementQualities_ = Ref{Ptr{Cdouble}}()
    api_elementQualities_n_ = Ref{Csize_t}()
    ierr = Ref{Cint}()
    ccall((:gmshModelMeshGetElementQualities, gmsh.lib), Cvoid,
          ...
          convert(Vector{Csize_t}, elementTags), length(elementTags), api_elementQualities_, api_elementQualities_n_, qualityName, task, numTasks, ierr)
    ierr[] != 0 && error(gmsh.logger.getLastError())
    elementQualities = unsafe_wrap(Array, api_elementQualities_[], api_elementQualities_n_[], own=true)
    return elementQualities
end
