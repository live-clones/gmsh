...
doc = '''Check if the parametric coordinates provided in `parametricCoord' correspond to points inside the entitiy of dimension `dim' and tag `tag', and return the number of points inside. This feature is only avalaiable for a subset of curves and surfaces, depending on the underyling geometrical representation.'''
model.add('isInside', doc, oint, iint('dim'), iint('tag'), ivectordouble('parametricCoord'))

doc = '''Reparametrize the boundary entity (point or curve, i.e. with `dim' == 0 or `dim' == 1) of tag `tag' on the surface `surfaceTag'. If `dim' == 1, reparametrize all the points corresponding to the parametric coordinates `parametricCoord'. Multiple matches in case of periodic surfaces can be selected with `which'. This feature is only avalaiable for a subset of entities, depending on the underyling geometrical representation.'''
