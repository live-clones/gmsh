...
doc = '''Get the quality of elements in 'elementTags'. `elementTags' is a vector containing the tags (unique, strictly positive identifiers) of the elements. `qualityName' is the quality mesure requested ("minSJ" for the minimal scaled jacobien, "minSICN" for the minimal signed inverted condition number and "minSIGE" for the signed inverted gradient error. Default is "minSJ"). 'elementQuality' is a vector containing qualities of elements in 'elementTags', in the same order as 'elementTags'. If `numTasks' > 1, only compute and return the part of the data indexed by `task'.'''
mesh.add('getElementQualities', doc, None, ivectorsize('elementTags'), ovectordouble('elementQualities'), istring('qualityName', '"minSJ"'), isize('task', '0'), isize('numTasks', '1'))
