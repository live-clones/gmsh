...
mesh.add('getBasisFunctions', doc, None, iint('elementType'), ivectordouble('integrationPoints'), istring('functionSpaceType'), oint('numComponents'), oint('numFunctionsPerElements'), ovectordouble('basisFunctions'))

doc = '''Get the element-dependent basis functions of the elements of type `elementType' in the entity of tag `tag' at the integration points `integrationPoints' (given as concatenated triplets of coordinates in the reference element [g1u, g1v, g1w, ..., gGu, gGv, gGw]), for the function space `functionSpaceType' (e.g. "H1Legendre3" or "GradH1Legendre3" for 3rd order hierarchical H1 Legendre functions or their gradient, in the u, v, w coordinates of the reference elements). `numComponents' returns the number C of components of a basis function. `numBasisFunctions' returns the number N of basis functions per element. `basisFunctions' returns the value of the basis functions at the integration points for all oritentations that exist for element of type `elementType' in entity `tag': [e1g1f1,..., e1g1fN, e1g2f1,..., e2g1f1, ...] when C == 1 or [e1g1f1u, e1g1f1v,..., e1g1fNw, e1g2f1u,..., e2g1f1u, ...]. `basisFunctionsIndex' returns the index of the basis function such that element `i' between have basis functions store at place i in `basisFunctions' array. Warning: this is an experimental feature and will probably change in a future release. If `numTasks' > 1, only compute and return the part of the data indexed by `task'.'''
mesh.add('getBasisFunctionsIndexForElements', doc, None, iint('elementType'), ivectordouble('integrationPoints'), istring('functionSpaceType'), ovectorint('basisFunctionsIndex'), iint('tag','-1'), isize('task', '0'), isize('numTasks', '1'))

doc = '''Preallocate data before calling `getBasisFunctionsIndexForElements' with `numTasks' > 1. For C and C++ only.'''
mesh.add_special('preallocateBasisFunctionsIndexForElements', doc, ['onlycc++'], None, iint('elementType'), ovectorint('basisFunctionsIndex'), iint('tag', '-1'))
