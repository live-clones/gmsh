...
doc = '''Get the basis function of the element of type `elementType' for the given `integrationType' integration rule. 'basisFunctions' contains the evaluation of de the basis functions at the integration points. 'integrationPoints' contains the Gauss weights and integration points. Each physical mesh edge (or Face) will  be assigned a unique orientation,and all edges (or Faces) of physical mesh will be equipped with an orientation tag , indicating whether the image of the corresponding edge (or Face) of the reference domain through the reference map has the same or opposite orientation.The global edge orientation always pointing from the vertex with the lower global vertex number to the one with the higher one.  '''
...
doc = ''' generate the vectorpair 'Keys' .  '''
mesh.add('getKeyForElements',doc,None,iint('dim'),iint('tag'),istring('functionSpaceType'),ovectordouble('coord'),ovectorpair('keys'),iint('elementType','-1'))
