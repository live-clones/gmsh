...
view.add('addModelData',doc,None,iint('tag'),istring('modelName'),istring('dataType'),iint('step'),ivectorint('tags'),ivectorvectordouble('data'),idouble('time','0.'),iint('numComponents','-1'),iint('partition','0'))

doc = '''Gets model-based post-processing data from the view with tag `tag' at step `step. Returns the `data' associated to the vertices or the elements with tags `tags'.'''
view.add('getModelData',doc,None,iint('tag'),iint('step'),ovectorint('tags'),ovectorvectordouble('data'),odouble('time'),oint('numComponents'))
