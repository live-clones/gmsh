...
struct faceXtetFM {
  ...
  faceXtetFM(MTet4 *_t = 0, int iFac = 0) : t1(_t), i1(iFac)
  {
    ...
    //	  std::cout<<"first vec of tet is
    //"<<t1->tet()->getVertex(0)->getNum()<<std::endl; 	  std::cout<<"second vec
    //of tet is "<<t1->tet()->getVertex(1)->getNum()<<std::endl;
    //	  std::cout<<"third vec of tet is
    //"<<t1->tet()->getVertex(2)->getNum()<<std::endl; 	  std::cout<<"fourth vec
    //of tet is "<<t1->tet()->getVertex(3)->getNum()<<std::endl;
...
public:
  ...
  std::string getShortHelp() const { return "Fix the mesh in thin parts"; }
  ...
  static std::map<MVertex *, std::vector<CorrespVerticesFixMesh *> >
    VertexToCorresp;
