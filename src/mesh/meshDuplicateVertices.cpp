...
static bool allowMerge(MVertex *v1, MVertex *v2,
                       std::map<MVertex *, std::vector<MFace>> &v2f)
{
  bool nm1, nm2;
  int d1, d2;
  ...
  std::map<MVertex *, std::vector<MFace>> v2f;
  {
    ...
    for(auto it = itf; it != ite; ++it) {
      ...
      for(size_t i = 0; i < f->triangles.size(); i++) {
        v2f[f->triangles[i]->getVertex(0)].push_back(
          f->triangles[i]->getFace(0));
        v2f[f->triangles[i]->getVertex(1)].push_back(
          f->triangles[i]->getFace(0));
        v2f[f->triangles[i]->getVertex(2)].push_back(
          f->triangles[i]->getFace(0));
      }
      for(size_t i = 0; i < f->quadrangles.size(); i++) {
        v2f[f->quadrangles[i]->getVertex(0)].push_back(
          f->quadrangles[i]->getFace(0));
        v2f[f->quadrangles[i]->getVertex(1)].push_back(
          f->quadrangles[i]->getFace(0));
        v2f[f->quadrangles[i]->getVertex(2)].push_back(
          f->quadrangles[i]->getFace(0));
        v2f[f->quadrangles[i]->getVertex(3)].push_back(
          f->quadrangles[i]->getFace(0));
      }
    }
  }
...
void removeAllMeshEntitiesThatHaveSmallEdges()
{
...

  {
    ...
    for(auto it = itf; it != ite; ++it) {
      ...
      for(size_t i = 0; i < f->lines.size(); i++) {
        MLine *t = f->lines[i];
        MVertex *v0 =
          degeneratedVertices::instance().representative(t->getVertex(0));
        MVertex *v1 =
          degeneratedVertices::instance().representative(t->getVertex(1));
        t->setVertex(0, v0);
        t->setVertex(1, v1);
        if(v0 != v1)
          ts.push_back(t);
          ...
          countLines++;
          //	  printf("%d %d %lu --
          //",b0->getNum(),b0->onWhat()->dim(),b0->onWhat()->edges().size());
          //	  printf("%d %d %lu
          //-->",b1->getNum(),b1->onWhat()->dim(),b1->onWhat()->edges().size());
          //	  printf("%d %d %lu \n"
    ...
    for(auto it = itf; it != ite; ++it) {
      ...
      for(size_t i = 0; i < f->triangles.size(); i++) {
        ...
        MVertex *v1 =
          degeneratedVertices::instance().representative(t->getVertex(1));
        MVertex *v2 =
          degeneratedVertices::instance().representative(t->getVertex(2));
        t->setVertex(0, v0);
        t->setVertex(1, v1);
        t->setVertex(2, v2);
      ...
      for(size_t i = 0; i < f->quadrangles.size(); i++) {
        ...
        MVertex *v0 =
          degeneratedVertices::instance().representative(q->getVertex(0));
        MVertex *v1 =
          degeneratedVertices::instance().representative(q->getVertex(1));
        MVertex *v2 =
          degeneratedVertices::instance().representative(q->getVertex(2));
        MVertex *v3 =
          degeneratedVertices::instance().representative(q->getVertex(3));
        q->setVertex(0, v0);
        q->setVertex(1, v1);
        q->setVertex(2, v2);
        q->setVertex(3, v3);
        std::set<MVertex *> S = {v0, v1, v2, v3};
        if(S.size() == 4) { qs.push_back(q); }
        else if(S.size() == 3) {
          if(v0 == v1)
            f->triangles.push_back(new MTriangle(v1, v2, v3));
          else if(v1 == v2)
            f->triangles.push_back(new MTriangle(v2, v3, v0));
          else if(v2 == v3)
            f->triangles.push_back(new MTriangle(v3, v0, v1));
          else if(v3 == v0)
  ...
  Msg::Info("Removing All Mesh Entities That Have Edges Smaller Than %12.5E",
            CTX::instance()->mesh.toleranceEdgeLength);
  Msg::Info("%d edges %d triangles %d quads %d quads2tri", countLines,
            countTriangles, countQuads, countQuadsToTri);
}
