...
// Filling a region with tetrahedra: the boundary mesh is recovered and
// classified, then points are inserted at the circumcenters of the tets that
// are too large. Two kernels do the insertion: the flat one, on index-based
// arrays, which is the default (Mesh.FlatRefineDelaunay3D), and the original
// one, on the MTet4 objects, which it is meant to replace.
