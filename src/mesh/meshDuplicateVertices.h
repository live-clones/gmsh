...
class degeneratedVertices {
  ...
  std::unordered_map<MVertex*, MVertex*> best;         // root -> vertex au max appétit
  std::unordered_map<MVertex*, int>      bestAppetite; // root -> valeur d’appétit max
  ...
  void make_set(MVertex *x){
    ...
    ufRank[x] = 0;
    best[x]   = x;
    bestAppetite[x] = getAppetiteForMotion(x); // snapshot (voir note ci-dessous)
  }
...
public:
  ...
  void add(MVertex* a, MVertex* b){
    ...
    if(appA < appB) { std::swap(ra, rb); std::swap(appA, appB); }
    ...
    // best.erase(rb); bestAppetite.erase(rb); ufRank.erase(rb); // pas obligatoire
  }
  ...
  // Optionnel : pré-enregistrer un ensemble de sommets (pour éviter la init paresseuse)
