...
static std::unordered_set<GVertex*> findAllEmbeddedVertices(
  GModel *model)
{
  std::unordered_set<GVertex*> embeddedVertices;
  for (auto it = model->firstFace(); it != model->lastFace(); ++it) {
    GFace* gf = *it;
    for (const auto& ev: gf->embeddedVertices())
      embeddedVertices.insert(ev);
  }
  for (auto it = model->firstRegion(); it != model->lastRegion(); ++it) {
    GRegion* gr = *it;
    for (const auto& ev: gr->embeddedVertices())
      embeddedVertices.insert(ev);
  }
...
{
  std::unordered_set<int> uniquePartitions(partitions.begin(), partitions.end());
  return std::vector<int>(uniquePartitions.begin(), uniquePartitions.end());
}

template <int dim>
static void entityCheckForEmbedded(
  typename OverlapHelpers<dim>::Entity *entity,
  const std::unordered_map<MVertex *, partitionVertex *> &mvToPartitionVertex)
{
  auto *pe =
    dynamic_cast<typename OverlapHelpers<dim>::PartitionEntity *>(entity);
  if(!pe) return;
  ...
  for (auto it = model->firstVertex(); it != model->lastVertex(); ++it) {
    ...
    if (gv->geomType() == GEntity::PartitionPoint) {
      ...
      if (embeddedVertices.count(parent)) {
        mvToPartitionVertex.insert(std::make_pair(
          pv->getMeshVertex(0), pv));
