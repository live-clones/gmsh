...
static void buildUniqueFaces(GVolume *gr,
                             std::map<MFace, GVolume *, MFaceLessThan> &bnd)
{
...
static void createMeshStore(GVolume *gr, HXTCombine::MeshStore &ms,
...
int meshCombine3D(GVolume *gr)
{
  ...
  std::unordered_set<MNode *> all_v;

  for(size_t k = 0; k < ccs.hexes().size(); k++) {
    if(ccs.selectedHexes()[k]) {
      MNode *v0 = c2v[ccs.hexes()[k].vertexes[0]];
      MNode *v1 = c2v[ccs.hexes()[k].vertexes[1]];
      MNode *v2 = c2v[ccs.hexes()[k].vertexes[2]];
      MNode *v3 = c2v[ccs.hexes()[k].vertexes[3]];
      MNode *v4 = c2v[ccs.hexes()[k].vertexes[4]];
      MNode *v5 = c2v[ccs.hexes()[k].vertexes[5]];
      MNode *v6 = c2v[ccs.hexes()[k].vertexes[6]];
      MNode *v7 = c2v[ccs.hexes()[k].vertexes[7]];
      all_v.insert(v0);
      all_v.insert(v1);
      all_v.insert(v2);
      all_v.insert(v3);
      ...
      for(int l = 0; l < 6; l++) {
        ...
        GSurface *gfound012 = findInFaceSearchStructure(f012, search);
        GSurface *gfound230 = findInFaceSearchStructure(f230, search);
        if(gfound012 && gfound230 && gfound012 == gfound230) {
          ...
          GSurface *gfound013 = findInFaceSearchStructure(f013, search);
          GSurface *gfound312 = findInFaceSearchStructure(f312, search);
  ...
  for(unsigned int k = 0; k < ms.tetCorners.size() / 4; k++) {
    if(ccs.selectedTets()[k]) {
      MNode *v0 = c2v[ms.tetCorners[4 * k]];
      MNode *v1 = c2v[ms.tetCorners[4 * k + 1]];
      MNode *v2 = c2v[ms.tetCorners[4 * k + 2]];
      MNode *v3 = c2v[ms.tetCorners[4 * k + 3]];
      all_v.insert(v0);
      all_v.insert(v1);
      all_v.insert(v2);
      all_v.insert(v3);
...
bool MakeHybridHexTetMeshConformalThroughTriHedron(GModel *gm)
{
  ...
  std::map<MFace, GVolume *, MFaceLessThan> bnd2;
  for(auto itf = bnd.begin(); itf != bnd.end(); ++itf) {
    GSurface *gfound = findInFaceSearchStructure(itf->first, search);
