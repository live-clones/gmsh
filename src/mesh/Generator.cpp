...
void OptimizeMesh(GModel *m, const std::string &how, bool force, int niter, double quality)
{
  ...
  if(how != "" && how != "Gmsh" && how != "Optimize" && how != "Optimize2D" &&
     ...
     how != "UntangleMeshGeometry" && how != "HXT") {
  ...
  if(how == "" || how == "Gmsh" || how == "Optimize") {
    ...
    for(auto pRegion : pRegions) {
      ...
      pRegion->tetrahedra.resize(pMesh->tetrahedra.num);
      for (uint64_t i = 0; i < pMesh->tetrahedra.num ; i++){
        auto t = &pMesh->tetrahedra.node[4*i];
        auto tet = new MTetrahedron(pVertices[t[0]], pVertices[t[1]], pVertices[t[2]], pVertices[t[3]], i+1);
        pRegion->tetrahedra[i] = tet;
      }
