...
public:
  ...
  void faceTriple(std::uint32_t e, std::uint32_t *tri) const
  {
    ...
    std::uint32_t a = tetV[4 * t + tet4Faces[f][0]];
    std::uint32_t b = tetV[4 * t + tet4Faces[f][1]];
    std::uint32_t c = tetV[4 * t + tet4Faces[f][2]];
    if(a > b) std::swap(a, b);
    if(b > c) std::swap(b, c);
    if(a > b) std::swap(a, b);
    tri[0] = a;
  ...
  {
    const std::uint32_t t = e >> 2, f = e & 3;
    const double *a = &vXYZ[3 * tetV[4 * t + tet4Faces[f][0]]];
    const double *b = &vXYZ[3 * tetV[4 * t + tet4Faces[f][1]]];
    const double *c = &vXYZ[3 * tetV[4 * t + tet4Faces[f][2]]];
                     ...
                     std::vector<std::uint32_t> &newTets, GModel *model)
  {
    ...
    for(std::size_t k = 0; k < shell.size(); k++) {
      ...
      const std::uint32_t f0 = tetV[4 * t + tet4Faces[f][0]];
      const std::uint32_t f1 = tetV[4 * t + tet4Faces[f][1]];
      const std::uint32_t f2 = tetV[4 * t + tet4Faces[f][2]];
...
void refineRegionFlat(GRegion *gr, int maxIter,
