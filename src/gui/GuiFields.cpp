...
// The mesh size fields, what each of them takes and which of them the mesh
// is built from. A field carries its own options, so the fields of the
// pane are made from whatever the size field one has picked says it takes.
//
// What one types is held until Apply, and then said in the language of
// the .geo file, so that the change is in the script as well as in the
// model.
...
namespace {
  ...
  // which field the window is editing, and what one has typed into it so
  // far
  ...
  // a list of numbers as one types it: separated by commas, no braces
  ...
  // write down what one has typed, through the script, and only for what
  // really changed
  void _apply()
  {
    ::Field *f = _current();
  ...
  // what the Help tab says: what the field is for, then every option and
...
namespace {
  ...
  static Form describeFields()
  {
    ...
    // column of names
    p.sideEm = 13.;
    // the name of the field takes a line over the panes
    p.leastRows = 10;
    ...
    // --- which field, with what one does to it: the list between the button
    // that makes one and the two that act on the one that is picked
    {
        ...
        [](int i, bool on) {
          ...
          for(auto &kv : *m)
            if(kv.second && k++ == i) {
              ...
              _load(true);
              // another field is another set of options
              Gui::instance().fields.rebuild();
              return;
            }
        },
        false);
    ...
    // --- what it takes
    {
      ...
      for(const auto &kv : _options(field)) {
        ...
        // a path is chosen rather than typed
...
Ui::Form GuiFields::build()
{
  Ui::Form f = describeFields();
  f.id = "fields";
  return f;
}
