...
// DefaultOptions.h: they do not say which numbers are switches, which ones
// are enumerations and what their values mean, which ones belong together
// on a line, or how to name them for a human. The panels are therefore laid
// out by hand, in the tables below, and only the mechanism is generic. The
// tables are their own little language -- CHECK(), COMBO(), COLOR() --
// which _paneFor() turns into the panes and fields of a Ui::Form. What a
// choice writes is not always the index of the entry picked.
//
// The help string of the tooltip is taken from the option table, so it
// cannot drift from the option itself.
...
namespace {
  ...
  Ui::ColourMap _colourMapOf(int index)
  {
    ...
    // the eight numbers the map is computed from, each with its keys; which
    // of ipar and dpar a parameter lives in, and what its place in them is,
    // stops here
...
namespace {
  ...
  namespace {
    ...
    enum rowKind {
      ...
      RowColor,
      RowAllColors, // every colour of the category, in a scrolling pane
    ...
    struct optionRow {
      ...
      // it shares the line of the row before it, as its second column
      bool beside;
...
// one as wide as a field: it lines up with the values above it rather
// than being as wide as its own text
#define ACTION_OF(id, label, share) \
  {RowAction, id, label, nullptr, nullptr, 0., 0., nullptr, false, share}
// and one that starts the second column of that line rather than ending
// it
...
// and one that takes only part of a field's width
...
#define END {RowCheck, nullptr, nullptr, nullptr, nullptr, 0., 0., nullptr, false}
  ...
  // values of an enumeration neither start at zero nor go up in order --
  ...
  // the switches a menu button gathers
  ...
  // goes with it standing apart from the three
  ...
  const optionRow _meshAdvanced[] = {
    ...
    // a third of a field: it holds a count of elements
  ...
  // three rows of a matrix, each followed by an offset and a raise, under
  // two captions: the one tab that is three columns wide rather than two
  ...
  const optionTab _viewTabs[] = {
    ...
    // the list down the left: five categories and then one line per view,
    // since the View options are those of one view
    ...
    struct enableRule {
      ...
      // the custom range of a view is live when its range mode is Custom, its
      // two-dimensional axes at two of the positions
      const char *is;
    };
    ...
    // what each value may be dragged by: a step means the value can be slid
    // with the mouse, and it is also what decides how many decimals it shows
    struct stepRule {
      const char *category;
      const char *name;
      double step;
                    ...
                    const std::string &label, int num, const optionRow *row)
    {
      ...
      return bindOption(f, category, name, num);
    }

    // several options on one line under one label: a point, a direction or a
    // range
                 ...
                 const char *category, int num)
    {
      ...
      for(std::size_t k = 0; k < names.size(); k++) {
        ...
        // included: the nine boxes that map the components of a field are one
        // strip
        f.packed = true;
    ...
    // a table, as a pane: one column of rows, no titled sections
    Pane _paneFor(const optionTab &tab, const char *category, int num)
    {
      Pane pane;
      pane.label = tab.label;
      // on columns, the same for every row of a tab: two of them, and three
      // where a view has its transformation matrix, its offsets and its raises
      // side by side
      pane.columns = 3;
      for(int i = 0; tab.rows[i].name || tab.rows[i].label ||
        ...
        case RowAction: {
          ...
          if(row.vmin == 1.) {
            // at the right end of the line above
            Field gap;
            gap.kind = Spacer;
            gap.widthEm = 1.;
            gap.sameRow = true;
        ...
        case RowSphere: {
          // the disc one drags to say where the light comes from: it writes the
          // same three options the row above it holds, and sits at the right end
          // of that row, hanging over the rows under it
          std::vector<std::string> names;
          for(int k = 0; row.choices && row.choices[k]; k++)
            names.push_back(row.choices[k]);
          std::string cat = category;
        ...
        case RowColorMap: {
          // the map itself, filling the tab
          Field f;
          f.kind = ColorMap;
          f.rows = 0;
          int index = num;
          // the interfaces are given what to read and what to write, and never the
          // table, which is a Gmsh type
        ...
        case RowFunction: {
          ...
          // the heavy visualization is not in this build
        ...
        default: {
          ...
          // a switch at the right of the line above rather than on one of its own
  ...
  static Form describeOptions()
  {
    Form p;
    // an option can be changed from anywhere -- a shortcut, the tree, a
    // script -- and the window shows it: the fields read the options they
    // name, and this is the excuse to read them again
    p.refreshEvery = .25;
    // the same height whatever category it is showing: twelve lines, of which
    // the row of tabs is one
    p.leastRows = 11;

    // a line that is no longer there -- a view that has been closed -- falls
    // back to the first
    if(Gui::instance().options.category >= _numCategories + _views())
      Gui::instance().options.category = 0;
    const char *category = _categoryName(Gui::instance().options.category);
    // the window says which category it is showing
    p.title = std::string("Options - ") +
              ((Gui::instance().options.category < _numCategories) ?
                 _categoryLabels[Gui::instance().options.category] :
                 ("View [" + std::to_string(Gui::instance().options.category - _numCategories) +
                  "]"));
    ...
    p.side.push_back(chooseFrom(
      ...
      [](int i) { return i == Gui::instance().options.category; },
      [](int i, bool on) {
        if(on) {
          Gui::instance().options.category = i;
          if(i >= _numCategories) Gui::instance().options.view = i - _numCategories;
    ...
    int num = (Gui::instance().options.category >= _numCategories) ? Gui::instance().options.view : 0;
    ...
    // otherwise. Restoring the defaults is the last row of the general
    // options.
    Field again;
    again.kind = Action;
    again.label = "Redraw";
