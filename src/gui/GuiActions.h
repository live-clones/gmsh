...
// Actions triggered by the graphical user interface that do not depend on
// the widget toolkit: the menus and buttons of an interface are only in
// charge of the presentation. Whatever interaction they need goes through
// Gui.h, in particular Msg::GetAnswer() for the questions that must be
...
// write what the message console holds to a file
...
// one entry of the quick access menu of the status bar, by the name the
// menu gives it; see Menu::quickAccess()
void quickAccessAction(const std::string &what);
// whether that entry is on, for the ones that are a switch
bool quickAccessChecked(const std::string &what);

// --- solver / ONELAB

// run one of the actions of the ONELAB tree: "check", "check_always",
// "compute", "reset", "reload", "refresh", "stop", "kill", "save", "load"
// or "initialize"
...
struct highOrderParameters {
  ...
  // what the dialog offers instead: 0 fixed, 1 free
  int boundaryNodes;
...
// the parameters the high order dialog holds
...
//
// These are the actions the modules tree starts: the user picks entities in
// the 3D view, and what they mean is written into the .geo script through
// scriptStringInterface.h, so that anything the interface does could also
// be typed by hand. They only return once the selection is aborted with
// 'q', so, like the other blocking loops, they must be started from a
// deferred action in an immediate-mode interface.
//
// Their parameters are read again at each 'e' rather than passed by value,
// because the panel they come from stays usable while the loop runs.
...
// the fields of one pane of the "Elementary Entity Context": pane says
// which entity is being created (0 Parameter, 1 Point, 2 Circle,
// 3 Ellipse, 4 Disk, 5 Rectangle, 6 Sphere, 7 Cylinder, 8 Box, 9 Torus,
// 10 Cone, 11 Wedge); for every pane but the first, the first three fields
// are the X, Y and Z the mouse drives
...
// the parameters the context dialogs hold: the dialogs are declared once
// and every interface binds its widgets to the same variables, which the
// interactive actions read live
...
// Orient or reset one view. what is "x", "y" or "z" to point that axis
// out of the screen, "r" to turn the view a quarter turn around the axis
// perpendicular to it, "1:1" to drop the translation and the zoom, or
// "reset" to drop the rotation as well. reverse (Shift) is the opposite
// axis direction, the opposite rotation, or, for "1:1", recomputing the
// bounding box around what is visible.
...
// one entry of the menu a view carries in the modules tree, by the name
// the menu gives it, on the view of the given index; see
// Menu::viewActions()
...
// the same, the way the status bar and the arrow keys ask for it:
// `forward` says which way, and `views` steps through the views rather
// than through the time steps
void animationStepBy(bool forward, bool views = false);
// step it if it is time to; how long a step lasts, how big it is and
// whether it cycles are read here
void animationTick();
