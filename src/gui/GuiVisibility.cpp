...
// What of the model is drawn: the list of entities and their state come
// from VisibilityList; what is here is hiding by number, hiding by
// picking, and saying which model or view each graphic window shows.
...
namespace {
  ...
  // what the window is showing and how it applies what one asks of it
  ...
  // what is picked in the list and in the tree, only put into the model
  // when one presses Apply
  struct picking {
  ...
  // entities is not put in one unless one insists, and insisting is
  // remembered
  bool &_treeWanted()
  {
  ...
  // the entities of the models, as the hierarchy they are; a line is named
  // by where it sits -- "0/1/4" -- rather than by what it says, since two
  // entities of two models may say the same thing
  ...
  void _applyTree()
  {
    ...
    // nothing is seen but what is picked
  ...
  // the list, as it is now; what is picked is read from the model only when
  // the list is not the same list any more, so that a pick survives until
  // it is applied
  void _refreshList()
  {
...
namespace {
  ...
  void _applyPicked(char mode, bool physical)
  {
    ...
    if(CTX::instance()->pickElements) {
      const std::vector<MElement *> &elements = Gui::instance().selectedElements();
      for(std::size_t i = 0; i < elements.size(); i++)
        elements[i]->setVisibility(mode);
    }
    else {
      const std::vector<GVertex *> &vertices = Gui::instance().selectedVertices();
...
namespace {
  ...
  namespace {
    ...
    void _byNumber(std::vector<Field> &into, const std::string &label, int what)
    {
      ...
      // in a column of its own, so that the buttons of every line stand under
      // one another whatever the line is called, and all of one width
    ...
    {
      // all of one width, whatever each says
      Field hide = does("Hide " + what,
  ...
  static Form describeVisibility()
  {
    Form p;
    p.title = "Visibility";
    // eighteen lines tall whatever tab it is on, of which the row of tabs and
    // the two lines under it take three: a list that fills what is left has
    ...
    // --- the list of what there is, and what of it is drawn
    {
      ...
      // half of it, and three that sort it, each the other way round when
      // pressed again
      Field all = does("*", []() {
        bool none = true;
    ...
    // --- the same, as the hierarchy it is
    {
      ...
      // a model of more than ten thousand entities is not put in a tree unless
      // one insists
...
Ui::Form GuiVisibility::build()
{
  Ui::Form f = describeVisibility();
  f.id = "visibility";
  return f;
}
