...
// The parameters a solver attached to one entity: whatever the templates
// named "ONELAB Context/<Dim> Template/..." say, instantiated for the entity
// that was double-clicked.
//
// The first half of the file is what a ONELAB parameter does when someone
// changes it: its attributes may set a Gmsh option, ask the server to hide
// or reset others, and have the solver run again.
...
namespace {
  ...
  struct contextState {
    ...
    // to a place in this list rather than to a name, so that a second entity
    // made from the same template does not have to build the window again
    std::vector<std::string> names;
  };
  ...
  // they are not on the server yet, sorted by name
  void _instantiate()
  {
    contextState &s = _state();
    s.names.clear();
  ...
  // a number: an enumeration when it names its values, a check box when it
  // is a yes or no, something one reads when it is read-only, and a value
  // one types otherwise
                         ...
                         const onelab::number &p)
  {
    ...
    if(p.getReadOnly()) {
      ...
      // in the list: a choice that is not a whole number would not survive
                 ...
                 Ui::Field &f)
  {
    ...
    range.action = [get, readOnlyRange]() {
      ...
      if(readOnlyRange) {
        Gui::instance().inputDialog("Range or choices", said,
                         "", true);
        return;
      }
  ...
  // the little buttons a line carries after its value
  void _trailing(const GuiOnelab::getString &get, const onelab::string &p,
                 Ui::Field &f)
  {
      ...
      b.menu = [get, readOnly]() {
        ...
        if(!readOnly) {
          ...
          choose.action = [get]() {
            ...
            if(!get(q)) return;
            std::string name = q.getValue();
            if(Gui::instance().fileDialog(0, "Choose", "", name)) _write(get, name);
          };
  ...
  // words: a button when the parameter is a macro, something one reads when
  // it is read-only, and a value one types otherwise, with what one may
  // want to type behind a little arrow when it says what that is
  ...
  // where a parameter asks to be drawn, read out of the "Aspect" attribute:
  // on a line of its own, or sharing one with the two beside it
...
namespace {
  ...
  static Form describeOnelabContext()
  {
    Form p;
    p.title = "Parameters";
    ...
    // physical groups it belongs to
    {
      ...
      which.changed = []() {
        _highlight();
        // another entity, or one of its groups, has parameters of its own
        Gui::instance().onelabContext.rebuild();
        // let the solver react to the entity or group that was picked
        if(CTX::instance()->solver.autoCheck) Gui::instance().onelabAction("check");
      };
...
Ui::Form GuiOnelabContext::build()
{
  Ui::Form f = describeOnelabContext();
  f.id = "onelabContext";
  return f;
}


void GuiOnelabContext::show(int dim, int tag)
{
#if defined(HAVE_ONELAB)
  contextState &s = _state();
  s.dim = dim;
  s.tag = tag;
  _look();
  // the last physical group it belongs to, and the entity itself when it is
  // in none
