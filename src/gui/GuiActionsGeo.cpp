...
// The interactive geometry and mesh-definition actions of the modules
// tree: the ones that let the user pick entities in the 3D view and write
// what they mean into the .geo script.
//
// They all follow the same shape: loop on Gui::instance().selectEntity(), gather what
// was picked, and act on 'e'. 'u' takes the last pick back, 'q' leaves. The
// parameters come from a getter rather than from a copy, so that the panel
// they are typed in stays live while the loop runs.
...
//
// Fixed storage rather than a vector: the dialogs bind widgets to the
// address of each field, so those addresses must not move.
...
// make the entities one is about to pick visible
...
//
// While it is on, the pointer drives the first three fields of the pane,
// the entity being built is previewed on top of the model, and 'e' writes
// it into the script.
...
void geometryAddPointBasedEntity(int pane)
{
  ...
  // say which shape is being placed: the pointer writes the coordinates
  // into that pane's fields, the preview is drawn from them, and 'e' adds
  // that shape
...
void geometryBoolean(const std::string &op)
{
  ...
  while(1) {
    ...
    char ib = Gui::instance().selectEntity(geometryStore().selection);
    if(!Gui::instance().available()) return;
