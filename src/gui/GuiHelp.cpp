...
// The three windows of the Help menu: what the keyboard and the mouse do,
// what the options are worth right now, and what this build is. The
// shortcuts come from GetShortcutsUsage() and GetMouseUsage(), which is
// also what -help prints, and the listing from PrintOptions(), so neither
// can drift from what Gmsh really does.
...
namespace {
  ...
  // size, and about how many characters of the second one fit on a line;
  // the second is deliberately mean, since a line too many leaves a gap
  ...
  {
    Ui::Field key = Declare::says([keys]() { return keys; });
    key.packed = true;
    key.widthEm = _keysEm;
    pane.fields.push_back(key);
    Ui::Field says = Declare::says([what]() { return what; });
    says.sameRow = true;
    says.wraps = true;
  ...
  // what the listing shows and what one has picked in it; not rebuilt at
  // every frame, since PrintOptions() writes out every option there is
  struct listingState {
...
namespace {
  ...
  static Form describeCurrentOptions()
  {
    Form p;
    ...
    // the listing itself, filling what is left of the window
    {
      Field listing = chooseFrom(
        ...
        [](int i, bool on) {
          ...
          // the field under the list is bound to the option picked last
          Gui::instance().currentOptions.rebuild();
        },
        true);
      listing.rows = 0;
      listing.widthEm = 40.;
    ...
    // what the picked line is worth, where one can change it: bound to the
    // option the line names
    {
  ...
  static Form describeAbout()
  {
    Form p;
    p.title = "About Gmsh";

    Pane what;
    // a page, not a form: a heading, two paragraphs, the addresses one may
    // follow, and a list of what this build has
    Field page = prose([]() {
    ...
    // the column the text is set in, and the height it takes to read it
    // without scrolling
    page.widthEm = 24.;
    page.rows = 30;
...
Ui::Form GuiShortcuts::build()
{
  Ui::Form f = describeShortcuts();
  f.id = "shortcuts";
  return f;
}

Ui::Form GuiCurrentOptions::build()
{
  Ui::Form f = describeCurrentOptions();
  f.id = "currentOptions";
  return f;
}

Ui::Form GuiAbout::build()
{
  Ui::Form f = describeAbout();
  f.id = "about";
  return f;
}
