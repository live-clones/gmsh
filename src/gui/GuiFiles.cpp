...
// What the File menu does, once, for whichever interface is up.
// Everything here goes through two things the interface owns and nothing
// else: Gui::instance().fileDialog(), which shows the chooser, and Msg::GetAnswer(),
...
// what may be opened, by name: the chooser offers them all and narrows
// what it lists to the one that is picked
...
// what a model may be written as, by name: which was picked is what says
// the format, "Guess From Extension" being the one that says nothing and
// lets the name decide
...
void fileOpen(bool merge)
{
  ...
  int which = -1;
  if(!Gui::instance().fileDialog(Gui::OpenSeveral, merge ? "Merge" : "Open",
                      Gui::instance().inputFormats(), names, which))
    return;
#if defined(HAVE_POST)
  ...
  if(PView::list.size() != views) Gui::instance().openModule("Post-processing");
#endif
...
{
  std::string name;
  if(!Gui::instance().fileDialog(1, "New geometry file", "*.geo", name)) return;
...
void fileRename()
{
  ...
  if(!Gui::instance().fileDialog(1, "Rename model file", "", name)) return;
...
{
  std::string name = GModel::current()->getFileName();
  std::vector<Gui::FileFormat> formats = Gui::instance().outputFormats();
  int which = -1;
  if(!Gui::instance().fileDialog(Gui::Create, "Export", formats, name, which)) return;
  if(!_mayOverwrite(name)) return;
  ...
  // what that format takes, asked through whichever interface is up
  std::string entry =
    (which > 0 && which < (int)formats.size()) ? formats[which].name : "";
  int answer = Gui::instance().exportOptionsDialog(format, name, entry);
...
{
  if(what == "start") {
    Gui::instance().history.showRemoteCommand();
    return;
  }
  std::string arg;
  if(what == "merge") {
    arg = "/tmp/data.pos";
    if(!Gui::instance().inputDialog("File the remote Gmsh should merge:", arg)) return;
  }
