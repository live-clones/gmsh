...
// The plugins, what each of them takes and what it is run on. A plugin
// carries its own options, so the fields of the pane are made from
// whatever the plugin one has picked says it takes.
...
namespace {
  ...
  // whether it writes down what it does
  struct pluginState {
    std::string plugin;
  ...
  // what the window writes down when it is asked to
...
namespace {
  ...
  static Form describePlugins()
  {
    ...
    // --- which plugin, and which views it is to be run on
    {
      Field which = chooseFrom(
        ...
        [](int i, bool on) {
          ...
          // another plugin is another set of options
          Gui::instance().plugins.rebuild();
        },
        false);
      which.rows = 0;
      which.widthEm = 12.5;
    ...
    // --- what the plugin one has picked takes
    {
      ...
      GMSH_Plugin *plugin = _current();
      if(plugin) {
        // its own options, as it says it takes them: the words first, the numbers
        // after
      ...
      // and whether what it does is written down, on the line of Run
...
Ui::Form GuiPlugins::build()
{
  Ui::Form f = describePlugins();
  f.id = "plugins";
  return f;
}
