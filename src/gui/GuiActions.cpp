...
void meshInspectElements()
{
  ...
  while(1) {
    ...
    if(!Gui::instance().available()) return;
    if(ib == 'l' && Gui::instance().selectedElements().size()) {
...
{
  // the box is all six planes at once
...
{
  if(!Gui::instance().available() || !Gui::instance().statistics.visible()) return;
  statisticsCompute(elementQuality);
  Gui::instance().statistics.reload();
}
...
void optionsAction(const std::string &what)
{
  if(what == "rotation_center_select") {
...
#if defined(HAVE_POST)
      else if(Gui::instance().selectedViews().size() && Gui::instance().selectedViews()[0]->getData())
        pc = Gui::instance().selectedViews()[0]->getData()->getBoundingBox().center();
#endif
    ...
    // it stays up while one tries a shape, looks at it, and tries another
...
namespace {
...
#if defined(HAVE_POST)
  // how far a view may be raised or displaced before it says nothing, from
  // the range of the view and the size of the model
  ...
  // stands for; named here because the touch bar of macOS reaches for the
...
void solverOptionAction(const std::string &what)
{
  ...
  bool on = !*v;
  // "Remesh automatically" and "Show new views" are counts rather than
  // switches: two is on
...
void onelabRun(const std::string &actionIn)
{
  ...
  // revealing the solver afterwards is left to the caller
}
...
// Match a file name against a shell glob, and order file names the way a
// person expects. fnmatch() is the POSIX one, character classes included;
// _globMatch() stands in for it where there is none, and only knows '*'
// and '?'.
#if !defined(WIN32) || defined(__CYGWIN__)
#define GMSH_HAVE_FNMATCH
#endif

// compare two file names treating runs of digits as numbers, so that
// "out2" comes before "out10"
