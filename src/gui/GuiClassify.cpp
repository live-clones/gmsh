...
// Reclassifying a triangulation: what turns a mesh with no geometry under
// it -- an STL -- into a model. The edges whose dihedral angle is sharp
// enough are taken as the curves between surfaces, and the surfaces are
// split along them.
//
// It is not a form one fills: the three steps it is written in are three
// things one does, two of them by picking in the 3D view, and the panel
// greys out what does not apply yet.
...
namespace {
  ...
  // what the editor has gathered so far
  ...
  // whether anything has been selected: what one may do next hangs on it
  ...
  // show the edges alone, so that one can see what has been detected, and
  // put the surfaces back afterwards
  void _showOnlyEdges()
  {
    if(_state().onlyEdges) {
...
namespace {
  ...
  // the lines are what one is about to work on: turn them on as it opens
  static void prepareClassify()
  {
    opt_mesh_lines(0, GMSH_SET | GMSH_GUI, 1.);
    drawContext::global()->draw();
  }

  static Form describeClassify()
  {
    Form p;
...
Ui::Form GuiClassify::build()
{
  Ui::Form f = describeClassify();
  f.id = "classify";
  return f;
}

void GuiClassify::load() { prepareClassify(); }
