...
{
  _m = GModel::current();
  int computeJac = static_cast<int>(MeshQuality2Options_Number[0].def);
  int computeIGE = static_cast<int>(MeshQuality2Options_Number[1].def);
  int computeICN = static_cast<int>(MeshQuality2Options_Number[2].def);
  double threshold = MeshQuality2Options_Number[3].def;
  bool thresholdGreater = static_cast<bool>(MeshQuality2Options_Number[4].def);
  bool createView = static_cast<bool>(MeshQuality2Options_Number[5].def);
  bool recompute = static_cast<bool>(MeshQuality2Options_Number[6].def);
  int askedDim = static_cast<int>(MeshQuality2Options_Number[7].def);
  ...
  _numElementToScan = static_cast<int>(MeshQuality2Options_Number[8].def);
  _viewOrder = 0;
  _dataPViewJac.clear();
  _dataPViewIGE.clear();
  _dataPViewICN.clear();
...
void GMSH_AnalyseMeshQuality2Plugin::_computeMinMaxJandValidity(int dim)
{
  ...
  for(auto it = entities.begin(); it != entities.end(); ++it) {
    ...
    for(unsigned i = 0; i < num; ++i) {
      ...
      _data.push_back(data_elementMinMax2(el, min, max));
...
void GMSH_AnalyseMeshQuality2Plugin::_computeMinIGE(int dim)
{
...
void GMSH_AnalyseMeshQuality2Plugin::_computeMinICN(int dim)
{
...
int GMSH_AnalyseMeshQuality2Plugin::_hideWithThreshold(int askedDim,
                                                      int whichMeasure,
                                                      double threshold,
                                                      bool greater)
{
...
void GMSH_AnalyseMeshQuality2Plugin::_printStatJacobian()
{
  if(_data.empty()) {
...
void GMSH_AnalyseMeshQuality2Plugin::_printStatIGE()
{
  if(_data.empty()) {
    Msg::Info("No stat to print");
    return;
...
void GMSH_AnalyseMeshQuality2Plugin::_printStatICN()
{
  if(_data.empty()) {
    Msg::Info("No stat to print");
    return;
...
void GMSH_AnalyseMeshQuality2Plugin::_clear(int askedDim)
{
  _data.clear();
  if(askedDim < 4) {
...
void GMSH_AnalyseMeshQuality2Plugin::_computePointwiseQuantities(
...
void GMSH_AnalyseMeshQuality2Plugin::_setInterpolationMatrices(PView *view)
{
...
void GMSH_AnalyseMeshQuality2Plugin::_createPViewPointwise()
{
  if(_pwJac) {
