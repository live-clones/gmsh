  ...
  for (auto gv : gvs){
	...
	for (std::size_t j = 0 ; j < 3 ; j++){
	  if (gf->triangles[i]->getVertex(j) == toDelete){
	    MVertex *vv [3] = {v, gf->triangles[i]->getVertex((j+1)%3),gf->triangles[i]->getVertex((j+2)%3)};
	...
	for (std::size_t j = 0 ; j < 4 ; j++){	  
	  if (gf->quadrangles[i]->getVertex(j) == toDelete){
	    if (foundindex >= 0){
	      found = true;
	      MVertex *qq[4] = {gf->quadrangles[i]->getVertex((foundindex+0)%4),
		...
		for (size_t k=0;k<3;k++){
		  ...
		  if (ttj0 == tj1 && ttj1 == tj0){
		    ...
		    for (size_t l=0;l<4;l++){
		      if (vv[l]->onWhat()->dim() != 2 && vv[(l+1)%4]->onWhat()->dim() != 2){ 
			gf->quadrangles.push_back(new  MQuadrangle(vv[(l+0)%4],vv[(l+1)%4],vv[(l+2)%4],vv[(l+3)%4]));
...
static std::vector<MVertex*> splitedge (MEdge me,  std::map<MEdge, std::vector<MVertex*>, MEdgeLessThan > &split,  std::vector<double> &t){
...
void splitounette (std::vector<GFace*> &f , std::map<MElement*, double> &layers, std::vector<double> &widths) {
  ...
  for (auto gf : f){
    ...
    for (size_t i=0;i<gf->getNumMeshElements();i++)temp.push_back(gf->getMeshElement(i));
	...
	for (size_t j=1;j<old.size();j++)
	  ge->lines.push_back(old[j]);
	delete old.front();
      }      
      if (it1 != split.end()){
	...
	for (size_t j=2;j<s;j++)
	  ge->lines.push_back(new MLine(it1->second[s-j],it1->second[s-j-1]));
      }      
    }
  }
}
...
PView *GMSH_BoundaryLayerPlugin::execute(PView *v)
{
  ...
  std::map<MElement*, std::array<std::array<double, 2>, 4> >
    perfectShapes;
  ...
  {      
	...
	if (pts.size() == 3){
	  ...
	  if (a > 0)perfectShapes [gf->getMeshElement(i)] = {vs0,vs1,vs2,vs2};
	  else perfectShapes [gf->getMeshElement(i)]= {vs0,vs2,vs1,vs1};
	}
	else {
	  std::array<double, 2>  vs0 = {pts[0].x(),pts[0].y()};
	  std::array<double, 2>  vs1 = {pts[1].x(),pts[1].y()};
	  ...
	  double a = triangle_area_2d (vs0,vs1,vs2);
	  if (a > 0)perfectShapes [gf->getMeshElement(i)] = {vs0,vs1,vs2,vs3};
	  else perfectShapes [gf->getMeshElement(i)] = {vs3,vs2,vs1,vs0};
	}
      }
    }
  }
