...
void MsgProgressStatus::next()
{
  ...
  {
    const double avgFromBeginning = (currentTime - _initialTime) / _currentI;
    const double avgFromLast = (currentTime - _lastTime) / (_currentI - _lastI);
    const double avgTimePerElement = .5 * (avgFromBeginning + avgFromLast);
    const double remaining =
      avgTimePerElement * (_totalElementToTreat - _currentI);

    if(remaining < 60*2)
      Msg::ProgressMeter(_currentI, true,
