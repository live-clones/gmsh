...
StringXNumber MeshOptions_Number[] = {
  ...
  { F|O, "OptimizeQuadsFinalSplitCadDistanceRatio", opt_mesh_optimize_quads_final_split_cad_distance_ratio, 0.2,
    "Final V2 quad splitting: trigger distance to CAD divided by local target size; insert the other diagonal only if within tolerance and at least twice closer (negative disables CAD trigger; invalid quads are still split when possible)" },
  ...
  { F|O, "PackScalingOnTriangulation", opt_mesh_pack_scaling_on_triangulation, 0.75,
    "Edge-length scaling for the PACK background triangulation" },
  { F|O, "PackTargetSize", opt_mesh_pack_target_size, 0.,
    "Physical target edge length for PACK (non-positive: use the mesh size settings)" },
  { F|O, "OptimizeQuadsTargetSize", opt_mesh_optimize_quads_target_size, 0.,
    "Target edge length for quad optimization (non-positive: inherit PackTargetSize, then the mesh size field)" },
  ...
  { F|O, "Pack3D" , opt_mesh_pack_3d, 0 ,
    "Use tangent-frame propagation and oriented exclusion cubes in 3D for "
      "packing points on CAD and discrete surfaces (0: UV parallelograms, 1: 3D "
      "cubes)" },
  { F|O, "PackForceAllPoints" ,
    opt_mesh_pack_force_all_points, 0 ,
    "Force every point generated by 3D packing into the surface "
      "triangulation and report a warning if any insertion still fails" },
  ...
  { F|O, "RecombineMinimumQuality" , opt_mesh_recombine_minimum_quality , 0.01 ,
    "Minimum quality for quadrangles generated by recombination. Negative: "
      "accept all matched qualities; PACK with V2 also defers validity splits "
      "to test optimizer repair of the raw quads" },
