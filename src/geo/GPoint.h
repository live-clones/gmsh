...
// A model vertex.
class GPoint : public GEntity {
protected:
  std::vector<GCurve *> l_edges;
  double meshSize;

public:
  GPoint(GModel *m, int tag, double ms = MAX_LC);
  virtual ~GPoint();
  ...
  void addEdge(GCurve *e);
  void delEdge(GCurve *e);
  ...
  virtual std::vector<GSurface *> faces() const;
  ...
  bool isOnSeam(const GSurface *gf) const;

  // relocate mesh vertex using GPoint coordinates
