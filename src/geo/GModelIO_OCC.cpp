  ...
  bool removeObject, bool removeTool)
{
  ...
  std::vector<std::pair<int, int>> multiBindDimTags = outDimTags;

  // compute entities to return in outDimTags by first returning all the output
  // entities in the input-output map
  outDimTags.clear();
  ...
  // then remove any duplicate, keeping the ordering intact, so that in .geo
  // files one can infer input-ouput relationships as well
  ...
  // then add any newly, highest dimensional entities that were created but that
  // were not identified as modified/generated by the input entities (can happen
  // when remove == false, cf. #3331, #3333)
  std::set<std::pair<int, int>> o(outDimTags.begin(), outDimTags.end());
  int dim = 0;
  for(auto t : outDimTags) dim = std::max(dim, t.first);
  for(auto t : multiBindDimTags){
    if(t.first == dim && o.find(t) == o.end()) {
      outDimTags.push_back(t);
    }
  }
