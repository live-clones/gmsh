 ...
 OverlapCollection<dim> quickOverlap(GModel *const model)
{
  ...
  // Function that takes a partitioned entity and adds fills the dictionary for all used (not owned!) vertices
  ...
  // For each partition and each partitioned entity, we keep a dict mapping a partitioned entity to the elements of its overlaps.
  // These elements are not separed by origin
  ...
  auto handleEntity = [&overlap, &vertexToPartitions](GEntity *entity) {
    ...
    for(size_t e = 0; e < partitionEntity->getNumMeshElements(); ++e) {
      ...
      for(size_t v = 0; v < element->getNumVertices(); ++v) {
        ...
        // It is critical in some cases to check we uses partitions of the vertex and not the partitions of its entities
        // (Particular case when a vertex is on partitions that do not use the interface on which it lies, if the BREP isn't perfect)
        auto parts = vertexToPartitions[element->getVertex(v)];
        // If this element has a vertex used by other partitions, it is part of their overlap
        for(int part : parts) {
          if(part != partition) {
                             ...
                             OverlapCollection<dim> &overlaps)
{
  ...
  // We will loop over all elements of the dimension. If it shares a vertex with one already in the overlap, we add it.
  ...
  if constexpr (dim == 2) {
    for (auto it = model->firstFace(); it != model->lastFace(); ++it) {
      auto face = *it;
      auto partitionEntity = dynamic_cast<typename EntityTraits<dim>::PartitionEntity *>(face);
      if (partitionEntity) {
    ...
    for (auto it = model->firstRegion(); it != model->lastRegion(); ++it) {
      auto region = *it;
      auto partitionEntity = dynamic_cast<typename EntityTraits<dim>::PartitionEntity *>(region);
      if (partitionEntity) {
        addElements(partitionEntity);
      }
    }
...
void buildOverlapEntities(GModel* const model, const OverlapCollection<dim>& overlaps)
{
  ...
  for (size_t i = 0; i < overlaps.size(); ++i) {
    ...
    for (const auto& [covered, elements] : overlaps[i]) {
      ...
      if (!add)
        Msg::Error("Failed to add overlap entity for partition %d and covered entity with tag %d. (Tag already existing)",
      ...
      if (!parent) {
        Msg::Error("Parent entity of dim %d and tag %d is not of the expected type or was not found.",
