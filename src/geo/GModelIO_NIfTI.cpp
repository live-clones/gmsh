...
/*
static int readAtlas (GModel *gm, const char* atlasFilename, int dim[3], int nvox, float * img){
...
#define kLabelStrLen 32
  ...
  for (int i = 1; i <= nLabel; i++) {
    ...
    if (meshify(imgbinary, dim, originalMC, 0.5, &tris, &pts, &ntri,
		&npt, preSmooth, onlyLargest, fillBubbles, 0) != EXIT_SUCCESS)continue;
...
int GModel::readNII(const std::string &name, float isolevel, int isoDarkMediumBright123,
		    float reduceFraction, int preSmooth, bool onlyLargest,
		    bool fillBubbles, int postSmooth){
  nifti_1_header hdr;
  ...
  int originalMC = 0;
  // marching cubes...
  if (meshify(img, dim, originalMC, isolevel, &tris, &pts, &ntri,
	      &npt, preSmooth, onlyLargest, fillBubbles, 0) != EXIT_SUCCESS)
    return -1;
  ...
  if ((reduceFraction < 1.0) || (quality > 1)) { //lossless for high quality
    ...
    quadric_simplify_mesh(&pts, &tris, &npt, &ntri, target_count, agressiveness, 0, (quality > 1));
  }
  ...
  for (int i=0;i<ntri;i++)  
    face->triangles.push_back(new MTriangle(vertices[tris[i].x],
					    vertices[tris[i].y],
					    vertices[tris[i].z]));
...
int GModel::readNII(const std::string &name, float isolevel, int isoDarkMediumBright123,
		    float reduceFraction, int preSmooth, bool onlyLargest,
		    bool fillBubbles, int postSmooth){
  Msg::Warning ("Gmsh must be compiled with nifti to load  Neuroimaging Informatics Technology Initiative (NII) files");
  return -1;
}
#endif
