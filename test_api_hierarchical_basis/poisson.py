    ...
    for iGroup in vGroups:
        ...
        for tagEntity in vEntities:
            ...
            for elementType in vElementTypes:
               ...
               # nbaseFunction=numElements*(4+4*(order-1)+(order-1)*(order-1))
...
#                           sol[:,None])
    return sol[:,None],unknown2node[1:]
...
sol_lagrange,unknown2node=fem_solve_lagrange()
gmsh.finalize()
...
sol_legendre,unknown2node=fem_solve_legendre()

sview= gmsh.view.add("solution")
gmsh.view.addModelData(sview, 0, "", "NodeData", unknown2node, sol_legendre-sol_lagrange)
...
plt.plot(sol_legendre-sol_lagrange)
