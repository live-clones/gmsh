        ...
        for tagEntity in vEntities:
            ...
            for elementType in vElementTypes:
                ...
                if dimEntity==2 :
                    ...
                    ky,_=gmsh.model.mesh.getKeysForElements(elementType,'H1Legendre1',tagEntity,False)
                    weigh ,_,_,sf= model.mesh.getBasisFunctionsForElements(elementType,INTEGRATION,'H1Legendre1',tagEntity)            
                    weights = np.array(weigh).reshape((-1,4))[:,3]
                    numGaussPoints = weights.shape[0]
                   # sf = np.array(sf).reshape((numGaussPoints,numElements,-1))  
                    sf = np.array(sf).reshape((numElements,numGaussPoints,-1))  
                    _,_,_,dsf= model.mesh.getBasisFunctionsForElements(elementType,INTEGRATION,'GradH1Legendre1',tagEntity)

                    #dsf = np.array(dsf).reshape((numGaussPoints,numElements,-1))
                    dsf = np.array(dsf).reshape((numElements,numGaussPoints,-1))
                    ...
                    if namGroup == 'SourceCircle' or namGroup == 'SourceSquare':
                        ...
                        localrhs = load * np.einsum("egn,eg,g->en", sf, qdet, weights)
                    #assembling
