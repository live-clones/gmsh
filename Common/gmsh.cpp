...
GMSH_API void gmsh::model::mesh::efficiencyIndex(const std::string &forestFile, const std::string &dataFile){
  ...
  if(!forestFile.empty() && !dataFile.empty()){
    HXTStatus s = forestOptionsCreate(&forestOptions);
    if(s != HXT_STATUS_OK) Msg::Error("Probleme a la creation de forestOptions");
    Msg::Info("Efficiency index : loading size field from %s and %s files", forestFile.c_str(), dataFile.c_str());
    s = forestLoad(&forest, forestFile.c_str(), dataFile.c_str(), forestOptions);
  ...
  // Getting all MEdges from tetrahedra and triangles
  for(std::size_t i = 0; i < regions.size(); i++) {
  ...
  for(std::vector<MEdge>::iterator it = allElementEdgesFromVolume.begin(); it != allElementEdgesFromVolume.end(); ++it){
    ...
    if(!useQuadrature){
      ...
      size = 1.0;
      if(!forestFile.empty() && !dataFile.empty()){
        HXTStatus s = forestSearchOne(forest, x, y, z, &size, true);
        if(s != HXT_STATUS_OK) Msg::Error("Erreur forestSearchOne");
      }

      e = it->length()/size;
    } else{
      // Quadrature pour évaluer la longueur dans la métrique
      double ax = it->getVertex(0)->x();
      double ay = it->getVertex(0)->y();
      double az = it->getVertex(0)->z();
      double bx = it->getVertex(1)->x();
      double by = it->getVertex(1)->y();
      double bz = it->getVertex(1)->z();

      // double abs[10] = {-0.1488743389816312, 0.1488743389816312,
      ...
      // double weight[10] = {0.2955242247147529, 0.2955242247147529,
      ...
      double abs[3] = {0.0, -0.7745966692414834, 0.7745966692414834};
      double weight[3] = {0.8888888888888888, 0.5555555555555556, 0.5555555555555556};
      int ng = 3;
      double l = 0;
      // int ng = 10;
      for(int i = 0; i < ng; ++i){
        size = DBL_MAX;
        HXTStatus s = forestSearchOne(forest, ax + (abs[i]/2. + 1./2.)*(bx-ax),
                                              ay + (abs[i]/2. + 1./2.)*(by-ay),
                                              az + (abs[i]/2. + 1./2.)*(bz-az), &size, true);
        if(s != HXT_STATUS_OK) Msg::Error("Erreur forestSearchOne");
        l += weight[i] * sqrt( ( (bx-ax)*(bx-ax) + (by-ay)*(by-ay) + (bz-az)*(bz-az) )/(size*size) );
      }
  ...
  double t_fromVolume = exp( sum/allElementEdgesFromVolume.size() );
  double tNoBad_fromVolume = exp( sumNoBad/(allElementEdgesFromVolume.size()-badCounterVolume) );
  ...
  double t_fromSurface = exp( sum/allElementEdgesFromSurface.size() );
  double tNoBad_fromSurface = exp( sumNoBad/(allElementEdgesFromSurface.size()-badCounterSurface) );
  ...
  for(std::vector<MEdge>::iterator it = allElementEdgesFromVolume.begin(); it != allElementEdgesFromVolume.end(); ++it){
    ...
    if(it2 == lengthsAtVerticesFromVolume.end()){
      std::vector<double> lengths {it->length()};
      lengthsAtVerticesFromVolume.insert( std::pair<MVertex *, std::vector<double>>(v0, lengths) );
    } else{
    ...
    if(it2 == lengthsAtVerticesFromVolume.end()){
      std::vector<double> lengths {it->length()};
      lengthsAtVerticesFromVolume.insert( std::pair<MVertex *, std::vector<double>>(v1, lengths) );
    } else{
      it2->second.push_back(it->length());
    }
  }

  // Moyennes des longueurs d'arêtes
  ...
  printf("Average discrete gradient (from edges of volume : gradient des moyennes) = %f\n", dGAvg);


  // SURFACES

  // Insérer les longueurs min et max par vertex
  for(std::vector<MEdge>::iterator it = allElementEdgesFromSurface.begin(); it != allElementEdgesFromSurface.end(); ++it){
    v0 = it->getVertex(0);
    v1 = it->getVertex(1);
    ...
    if(it2 == lengthsAtVerticesFromSurface.end()){
      std::vector<double> lengths {it->length()};
      lengthsAtVerticesFromSurface.insert( std::pair<MVertex *, std::vector<double>>(v0, lengths) );
    } else{
      it2->second.push_back(it->length());
    }
    ...
    if(it2 == lengthsAtVerticesFromSurface.end()){
      std::vector<double> lengths {it->length()};
      lengthsAtVerticesFromSurface.insert( std::pair<MVertex *, std::vector<double>>(v1, lengths) );
    } else{
      it2->second.push_back(it->length());
    }
  }

  // Average incident edge length
  ...
  for(std::vector<MEdge>::iterator it = allElementEdgesFromSurface.begin(); it != allElementEdgesFromSurface.end(); ++it){
    v0 = it->getVertex(0);
    v1 = it->getVertex(1);
    std::map<MVertex *, std::vector<double>>::iterator it0 = lengthsAtVerticesFromSurface.find(v0);
    std::map<MVertex *, std::vector<double>>::iterator it1 = lengthsAtVerticesFromSurface.find(v1);
    double min = fmin(it0->second[0], it1->second[0]);
    double max = fmax(it0->second[0], it1->second[0]);
    dGAvg += max/min;
  }
  dGAvg /= allElementEdgesFromSurface.size();
  printf("Average discrete gradient (from edges of surface : gradient des moyennes) = %f\n", dGAvg);

  printf("Efficiency index from volumes  = %f\n",t_fromVolume);
  printf("%d bad edges out of %d (%f %%). Without them : t = %f.\n", badCounterVolume, allElementEdgesFromVolume.size(), 100.* (float) badCounterVolume/ (float) allElementEdgesFromVolume.size(), tNoBad_fromVolume);
  printf("Efficiency index from surfaces = %f\n",t_fromSurface);
  printf("%d bad edges out of %d (%f %%). Without them : t = %f.\n", badCounterSurface, allElementEdgesFromSurface.size(), 100. *(float) badCounterSurface/ (float) allElementEdgesFromSurface.size(),tNoBad_fromSurface);
