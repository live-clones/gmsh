...
void mumpserror2(int id, int subid)
{
  if(id < 0) {
...
linearSystemMUMPS2<double>::linearSystemMUMPS2()
{
  _n = 0;
  _nz = 0;
}

bool linearSystemMUMPS2<double>::isAllocated() const
{
  if(_n > 0)
    return true;
  else
...
void linearSystemMUMPS2<double>::allocate(int nbRows)
{
  _n = nbRows;
  _b.resize(_n);
  _x.resize(_n);
...
void linearSystemMUMPS2<double>::clear()
{
  _x.clear();
  _a.clear();
  _b.clear();
...
void linearSystemMUMPS2<double>::zeroMatrix()
{
  _nz = 0;
  _a.clear();
  _ij.clear();
...
void linearSystemMUMPS2<double>::zeroRightHandSide()
{
  for(std::size_t i = 0; i < _b.size(); i++) _b[i] = 0.;
}

void linearSystemMUMPS2<double>::zeroSolution()
{
  for(std::size_t i = 0; i < _x.size(); i++) _x[i] = 0.;
}

int linearSystemMUMPS2<double>::systemSolve()
{
...
int linearSystemMUMPS2<double>::systemComputeSolution() {
  // Fortran indices start from 1
  for(std::size_t i = 0; i < _irn.size(); i++) _irn[i]++;
  for(std::size_t i = 0; i < _jcn.size(); i++) _jcn[i]++;
  ...
  Msg::Debug("MUMPS computes the solution");
  id.job = 3;
  dmumps_c(&id);
  mumpserror2(id.info[0], id.info[1]);
...
int linearSystemMUMPS2<double>::systemDestroy() {
  Msg::Debug("MUMPS destroy");
  id.job = -2;
  dmumps_c(&id);
  Msg::Debug("MUMPS end");
...
double linearSystemMUMPS2<double>::normInfSolution() const
{
  DMUMPS_REAL norm = 0.;
...
void linearSystemMUMPS2<double>::addToMatrix(int row, int col, const double &val)
{
...
void linearSystemMUMPS2<double>::getFromMatrix(int row, int col,
                                              double &val) const
{
  // Msg::Error("getFromMatrix not implemented for linearSystemMUMPS2");
  if((int)_ij.size() <= row) {
    val = 0.;
    return;
  }
...
void linearSystemMUMPS2<double>::addToRightHandSide(int row, const double &val,
                                                   int ith)
{
...
void linearSystemMUMPS2<double>::getFromRightHandSide(int row, double &val) const
{
  if((int)_b.size() <= row) val = 0.;
  val = _b[row];
}

void linearSystemMUMPS2<double>::getFromSolution(int row, double &val) const
{
  //  printf("x[%d] = %g\n",row,_x[row]);
  if((int)_x.size() <= row)
    val = 0.;
...
void linearSystemMUMPS2<double>::addToSolution(int row, const double &val)
{
  if((int)_x.size() <= row) {
    _x.resize(row + 1);
    _x[row] = val;
