...
HierarchicalBasisH1Brick::HierarchicalBasisH1Brick(int order) {
    ...
    _numBubbleFunction = ((order >= 2) ? ((order - 1) * (order - 1) * (order - 1)) : 0);
...
double HierarchicalBasisH1Brick::_affineCoordinate(const int &j, const double &u, const double &v, const double &w) {
    switch(j) {
        case(1):
            return 0.5 * (1 + u);
        case(2):
        ...
        case(6):
            return 0.5 * (1 - w);
        default:
            throw std::runtime_error("j must be : 1<=j<=6");
    }
}
...
void HierarchicalBasisH1Brick::generateBasis(double const &u, double const &v, double const &w,
    ...
    for(int iEdge = 0; iEdge < _numEdge; iEdge++) {
        ...
        for(int indexEdgeFunc = 0; indexEdgeFunc < _pOrderEdge[iEdge] - 1; indexEdgeFunc++) {
            edgeBasis[indexEdgeBasis] = (*vectorTarget1)[indexEdgeFunc] * product[iEdge];
            indexEdgeBasis++;
        }
    }
    ...
    for(int iFace = 0; iFace < _numQuadFace; iFace++) {
        ...
        for(int index1 = 0; index1 < _pOrderFace[0][iFace] - 1; index1++) {
            for(int index2 = 0; index2 < _pOrderFace[1][iFace] - 1; index2++) {
                faceBasis[indexFaceFunction] = lambda[indexLambda] * (*vectorTarget1)[index1] * (*vectorTarget2)[index2];
                indexFaceFunction++;
            }
        }
    }
    ...
    for(int ipb1 = 0; ipb1 < _pb[0] - 1; ipb1++) {
        for(int ipb2 = 0; ipb2 < _pb[1] - 1; ipb2++) {
            for(int ipb3 = 0; ipb3 < _pb[2] - 1; ipb3++) {
                bubbleBasis[indexBubbleBasis] = lkVectorU[ipb1] * lkVectorV[ipb2] * lkVectorW[ipb3];
                indexBubbleBasis++;
            }
        }
    }
...
void HierarchicalBasisH1Brick::_someProductGrad(double const &u, double const &v, double const &w,
                                                std::vector<double> &product,
...
void HierarchicalBasisH1Brick::generateGradientBasis(double const &u, double const &v, double const &w,
                                                     std::vector<std::vector<double> > &gradientVertex,
                                                     std::vector<std::vector<double> > &gradientEdge,
                                                     std::vector<std::vector<double> > &gradientFace,
                                                     std::vector<std::vector<double> > &gradientBubble) {
    std::vector<double> product(12, 0);
    std::vector<std::vector<double> > gradientProduct(12, std::vector<double>(3, 0));
    std::vector<double> lambda(6, 0);
    std::vector<std::vector<double> > gradientLambda(6, std::vector<double>(3, 0));
    HierarchicalBasisH1Brick::_someProductGrad(u, v, w, product, gradientProduct, lambda, gradientLambda);
    // vertex gradient:
    for(int it = 0; it < 3; it++) {
        gradientVertex[0][it] = gradientLambda[1][it] * product[0] + gradientProduct[0][it] * lambda[1];
        gradientVertex[1][it] = gradientLambda[0][it] * product[0] + gradientProduct[0][it] * lambda[0];
        gradientVertex[2][it] = gradientLambda[0][it] * product[5] + gradientProduct[5][it] * lambda[0];
        gradientVertex[3][it] = gradientLambda[1][it] * product[5] + gradientProduct[5][it] * lambda[1];
        gradientVertex[4][it] = gradientLambda[1][it] * product[8] + gradientProduct[8][it] * lambda[1];
        gradientVertex[5][it] = gradientLambda[0][it] * product[8] + gradientProduct[8][it] * lambda[0];
        gradientVertex[6][it] = gradientLambda[0][it] * product[11] + gradientProduct[11][it] * lambda[0];
        gradientVertex[7][it] = gradientLambda[1][it] * product[11] + gradientProduct[11][it] * lambda[1];
    }
    std::vector<double> lkVectorU(_pb[0] - 1);
    std::vector<double> lkVectorV(_pb[1] - 1);
    std::vector<double> lkVectorW(_pb[2] - 1);
    std::vector<std::vector<double> > dlkVectorU(_pb[0] - 1, std::vector<double>(3, 0.));
    std::vector<std::vector<double> > dlkVectorV(_pb[1] - 1, std::vector<double>(3, 0.));
    std::vector<std::vector<double> > dlkVectorW(_pb[2] - 1, std::vector<double>(3, 0.));
    for(int it = 2; it <= _pb[0]; it++) {
        lkVectorU[it - 2] = OrthogonalPoly::EvalLobatto(it, u);
    ...
    for(int iEdge = 0; iEdge < _numEdge; iEdge++) {
        ...
        for(int indexEdgeFunc = 0; indexEdgeFunc < _pOrderEdge[iEdge] - 1; indexEdgeFunc++) {
            for(int it = 0; it < 3; it++) {
                gradientEdge[indexEdgeBasis][it] = (*dvectorTarget1)[indexEdgeFunc][it] * product[iEdge] +
            ...
            for(int index2 = 0; index2 < _pOrderFace[1][iFace] - 1; index2++) {
                for(int it = 0; it < 3; it++) {
                    gradientFace[indexFaceFunction][it] =
                        gradientLambda[indexLambda][it] * (*vectorTarget1)[index1] * (*vectorTarget2)[index2]
                    + lambda[indexLambda] * (*dvectorTarget1)[index1][it] * (*vectorTarget2)[index2]
                    + lambda[indexLambda] * (*vectorTarget1)[index1] * (*dvectorTarget2)[index2][it];
                }
                indexFaceFunction++;
            }
        }
    ...
    for(int ipb1 = 0; ipb1 < _pb[0] - 1; ipb1++) {
        for(int ipb2 = 0; ipb2 < _pb[1] - 1; ipb2++) {
            for(int ipb3 = 0; ipb3 < _pb[2] - 1; ipb3++) {
                gradientBubble[indexBubbleBasis][0] = dlkVectorU[ipb1][0] * lkVectorV[ipb2] * lkVectorW[ipb3];
                gradientBubble[indexBubbleBasis][1] = lkVectorU[ipb1] * dlkVectorV[ipb2][1] * lkVectorW[ipb3];
                gradientBubble[indexBubbleBasis][2] = lkVectorU[ipb1] * lkVectorV[ipb2] * dlkVectorW[ipb3][2];
                indexBubbleBasis++;
            }
        }
    }
...
void HierarchicalBasisH1Brick::orientEdge(int const &flagOrientation, int const &edgeNumber,
        ...
        if(flag3 == 1) {
            ...
            for(int it1 = 2; it1 <= _pOrderFace[1][faceNumber]; it1++) {
                for(int it2 = 2; it2 <= _pOrderFace[0][faceNumber]; it2++) {
                    ...
                    faceBasis[iterator] = lambda * lkVector1[it2 - 2] * lkVector2[it1 - 2] * impactFlag1 * impactFlag2;
                    iterator++;
                }
            }
        }
...
void HierarchicalBasisH1Brick::orientOneFace(double const &u, double const &v, double const &w,
    ...
    if(!(flag1 == 1 && flag2 == 1 && flag3 == 1)) {
        ...
        if(flag3 == 1) {
            ...
            std::vector<double> lkVector1(_pOrderFace[0][faceNumber] - 1);
            std::vector<double> lkVector2(_pOrderFace[1][faceNumber] - 1);
            std::vector<std::vector<double> > dlkVector1(_pOrderFace[0][faceNumber] - 1, std::vector<double>(3, 0.));
            std::vector<std::vector<double> > dlkVector2(_pOrderFace[1][faceNumber] - 1, std::vector<double>(3, 0.));
            for(int it = 2; it <= _pOrderFace[0][faceNumber]; it++) {
            ...
            for(int it1 = 2; it1 <= _pOrderFace[1][faceNumber]; it1++) {
                for(int it2 = 2; it2 <= _pOrderFace[0][faceNumber]; it2++) {
                    ...
                    for(int itVector = 0; itVector < 3; itVector++) {
                        gradientFace[iterator][itVector] = (gradientLambda[itVector] * lkVector1[it2 - 2] * lkVector2[it1 - 2]
                                                            + lambda * dlkVector1[it2 - 2][itVector] * lkVector2[it1 - 2]
                                                            + lambda * lkVector1[it2 - 2] * dlkVector2[it1 - 2][itVector]) * impactFlag1 * impactFlag2;
                    }
                    iterator++;
                }
            }
...
void HierarchicalBasisH1Brick::orientFace(int const &flag1, int const &flag2, int const &flag3, int const &faceNumber,
    ...
    int numFaceFunctions = (_pOrderFace[0][faceNumber] - 1) * (_pOrderFace[1][faceNumber] - 1);
...
void HierarchicalBasisH1Brick::getKeysInfo(std::vector<int> &functionTypeInfo, std::vector<int> &orderInfo) {
    for(int i = 0; i < 8; i++) {
