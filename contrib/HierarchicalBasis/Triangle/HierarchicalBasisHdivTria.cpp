...
double HierarchicalBasisHdivTria::_affineCoordinate(int const &j, double const &u, double const &v) {
    switch(j) {
        case(1):
            return 0.5 * (1 + v);
        case(2):
...
void HierarchicalBasisHdivTria::generateHdivBasis(double const &u, double const &v, double const &w,
    ...
    for(int i = 0; i < 3; i++) {
        gamma_0[0][i] = lambda3 * t2[i] / dotProduct(t2, n1) + lambda2 * t3[i] / dotProduct(t3, n1);
        gamma_0[1][i] = lambda1 * t3[i] / dotProduct(t3, n2) + lambda3 * t1[i] / dotProduct(t1, n2);
        gamma_0[2][i] = lambda2 * t1[i] / dotProduct(t1, n3) + lambda1 * t2[i] / dotProduct(t2, n3);

        gamma_1[0][i] = lambda3 * t2[i] / dotProduct(t2, n1) - lambda2 * t3[i] / dotProduct(t3, n1);
        gamma_1[1][i] = lambda1 * t3[i] / dotProduct(t3, n2) - lambda3 * t1[i] / dotProduct(t1, n2);
        gamma_1[2][i] = lambda2 * t1[i] / dotProduct(t1, n3) - lambda1 * t2[i] / dotProduct(t2, n3);
    }
    ...
    legendreVector[0] = std::vector<double>(std::max(std::max(_pOrderEdge[0], _pf - 1), 0));
    legendreVector[1] = std::vector<double>(std::max(std::max(_pOrderEdge[1], _pf - 1), 0));
    legendreVector[2] = std::vector<double>(std::max(std::max(_pOrderEdge[2], _pf - 1), 0));
    ...
    for(int i = 0; i < _numEdge; i++) {
        for(int j = 0; j < 3; j++) {
            edgeBasis[edgeIt][j] = jacob * gamma_0[i][j];
        }
        edgeIt++;
        if(_pOrderEdge[i] >= 1) {
            for(int j = 0; j < 3; j++) {
            ...
            for(int iedge = 2; iedge <= _pOrderEdge[i]; iedge++) {
                for(int j = 0; j < 3; j++) {
                    edgeBasis[edgeIt][j] = jacob * (
                                                    (2 * float(iedge) - 1) / float(iedge) * legendreVector[i][iedge - 1] * gamma_1[i][j]
                                                    - (float(iedge) - 1) / float(iedge) * legendreVector[i][iedge - 2] * gamma_0[i][j]
                                                    );
                }
                edgeIt++;
            }
        ...
        for(int i1 = 2; i1 <= _pf; i1++) {
            for(int j = 0; j < 3; j++) {
                faceBasis[faceIt][j] = jacob * product * legendreVector[i][i1 - 2] * tD[j];
            }
            faceIt++;
        }
    }
    ...
    for(int n1 = 0; n1 < _pf - 2; n1++) {
        for(int n2 = 0; n2 < _pf - 2 - n1; n2++) {
            faceBasis[faceIt][0] = 0.5 * jacob * (product * legendreVector[0][n1] * legendreVector[2][n2]);
...
void HierarchicalBasisHdivTria::generateDivBasis(double const &u, double const &v, double const &w,
...
// to map onto the reference domain of gmsh:
    ...
    dlegendreVector[0] = std::vector<double>(std::max(std::max(_pOrderEdge[0], _pf - 1), 0));
    dlegendreVector[1] = std::vector<double>(std::max(std::max(_pOrderEdge[1], _pf - 1), 0));
    dlegendreVector[2] = std::vector<double>(std::max(std::max(_pOrderEdge[2], _pf - 1), 0));
    ...
    for(int i = 0; i < _numEdge; i++) {
        ...
        if(_pOrderEdge[i] >= 1) {
            ...
            edgeIt++;
            for(int iedge = 2; iedge <= _pOrderEdge[i]; iedge++) {
                edgeBasis[edgeIt] = det * (
                    (2 * float(iedge) - 1) / float(iedge) * (  dsubtraction[i][0] * dlegendreVector[i][iedge - 1] * gamma_1[i][0]
                                                             + dsubtraction[i][1] * dlegendreVector[i][iedge - 1] * gamma_1[i][1])
                   -(float(iedge) - 1) / float(iedge) * (  dsubtraction[i][0] * dlegendreVector[i][iedge - 2] * gamma_0[i][0]
                                                         + dsubtraction[i][1] * dlegendreVector[i][iedge - 2] * gamma_0[i][1]
    ...
    for(int n1 = 2; n1 <= _pf; n1++) {
        faceBasis[faceIt] = 0.5 * det * (dlambda23 * legendreVector[0][n1 - 2] + prod32 * dsubtraction[0][0] * dlegendreVector[0][n1 - 2]);
    ...
    for(int n1 = 2; n1 <= _pf; n1++) {
        faceBasis[faceIt] = -0.5 * det * (
                                          dlambda13U * legendreVector[1][n1 - 2] + prod13 * dsubtraction[1][0] * dlegendreVector[1][n1 - 2]
                                      - ( dlambda13V * legendreVector[1][n1 - 2] + prod13 * dsubtraction[1][1] * dlegendreVector[1][n1 - 2]));
        faceIt++;
    }
    double dlambda12 = 0.5 * (lambda2 - lambda1);
    double prod12 = lambda2 * lambda1;
    for(int n1 = 2; n1 <= _pf; n1++) {
        faceBasis[faceIt] = -0.5 * det * (dlambda12 * legendreVector[2][n1 - 2] + prod12 * dsubtraction[2][1] * dlegendreVector[2][n1 - 2]);
        faceIt++;
    }
    ...
    for(int n1 = 0; n1 < _pf - 2; n1++) {
        for(int n2 = 0; n2 < _pf - 2 - n1; n2++) {
            faceBasis[faceIt] = 0.5 * det * (
                                    ...
                                    + prod123 * dsubtraction[0][0] * dlegendreVector[0][n1] * legendreVector[2][n2]
                                    + prod123 * dsubtraction[2][0] * legendreVector[0][n1]  * dlegendreVector[2][n2]);
    ...
    for(int n1 = 0; n1 < _pf - 2; n1++) {
        for(int n2 = 0; n2 < _pf - 2 - n1; n2++) {
            faceBasis[faceIt] = 0.5 * det * (
                                    ...
                                    + prod123 * dsubtraction[0][1] * dlegendreVector[0][n1] * legendreVector[2][n2]
                                    + prod123 * dsubtraction[2][1] * legendreVector[0][n1] * dlegendreVector[2][n2] );
            faceIt++;
        }
    }
}

void HierarchicalBasisHdivTria::orientEdgeFunctionsForNegativeFlag(std::vector<std::vector<double> > &edgeFunctions) {
    int constant1 = 0;
    int constant2 = 0;
...
void HierarchicalBasisHdivTria::orientEdgeFunctionsForNegativeFlag(std::vector<double> &edgeFunctions) {
    int constant1 = 0;
    int constant2 = 0;
    for(int edgeNumber = 0; edgeNumber < _numEdge; edgeNumber++) {
        constant2 = 0;
    ...
    if(!(flag1 == 0 && flag2 == 1)) {
        if(typeFunction == "HdivLegendre") {
            ...
            for(int i = 0; i < 3; i++) {
                ...
                for(int i1 = 2; i1 <= _pf; i1++) {
                    for(int j = 0; j < 2; j++) {
                        faceFunctions[faceIt][j] = jacob * product2 * OrthogonalPoly::EvalLegendre(i1 - 2, sub[i]) * (*tangent)[j];
                    }
            ...
            for(int n1 = 0; n1 < _pf - 2; n1++) {
                ...
                for(int n2 = 0; n2 < _pf - 2 - n1; n2++) {
                    faceFunctions[faceIt][0] = jacob * product * LSub1 * LSub2[n2] * dlambda[1][0];
                    faceFunctions[faceIt][1] = jacob * product * LSub1 * LSub2[n2] * dlambda[1][1];
                    faceIt++;
                }
            }
            //int faceIt2 = 3 * (_pf - 1);
            for(int n1 = 0; n1 < _pf - 2; n1++) {
                double LSub1 = OrthogonalPoly::EvalLegendre(n1, sub1);
                for(int n2 = 0; n2 < _pf - 2 - n1; n2++) {
                    faceFunctions[faceIt][0] = jacob * product * LSub1 * LSub2[n2] * dlambda[2][0];
                    faceFunctions[faceIt][1] = jacob * product * LSub1 * LSub2[n2] * dlambda[2][1];
                    faceIt++;
                    //faceIt2++;
                }
            }
...
void HierarchicalBasisHdivTria::orientOneFace(double const &u, double const &v, double const &w,
    ...
    if(!(flag1 == 0 && flag2 == 1)) {
        if("DivHdivLegendre" == typeFunction) {
            ...
            for(int i1 = 2; i1 <= _pf; i1++) {
                double dphiU = dlambda23U * OrthogonalPoly::EvalLegendre(i1 - 2, sub[0]) +
                prod32 * dsub[0][0] * OrthogonalPoly::EvalDLegendre(i1 - 2, sub[0]);
                double dphiV = dlambda23V * OrthogonalPoly::EvalLegendre(i1 - 2, sub[0]) +
            ...
            for(int i1 = 2; i1 <= _pf; i1++) {
                double dphiU = dlambda13U * OrthogonalPoly::EvalLegendre(i1 - 2, sub[1]) +
                    prod13 * dsub[1][0] * OrthogonalPoly::EvalDLegendre(i1 - 2, sub[1]);
                double dphiV = dlambda13V * OrthogonalPoly::EvalLegendre(i1 - 2, sub[1]) +
            ...
            for(int i1 = 2; i1 <= _pf; i1++) {
                double dphiU = dlambda12U * OrthogonalPoly::EvalLegendre(i1 - 2, sub[2]) +
                        prod12 * dsub[2][0] * OrthogonalPoly::EvalDLegendre(i1 - 2, sub[2]);
                double dphiV = dlambda12V * OrthogonalPoly::EvalLegendre(i1 - 2, sub[2]) +
            ...
            for(int n1 = 0; n1 < _pf - 2; n1++) {
                ...
                ( dProduct[0] * LSubAC[n2] * LSubBA[n1]
                + product * dsubBA[0] * LSubAC[n2] * dLSubBA[n1]
                + product * dsubAC[0] * dLSubAC[n2] * LSubBA[n1] ) * dlambda[2][0]
              + (dProduct[1] * LSubAC[n2] * LSubBA[n1]
                + product * dsubBA[1] * LSubAC[n2] * dLSubBA[n1]
                + product * dsubAC[1] * dLSubAC[n2] * LSubBA[n1] ) * dlambda[2][1]
                                                   );
                    faceIt++;
                }
            }
...
void HierarchicalBasisHdivTria::orientFace(int const &flag1, int const &flag2, int const &flag3, int const &faceNumber,
...
void HierarchicalBasisHdivTria::getKeysInfo(std::vector<int> &functionTypeInfo, std::vector<int> &orderInfo) {
    int it = 0;
