...
double HierarchicalBasisHcurlTria::dotProduct(const std::vector<double> &u, const std::vector<double> &v) {
...
double HierarchicalBasisHcurlTria::_affineCoordinate(int const &j, double const &u, double const &v) {
    switch(j) {
...
void HierarchicalBasisHcurlTria::generateHcurlBasis(double const &u, double const &v, double const &w,
    ...
    for(int i = 0; i < 3; i++) {
        psie_0[0][i] = lambda3 * n2[i] / dotProduct(n2, t1) + lambda2 * n3[i] / dotProduct(n3, t1);
        psie_0[1][i] = lambda1 * n3[i] / dotProduct(n3, t2) + lambda3 * n1[i] / dotProduct(n1, t2);
        psie_0[2][i] = lambda2 * n1[i] / dotProduct(n1, t3) + lambda1 * n2[i] / dotProduct(n2, t3);

        psie_1[0][i] = lambda3 * n2[i] / dotProduct(n2, t1) - lambda2 * n3[i] / dotProduct(n3, t1);
        psie_1[1][i] = lambda1 * n3[i] / dotProduct(n3, t2) - lambda3 * n1[i] / dotProduct(n1, t2);
        psie_1[2][i] = lambda2 * n1[i] / dotProduct(n1, t3) - lambda1 * n2[i] / dotProduct(n2, t3);
    }
    ...
    for(int i = 0; i < _numEdge; i++) {
        for(int j = 0; j < 3; j++) {
            edgeBasis[edgeIt][j] = jacob * psie_0[i][j];
        }
        edgeIt++;
        if(_pOrderEdge[i] >= 1) {
            for(int j = 0; j < 3; j++) {
            ...
            for(int iedge = 2; iedge <= _pOrderEdge[i]; iedge++) {
                for(int j = 0; j < 3; j++) {
                    edgeBasis[edgeIt][j] = jacob * (
                                                    (2 * float(iedge) - 1) / float(iedge) * legendreVector[i][iedge - 1] * psie_1[i][j]
                                                    - (float(iedge) - 1) / float(iedge) * legendreVector[i][iedge - 2] * psie_0[i][j]
                                                    );
                }
                edgeIt++;
            }
        ...
        for(int i1 = 2; i1 <= _pf; i1++) {
            for(int j = 0; j < 3; j++) {
                faceBasis[faceIt][j] = jacob * product * legendreVector[i][i1 - 2] * nD[j];
            }
            faceIt++;
        }
    }
...
void HierarchicalBasisHcurlTria::orientEdge(int const &flagOrientation, int const &edgeNumber,
...
void HierarchicalBasisHcurlTria::orientEdgeFunctionsForNegativeFlag(std::vector<std::vector<double> > &edgeFunctions) {
    int constant1 = 0;
    int constant2 = 0;
...
void HierarchicalBasisHcurlTria::generateCurlBasis(double const &u, double const &v, double const &w,
                                                   std::vector<std::vector<double> > &edgeBasis,
                                                   std::vector<std::vector<double> > &faceBasis,
                                                   std::vector<std::vector<double> > &bubbleBasis) {
//***
// to map onto the reference domain of gmsh:
    ...
    for(int i = 0; i < _numEdge; i++) {
        ...
        if(_pOrderEdge[i] >= 1) {
            ...
            edgeIt++;
            for(int iedge = 2; iedge <= _pOrderEdge[i]; iedge++) {
                edgeBasis[edgeIt][2] = det * (
                    (2 * float(iedge) - 1) / float(iedge) * ( dsubtraction[i][0] * dlegendreVector[i][iedge - 1] * psie_1[i][1]
                                                             -dsubtraction[i][1] * dlegendreVector[i][iedge - 1] * psie_1[i][0])
                                              - (float(iedge) - 1) / float(iedge) * (curlpsie_0[i] * legendreVector[i][iedge - 2]
                                                                                     + dsubtraction[i][0] * dlegendreVector[i][iedge - 2] * psie_0[i][1] - dsubtraction[i][1] * dlegendreVector[i][iedge - 2] * psie_0[i][0]));
                edgeIt++;
            }
        }
    }
    ...
    for(int n1 = 2; n1 <= _pf; n1++) {
        faceBasis[faceIt][2] = 0.5 * det * (dlambda23 * legendreVector[0][n1 - 2] +
    ...
    for(int n1 = 2; n1 <= _pf; n1++) {
        faceBasis[faceIt][2] = -det * 0.5 * (dlambda13U * legendreVector[1][n1 - 2] +
    ...
    for(int n1 = 2; n1 <= _pf; n1++) {
        faceBasis[faceIt][2] = -0.5 * det * (dlambda12 * legendreVector[2][n1 - 2] +
    ...
    for(int n1 = 0; n1 < _pf - 2; n1++) {
        for(int n2 = 0; n2 < _pf - 2 - n1; n2++) {
            faceBasis[faceIt][2] = -0.5 * det * (dlambda123V * legendreVector[0][n1] * legendreVector[2][n2] +
                                                 prod123 * dsubtraction[0][1] * dlegendreVector[0][n1] *
                                                 legendreVector[2][n2] + prod123 * dsubtraction[2][1]
                                                 * legendreVector[0][n1] * dlegendreVector[2][n2]);
    ...
    for(int n1 = 0; n1 < _pf - 2; n1++) {
        for(int n2 = 0; n2 < _pf - 2 - n1; n2++) {
            faceBasis[faceIt][2] = 0.5 * det * (dlambda123U * legendreVector[0][n1] * legendreVector[2][n2] +
                                                prod123 * dsubtraction[0][0] * dlegendreVector[0][n1] *
                                                legendreVector[2][n2] + prod123 * dsubtraction[2][0] * legendreVector[0][n1] *
...
void HierarchicalBasisHcurlTria::orientOneFace(double const &u, double const &v, double const &w,
    ...
    if(!(flag1 == 0 && flag2 == 1)) {
        if(typeFunction == "HcurlLegendre") {
            ...
            for(int i = 0; i < 3; i++) {
                ...
                for(int i1 = 2; i1 <= _pf; i1++) {
                    for(int j = 0; j < 2; j++) {
                        faceFunctions[faceIt][j] = jacob * product2 * OrthogonalPoly::EvalLegendre(i1 - 2, sub[i]) * (*normal)[j];
                    }
            ...
            for(int n1 = 0; n1 < _pf - 2; n1++) {
                for(int n2 = 0; n2 < _pf - 2 - n1; n2++) {
                    faceFunctions[faceIt][2] = det * ((dProduct[0] * LSubAC[n2] * LSubBA[n1] +
                                                       product * dsubBA[0] * LSubAC[n2] * dLSubBA[n1] +
                                                       product * dsubAC[0] * dLSubAC[n2] * LSubBA[n1]) *
                                                      dlambda[1][1] - dlambda[1][0] * (dProduct[1] * LSubAC[n2] * LSubBA[n1] +
            ...
            for(int n1 = 0; n1 < _pf - 2; n1++) {
                for(int n2 = 0; n2 < _pf - 2 - n1; n2++) {
                    faceFunctions[faceIt][2] = det * ((dProduct[0] * LSubAC[n2] * LSubBA[n1] +
                                                       product * dsubBA[0] * LSubAC[n2] * dLSubBA[n1] +
                                                       product * dsubAC[0] * dLSubAC[n2] * LSubBA[n1]) *
                                                      dlambda[2][1] - dlambda[2][0] * (dProduct[1] * LSubAC[n2] * LSubBA[n1] +
                                                                                       product * dsubBA[1] * LSubAC[n2] * dLSubBA[n1] +
                                                                                       product * dsubAC[1] * dLSubAC[n2] * LSubBA[n1]));
                    faceIt++;
                }
...
void HierarchicalBasisHcurlTria::orientFace(int const &flag1, int const &flag2, int const &flag3, int const &faceNumber,
...
void HierarchicalBasisHcurlTria::getKeysInfo(std::vector<int> &functionTypeInfo, std::vector<int> &orderInfo) {
    int it = 0;
