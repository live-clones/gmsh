...
HierarchicalBasisH1Tetra::HierarchicalBasisH1Tetra(int order) {
    ...
    _numEdgeFunction = ((order >= 2) ? (6 * order - 6) : 0);
    _numQuadFaceFunction = (0);
    _numTriFaceFunction = ((order >= 3) ? (2 * (order - 2) * (order - 1)) : 0);
    _numBubbleFunction = ((order >= 4) ? ((order - 1) * (order - 2) * (order - 3) / 6) : 0);
    _pb = (order);
...
double HierarchicalBasisH1Tetra::_affineCoordinate(const int &j, const double &u, const double &v, const double &w) {
    switch(j) {
        case(1):
            return 0.5 * (1 + v);
        case(2):
...
void HierarchicalBasisH1Tetra::generateBasis(double const &u, double const &v, double const &w,
    ...
    std::vector<double> substraction(_numEdge);
    ...
    std::vector<std::vector<double> > phi(_numEdge);
    phi[0] = std::vector<double>(std::max(std::max(std::max(_pOrderEdge[0] - 1, _pOrderFace[0] - 2),
                                                   _pOrderFace[1] - 2),
                                          _pb - 3));
    phi[1] = std::vector<double>(std::max(_pOrderEdge[1] - 1, _pOrderFace[3] - 2));
    phi[2] = std::vector<double>(std::max(std::max(std::max(_pOrderEdge[2] - 1, _pOrderFace[0] - 2),
                                                   _pOrderFace[2] - 2),
                                          _pb - 3));
    phi[3] = std::vector<double>(std::max(std::max(std::max(_pOrderEdge[3] - 1, _pOrderFace[1] - 2),
                                                   _pOrderFace[2] - 2),
                                          _pb - 3));
    phi[4] = std::vector<double>(_pOrderEdge[4] - 1);
    phi[5] = std::vector<double>(std::max(_pOrderEdge[5] - 1, _pOrderFace[3] - 2));
    ...
    std::vector<double> edgeProduct(_numEdge);
    ...
    std::vector<double> faceProduct(_numTriFace);
    ...
    for(int iEdge = 0; iEdge < _numEdge; iEdge++) {
        for(int indexEdgeFunc = 0; indexEdgeFunc < _pOrderEdge[iEdge] - 1; indexEdgeFunc++) {
            if(iEdge == 2 && indexEdgeFunc % 2 != 0) {
                edgeBasis[indexEdgeBasis] = (-1) * edgeProduct[iEdge] * phi[iEdge][indexEdgeFunc];
            }
            else {
                edgeBasis[indexEdgeBasis] = edgeProduct[iEdge] * phi[iEdge][indexEdgeFunc];
            }
            indexEdgeBasis++;
        }
    }
    ...
    for(int iFace = 0; iFace < _numTriFace; iFace++) {
        ...
        for(int n1 = 0; n1 < _pOrderFace[iFace] - 2; n1++) {
            for(int n2 = 0; n2 < _pOrderFace[iFace] - 2 - n1; n2++) {
                ...
                faceBasis[indexFaceFunction] = impact * faceProduct[iFace] * phi[indexVectorTarget1][n1]
    ...
    for(int n1 = 0; n1 < _pb - 3; n1++) {
        for(int n2 = 0; n2 < _pb - 3 - n1; n2++) {
            for(int n3 = 0; n3 < _pb - 3 - n2 - n1; n3++) {
                bubbleBasis[indexBubbleBasis] = phi[2][n1] * phi[0][n2] * phi[3][n3] * prod;
                indexBubbleBasis++;
            }
        }
    }
...
void HierarchicalBasisH1Tetra::generateGradientBasis(double const &u, double const &v, double const &w,
    ...
    dprod[0] = jacob * (lambda1 * dlambda2[0] * lambda3 * lambda4 + lambda1 * lambda2 * dlambda3[0] * lambda4);
    dprod[1] = jacob * (dlambda1[1] * lambda2 * lambda3 * lambda4 + lambda1 * dlambda2[1] * lambda3 * lambda4);
    dprod[2] = jacob * (lambda1 * dlambda2[2] * lambda3 * lambda4 + lambda1 * lambda2 * lambda3 * dlambda4[2]);
    ...
    substraction[4] = lambda4 - lambda1;
    substraction[5] = lambda4 - lambda3;
    std::vector<std::vector<double> > dsubstraction(_numEdge, std::vector<double>(3, 1)); // = dsubstraction*jacob
    dsubstraction[0][0] = 2;
    dsubstraction[1][0] = -1;
    dsubstraction[1][2] = 0;
    dsubstraction[2][1] = 2.;
    ...
    dsubstraction[5][1] = 0;
    std::vector<std::vector<double> > phi(_numEdge);
    std::vector<std::vector<double> > dphi(_numEdge);
    for(int i = 0; i < _numEdge; i++) {
        int vectorSize = 0;
        switch(i) {
            case(0):
                vectorSize = std::max(std::max(std::max(_pOrderEdge[0] - 1, _pOrderFace[0] - 2),
                                               _pOrderFace[1] - 2),
                                      _pb - 3);
                break;
            case(1):
                ...
                break;
            case(2):
                vectorSize = std::max(std::max(std::max(_pOrderEdge[2] - 1, _pOrderFace[0] - 2),
                                               _pOrderFace[2] - 2),
                                      _pb - 3);
                break;
            case(3):
                vectorSize = std::max(std::max(std::max(_pOrderEdge[3] - 1, _pOrderFace[1] - 2),
                                               _pOrderFace[2] - 2),
                                      _pb - 3);
                break;
            case(4):
                vectorSize = _pOrderEdge[4] - 1;
                break;
            case(5):
    ...
    edgeProduct[4] = lambda4 * lambda1;
    edgeProduct[5] = lambda4 * lambda3;
    std::vector<std::vector<double> > dEdgeProduct(_numEdge, std::vector<double>(3));
    for(int i = 0; i < 3; i++) {
        dEdgeProduct[0][i] = jacob * (dlambda3[i] * lambda2 + lambda3 * dlambda2[i]);
        dEdgeProduct[1][i] = jacob * (dlambda1[i] * lambda3 + lambda1 * dlambda3[i]);
        dEdgeProduct[2][i] = jacob * (dlambda2[i] * lambda1 + lambda2 * dlambda1[i]);
        dEdgeProduct[3][i] = jacob * (dlambda4[i] * lambda2 + lambda4 * dlambda2[i]);
        dEdgeProduct[4][i] = jacob * (dlambda4[i] * lambda1 + lambda4 * dlambda1[i]);
        dEdgeProduct[5][i] = jacob * (dlambda4[i] * lambda3 + lambda4 * dlambda3[i]);
    }
    std::vector<double> faceProduct(_numTriFace);
    faceProduct[0] = edgeProduct[0] * lambda1;
    faceProduct[1] = edgeProduct[0] * lambda4;
    faceProduct[2] = edgeProduct[2] * lambda4;
    faceProduct[3] = edgeProduct[1] * lambda4;
    std::vector<std::vector<double> > dfaceProduct(_numTriFace, std::vector<double>(3));
    for(int i = 0; i < 3; i++) {
        dfaceProduct[0][i] = jacob * (edgeProduct[0] * dlambda1[i]) + dEdgeProduct[0][i] * lambda1;
        dfaceProduct[1][i] = jacob * (edgeProduct[0] * dlambda4[i]) + dEdgeProduct[0][i] * lambda4;
        dfaceProduct[2][i] = jacob * (edgeProduct[2] * dlambda4[i]) + dEdgeProduct[2][i] * lambda4;
        dfaceProduct[3][i] = jacob * (edgeProduct[1] * dlambda4[i]) + dEdgeProduct[1][i] * lambda4;
    }
    // edge shape functions:
    int indexEdgeBasis = 0;
    for(int iEdge = 0; iEdge < _numEdge; iEdge++) {
        for(int indexEdgeFunc = 0; indexEdgeFunc < _pOrderEdge[iEdge] - 1; indexEdgeFunc++) {
            ...
            if(iEdge == 2) {
                impact1 = 1;
                if(indexEdgeFunc % 2 != 0) {
                    impact1 = -1;
                }
            }
            for(int i = 0; i < 3; i++) {
                gradientEdge[indexEdgeBasis][i] = impact1 * (dEdgeProduct[iEdge][i] * phi[iEdge][indexEdgeFunc] +
    ...
    for(int iFace = 0; iFace < _numTriFace; iFace++) {
        ...
        for(int n1 = 0; n1 < _pOrderFace[iFace] - 2; n1++) {
            for(int n2 = 0; n2 < _pOrderFace[iFace] - 2 - n1; n2++) {
                ...
                for(int i = 0; i < 3; i++) {
                    gradientFace[indexFaceFunction][i] = impact * ( dfaceProduct[iFace][i]
                                                                    * phi[indexVectorTarget1][n1]
                                                                    * phi[indexVectorTarget2][n2]
                                                                   + faceProduct[iFace]
                                                                    * dphi[indexVectorTarget1][n1] * dsubstraction[indexVectorTarget1][i]
                                                                    * phi[indexVectorTarget2][n2]
                                                                   + faceProduct[iFace]
                                                                    * phi[indexVectorTarget1][n1]
                                                                    * dsubstraction[indexVectorTarget2][i] * dphi[indexVectorTarget2][n2]
                                                                   );
                }
                indexFaceFunction++;
            }
    ...
    for(int n1 = 0; n1 < _pb - 3; n1++) {
        for(int n2 = 0; n2 < _pb - 3 - n1; n2++) {
            for(int n3 = 0; n3 < _pb - 3 - n2 - n1; n3++) {
                for(int i = 0; i < 3; i++) {
                    gradientBubble[indexBubbleBasis][i] = phi[2][n1] *  phi[0][n2] *  phi[3][n3] * dprod[i]
...
void HierarchicalBasisH1Tetra::orientEdgeFunctionsForNegativeFlag(std::vector<double> &edgeFunctions) {
    int constant1 = 0;
    int constant2 = 0;
...
void HierarchicalBasisH1Tetra::orientEdgeFunctionsForNegativeFlag(std::vector<std::vector<double> > &edgeFunctions) {
    int constant1 = 0;
    int constant2 = 0;
    for(int edgeNumber = 0; edgeNumber < _numEdge; edgeNumber++) {
        constant2 = 0;
...
void HierarchicalBasisH1Tetra::orientEdge(int const &flagOrientation, int const &edgeNumber,
...
void HierarchicalBasisH1Tetra::orientOneFace(double const &u, double const &v, double const &w,
                                             int const &flag1, int const &flag2, int const &flag3,
                                             int const &faceNumber, std::vector<std::vector<double> > &gradientFace,
                                             std::string typeFunction) {
    if(!(flag1 == 0 && flag2 == 1)) {
        // to map onto the reference domain of gmsh:
        double uc = 2 * u - 1;
...
void HierarchicalBasisH1Tetra::orientFace(int const &flag1, int const &flag2, int const &flag3, int const &faceNumber,
    ...
    int numFaceFunctions = int((_pOrderFace[faceNumber] - 1) * (_pOrderFace[faceNumber] - 2) / 2);
...
void HierarchicalBasisH1Tetra::getKeysInfo(std::vector<int> &functionTypeInfo, std::vector<int> &orderInfo) {
