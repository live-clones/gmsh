...
{
  MVertexPtrLessThan
    comp; /* struct MVertexPtrLessThan {
           bool operator()(const MVertex *v1 , const MVertex *v2) const { return
           v1->getNum() < v2->getNum(); }  }; */

  std::next_permutation(
    vertices.begin(), vertices.end(),
    comp); // Generates the next permutation based on the increasing order of
           // getNum() values.
           // Ex: 2 5 8 -> 2 8 5 -> 5 2 8 -> 5 8 2 -> 8 2 5 -> 8 5 2
  unsigned int numVertices = vertices.size();
  for(unsigned int i = 0; i < numVertices; ++i) {
    element->setVertex(
      i, vertices[i]); // Updates element->_Vertex[i] with vertices[i] generated
                       // by next_permutation.
  }
}
