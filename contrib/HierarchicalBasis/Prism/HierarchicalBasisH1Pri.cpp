...
double HierarchicalBasisH1Pri::_affineCoordinate(const int &j, const double &u, const double &v, const double &w) {
  switch(j) {
      case(1):
          return 0.5 * (1 + v);
      case(2):
      ...
      case(5):
          return 0.5 * (1 - w);
      default:
          throw std::runtime_error("j must be : 1<=j<=5");
    }
}

void HierarchicalBasisH1Pri::generateBasis(double const &u, double const &v,double const &w,
    ...
    std::vector<double> product(_numEdge);
    ...
    phi[0] = std::vector<double>(std::max(std::max(std::max(std::max(std::max(_pOrderEdge[0] - 1, _pOrderEdge[6] - 1),
                                                                     _pOrderQuadFace[0][0] - 1),
                                                            _pOrderTriFace[0] - 2),
                                                   _pOrderTriFace[1] - 2),
                                          _pb[0] - 2));
    phi[1] = std::vector<double>(std::max(std::max(_pOrderEdge[1] - 1, _pOrderEdge[7] - 1), _pOrderQuadFace[0][1] - 1));
    phi[2] = std::vector<double>(std::max(std::max(std::max(std::max(std::max(_pOrderEdge[2] - 1, _pOrderEdge[4] - 1),
                                                                     _pOrderEdge[5] - 1),
                                                            _pOrderQuadFace[0][0] - 1),
                                                   _pOrderQuadFace[1][1] - 1),
                                          _pOrderQuadFace[1][2] - 1));
    phi[3] = std::vector<double>(std::max(std::max(_pOrderEdge[8] - 1, _pOrderEdge[3] - 1), _pOrderQuadFace[0][2] - 1));
    phi[4] = std::vector<double>(std::max(std::max(std::max(_pOrderTriFace[1] - 2, _pb[0] - 2), _pOrderTriFace[0] - 2), 0));
    ...
    for(int iEdge = 0; iEdge < _numEdge; iEdge++) {
        ...
        for(int indexEdgeFunc = 0; indexEdgeFunc < _pOrderEdge[iEdge] - 1; indexEdgeFunc++) {
    ...
    for(int iFace = 0; iFace < _numQuadFace + _numTriFace; iFace++) {
        int indexPhi1 = 0;
        int indexPhi2 = 0;
        ...
        if(iFace < 3) {
            for(int n1 = 0; n1 < _pOrderQuadFace[0][iFace] - 1; n1++) {
                for(int n2 = 0; n2 < _pOrderQuadFace[1][iFace] - 1; n2++) {
                    faceBasis[indexFaceFunction] = lambdaProduct * phi[indexPhi1][n1] * phi[indexPhi2][n2];
                    indexFaceFunction++;
                }
            }
        }
    ...
    for(int n1 = 0; n1 < _pb[0] - 2; n1++) {
        for(int n2 = 0; n2 < _pb[0] - 2 - n1; n2++) {
            for(int n3 = 2; n3 <= _pb[1]; n3++) {
                bubbleBasis[indexBubbleBasis] = lambdaProductBubble * phi[0][n1] * phi[4][n2] * OrthogonalPoly::EvalLobatto(n3, w);
                indexBubbleBasis++;
            }
        }
    }
...
void HierarchicalBasisH1Pri::generateGradientBasis(double const &u, double const &v, double const &w,
    ...
    substraction[3] = lambda1 - lambda3;
    substraction[4] = lambda2 - lambda1;
    std::vector<std::vector<double> > dsubstraction(5, std::vector<double>(3, 0)); // = dsubstraction*jacob
    for(int i = 0; i < 3; i++) {
    ...
    phi[2] = std::vector<double>(std::max(std::max(std::max(std::max(std::max(_pOrderEdge[2] - 1, _pOrderEdge[4] - 1),
                                                                     _pOrderEdge[5] - 1),
                                                            _pOrderQuadFace[1][0] - 1),
                                                   _pOrderQuadFace[1][1] - 1),
                                          _pOrderQuadFace[1][2] - 1));
    phi[3] = std::vector<double>(std::max(std::max(_pOrderEdge[8] - 1, _pOrderEdge[3] - 1), _pOrderQuadFace[0][2] - 1));
    phi[4] = std::vector<double>(std::max(std::max(std::max(_pOrderTriFace[1] - 2, _pb[0] - 2), _pOrderTriFace[0] - 2), 0));
    dphi[0] = std::vector<double>(std::max(std::max(std::max(std::max(std::max(_pOrderEdge[0] - 1, _pOrderEdge[6] - 1),
                                                                      _pOrderQuadFace[0][0] - 1),
                                                             _pOrderTriFace[0] - 2),
                                                    _pOrderTriFace[1] - 2),
                                           _pb[0] - 2));
    dphi[1] = std::vector<double>(std::max(std::max(_pOrderEdge[1] - 1, _pOrderEdge[7] - 1), _pOrderQuadFace[0][1] - 1));
    dphi[2] = std::vector<double>(std::max(std::max(std::max(std::max(std::max(_pOrderEdge[2] - 1, _pOrderEdge[4] - 1),
                                                                      _pOrderEdge[5] - 1),
                                                             _pOrderQuadFace[1][0] - 1),
                                                    _pOrderQuadFace[1][1] - 1),
                                           _pOrderQuadFace[1][2] - 1));
    dphi[3] = std::vector<double>(std::max(std::max(_pOrderEdge[8] - 1, _pOrderEdge[3] - 1), _pOrderQuadFace[0][2] - 1));
    dphi[4] = std::vector<double>(std::max(std::max(std::max(_pOrderTriFace[1] - 2, _pb[0] - 2), _pOrderTriFace[0] - 2), 0));
    ...
    std::vector<double> productEdgeTerm(_numEdge);
    ...
    std::vector<std::vector<double> > dproductEdgeTerm(_numEdge, std::vector<double>(3));
    for(int i = 0; i < 3; i++) {
        dproductEdgeTerm[0][i] = gradientVertex[0][i] * lambda3 + dlambda3[i] * product1[0];
        dproductEdgeTerm[1][i] = gradientVertex[0][i] * lambda1 + dlambda1[i] * product1[0];
        dproductEdgeTerm[2][i] = gradientVertex[0][i] * lambda4 + dlambda4[i] * product1[0];
        dproductEdgeTerm[3][i] = gradientVertex[1][i] * lambda1 + dlambda1[i] * product1[1];
        dproductEdgeTerm[4][i] = gradientVertex[1][i] * lambda4 + dlambda4[i] * product1[1];
        dproductEdgeTerm[5][i] = gradientVertex[2][i] * lambda4 + dlambda4[i] * product1[2];
        dproductEdgeTerm[6][i] = gradientVertex[3][i] * lambda3 + dlambda3[i] * product1[3];
        dproductEdgeTerm[7][i] = gradientVertex[3][i] * lambda1 + dlambda1[i] * product1[3];
        dproductEdgeTerm[8][i] = gradientVertex[4][i] * lambda1 + dlambda1[i] * product1[4];
    }
    ...
    for(int iEdge = 0; iEdge < _numEdge; iEdge++) {
        ...
        for(int indexEdgeFunc = 0; indexEdgeFunc < _pOrderEdge[iEdge] - 1; indexEdgeFunc++) {
            for(int i = 0; i < 3; i++) {
                gradientEdge[indexEdgeBasis][i] = dproductEdgeTerm[iEdge][i] * phi[numPhi][indexEdgeFunc]
                + dsubstraction[numPhi][i] * productEdgeTerm[iEdge] * dphi[numPhi][indexEdgeFunc];
            }
            indexEdgeBasis++;
        }
    }
    ...
    for(int iFace = 0; iFace < _numQuadFace + _numTriFace; iFace++) {
        ...
        switch(iFace) {
            case(3):
                ...
                for(int i = 0; i < 3; i++) {
                    dproductFaceTerm[i] = dproductEdgeTerm[0][i] * lambda1 + productEdgeTerm[0] * dlambda1[i];
                }
                break;
            case(4):
                indexPhi1 = 0;
                ...
                for(int i = 0; i < 3; i++) {
                    dproductFaceTerm[i] = dproductEdgeTerm[7][i] * lambda3 + productEdgeTerm[7] * dlambda3[i];
                }
                break;
            case(0):
                indexPhi1 = 0;
                ...
                for(int i = 0; i < 3; i++) {
                    dproductFaceTerm[i] = dproductEdgeTerm[0][i] * lambda4 + productEdgeTerm[0] * dlambda4[i];
                }
                break;
            case(1):
                indexPhi1 = 1;
                ...
                for(int i = 0; i < 3; i++) {
                    dproductFaceTerm[i] = dproductEdgeTerm[7][i] * lambda5 + productEdgeTerm[7] * dlambda5[i];
                }
                break;
            case(2):
                indexPhi1 = 3;
                ...
                for(int i = 0; i < 3; i++) {
                    dproductFaceTerm[i] = dproductEdgeTerm[8][i] * lambda5 + productEdgeTerm[8] * dlambda5[i];
                }
                break;
        }
        if(iFace < 3) {
            for(int n1 = 0; n1 < _pOrderQuadFace[0][iFace] - 1; n1++) {
                for(int n2 = 0; n2 < _pOrderQuadFace[1][iFace] - 1; n2++) {
                    for(int i = 0; i < 3; i++) {
                        gradientFace[indexFaceFunction][i] = dproductFaceTerm[i] * phi[indexPhi1][n1] * phi[indexPhi2][n2]
                        + productFaceTerm * dsubstraction[indexPhi1][i] * dphi[indexPhi1][n1] * phi[indexPhi2][n2]
                        + productFaceTerm * dsubstraction[indexPhi2][i] * phi[indexPhi1][n1] * dphi[indexPhi2][n2];
                    }
                    indexFaceFunction++;
                }
            }
    ...
    for(int n1 = 0; n1 < _pb[0] - 2; n1++) {
        for(int n2 = 0; n2 < _pb[0] - 2 - n1; n2++) {
            for(int n3 = 2; n3 <= _pb[1]; n3++) {
                gradientBubble[indexBubbleBasis][0] = (dlambdaProductBubble[0] * phi[0][n1] * phi[4][n2]
                                                       + lambdaProductBubble * dsubstraction[0][0] * dphi[0][n1] * phi[4][n2]
                                                       + lambdaProductBubble * dsubstraction[4][0] * phi[0][n1] * dphi[4][n2] )
                                                    * OrthogonalPoly::EvalLobatto(n3, w);
                gradientBubble[indexBubbleBasis][1] = (dlambdaProductBubble[1] * phi[0][n1] * phi[4][n2]
                                                       + lambdaProductBubble * dsubstraction[0][1] * dphi[0][n1] * phi[4][n2]
                                                       + lambdaProductBubble * dsubstraction[4][1] * phi[0][n1] * dphi[4][n2] )
                                                    * OrthogonalPoly::EvalLobatto(n3, w);
                gradientBubble[indexBubbleBasis][2] = (dlambdaProductBubble[2] * phi[0][n1] * phi[4][n2]
                                                       + lambdaProductBubble * dsubstraction[0][2] * dphi[0][n1] * phi[4][n2]
                                                       + lambdaProductBubble * dsubstraction[4][2] * phi[0][n1] * dphi[4][n2] )
                                                    * OrthogonalPoly::EvalLobatto(n3, w)
...
void HierarchicalBasisH1Pri::orientEdge(int const &flagOrientation, int const &edgeNumber,
    ...
    if(faceNumber < 3) {
        if(!(flag1 == 1 && flag2 == 1 && flag3 == 1)) {
            ...
            for(int i = 0; i < faceNumber; i++) {
                iterator += (_pOrderQuadFace[0][i] - 1) * (_pOrderQuadFace[1][i] - 1);
            }
            if(flag3 == 1) {
                for(int it1 = 2; it1 <= _pOrderQuadFace[0][faceNumber]; it1++) {
                    for(int it2 = 2; it2 <= _pOrderQuadFace[1][faceNumber]; it2++) {
                        int impactFlag1 = 1;
                        int impactFlag2 = 1;
                        if(flag1 == -1 && it1 % 2 != 0) {
                            impactFlag1 = -1;
                ...
                std::vector<double> phi1(_pOrderQuadFace[0][faceNumber] - 1);
                std::vector<double> phi2(_pOrderQuadFace[1][faceNumber] - 1);
                for(int it = 0; it <= _pOrderQuadFace[0][faceNumber] - 2; it++) {
                    phi1[it] = OrthogonalPoly::EvalKernelFunction(it, var1);
                }
                for(int it = 0; it <= _pOrderQuadFace[1][faceNumber] - 2; it++) {
                    phi2[it] = OrthogonalPoly::EvalKernelFunction(it, var2);
                }
                for(int it1 = 0; it1 <= _pOrderQuadFace[1][faceNumber] - 2; it1++) {
                    for(int it2 = 0; it2 <= _pOrderQuadFace[0][faceNumber] - 2; it2++) {
                        int impactFlag1 = 1;
                        int impactFlag2 = 1;
                        if(flag2 == -1 && it1 % 2 != 0) {
                            impactFlag1 = -1;
                        ...
                        faceBasis[iterator] = lambdaProduct * phi1[it2] * phi2[it1] * impactFlag1 * impactFlag2;
                        iterator++;
                    }
                }
            }
        ...
        if(!(flag1 == 0 && flag2 == 1)) {
            ...
            double wc = w;
            //*****
            int iterator = (_pOrderQuadFace[1][0] - 1) * (_pOrderQuadFace[0][0] - 1)
                         + (_pOrderQuadFace[1][1] - 1) * (_pOrderQuadFace[0][1] - 1)
                         + (_pOrderQuadFace[1][2] - 1) * (_pOrderQuadFace[0][2] - 1);
...
void HierarchicalBasisH1Pri::orientOneFace(double const &u, double const &v, double const &w, int const &flag1,
    ...
    if(faceNumber < 3) {
            ...
            if(flag3 == 1) {
                for(int it1 = 2; it1 <= _pOrderQuadFace[0][faceNumber]; it1++) {
                    for(int it2 = 2; it2 <= _pOrderQuadFace[1][faceNumber]; it2++) {
                        ...
                        gradientFace[iterator][0] = gradientFace[iterator][0] * impactFlag1 * impactFlag2;
                        gradientFace[iterator][1] = gradientFace[iterator][1] * impactFlag1 * impactFlag2;
                        gradientFace[iterator][2] = gradientFace[iterator][2] * impactFlag1 * impactFlag2;
                        iterator++;
                    }
                }
            }
                ...
                std::vector<double> phi1(_pOrderQuadFace[0][faceNumber] - 1);
                std::vector<double> phi2(_pOrderQuadFace[1][faceNumber] - 1);
                std::vector<double> dphi1(_pOrderQuadFace[0][faceNumber] - 1);
                std::vector<double> dphi2(_pOrderQuadFace[1][faceNumber] - 1);
                for(int it = 0; it <= _pOrderQuadFace[0][faceNumber] - 2; it++) {
                    phi1[it] = OrthogonalPoly::EvalKernelFunction(it, var1);
                ...
                for(int it1 = 0; it1 <= _pOrderQuadFace[1][faceNumber] - 2; it1++) {
                    for(int it2 = 0; it2 <= _pOrderQuadFace[0][faceNumber] - 2; it2++) {
                        ...
                        for(int i = 0; i < 3; i++) {
                            gradientFace[iterator][i] = impactFlag1 * impactFlag2 * (dlambdaProduct[i] * phi1[it2] * phi2[it1]
        ...
        if(!(flag1 == 0 && flag2 == 1)) {
            ...
            for(int n1 = 0; n1 < _pOrderTriFace[constant] - 2; n1++) {
                ...
                for(int n2 = 0; n2 < _pOrderTriFace[constant] - 2 - n1; n2++) {
                    for(int i = 0; i < 3; i++) {
                        gradientFace[iterator][i] = dproduct[i] * phiSubs1 * phiSubs2[n2]
...
void HierarchicalBasisH1Pri::orientFace(int const &flag1, int const &flag2, int const &flag3, int const &faceNumber,
    ...
    if(faceNumber < 3) {
        ...
        int numFaceFunctions = (_pOrderQuadFace[0][faceNumber] - 1) * (_pOrderQuadFace[1][faceNumber] - 1);
        ...
        int iterator = _numQuadFaceFunction;
        int numface = faceNumber - 3;
        for(int i = 0; i < numface; i++) {
            iterator += int((_pOrderTriFace[i] - 1) * (_pOrderTriFace[i] - 2) / 2);
        }
        int numFaceFunctions = int((_pOrderTriFace[numface] - 1) * (_pOrderTriFace[numface] - 2) / 2);
        int iOrientation = getOrientationTriFaceIndex(flag1, flag2);
        int offset = iOrientation * _numTriFaceFunction - _numQuadFaceFunction;
        int offset2 = iterator + numFaceFunctions;
        for(int i = iterator; i < offset2; i++) {
...
void HierarchicalBasisH1Pri::getKeysInfo(std::vector<int> &functionTypeInfo, std::vector<int> &orderInfo) {
    for(int i = 0; i < 6; i++) {
    ...
    for(int iFace = 0; iFace < _numQuadFace + _numTriFace; iFace++) {
        if(iFace < 3) {
            for(int n1 = 2; n1 <= _pOrderQuadFace[0][iFace]; n1++) {
                for(int n2 = 2; n2 <= _pOrderQuadFace[1][iFace]; n2++) {
    ...
    for(int n1 = 1; n1 < _pb[0] - 1; n1++) {
        for(int n2 = 1; n2 <= _pb[0] - 1 - n1; n2++) {
            for(int n3 = 2; n3 <= _pb[1]; n3++) {
