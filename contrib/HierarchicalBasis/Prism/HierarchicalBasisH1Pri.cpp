                                           ...
                                           std::string typeFunction)
{
  ...
  phi[0] = std::vector<double>(std::max(
    std::max(std::max(std::max(std::max(_pOrderEdge[0] - 1, _pOrderEdge[6] - 1),
             ...
             _pOrderTriFace[1] - 2),
    _pb[0] - 2));
  phi[1] = std::vector<double>(
    std::max(std::max(_pOrderEdge[1] - 1, _pOrderEdge[7] - 1),
  ...
  phi[3] = std::vector<double>(
    std::max(std::max(_pOrderEdge[8] - 1, _pOrderEdge[3] - 1),
             _pOrderQuadFace[0][2] - 1));
  phi[4] = std::vector<double>(
    std::max(std::max(std::max(_pOrderTriFace[1] - 2, _pb[0] - 2),
                      _pOrderTriFace[0] - 2),
             0));
  ...
  std::vector<std::vector<double>> &gradientBubble)
{
  ...
  dphi[2] = std::vector<double>(std::max(
    std::max(std::max(std::max(std::max(_pOrderEdge[2] - 1, _pOrderEdge[4] - 1),
             ...
             _pOrderQuadFace[1][1] - 1),
    _pOrderQuadFace[1][2] - 1));
  dphi[3] = std::vector<double>(
    std::max(std::max(_pOrderEdge[8] - 1, _pOrderEdge[3] - 1),
             _pOrderQuadFace[0][2] - 1));
  dphi[4] = std::vector<double>(
    std::max(std::max(std::max(_pOrderTriFace[1] - 2, _pb[0] - 2),
                      _pOrderTriFace[0] - 2),
             0));
  ...
  std::vector<std::vector<double>> dproductEdgeTerm(_numEdge,
                                                    std::vector<double>(3));
  for(int i = 0; i < 3; i++) {
  ...
  std::vector<double> &fTableCopy)
{
  if(faceNumber < 3) {
    ...
    int numFaceFunctions = (_pOrderQuadFace[0][faceNumber] - 1) *
                           (_pOrderQuadFace[1][faceNumber] - 1);
