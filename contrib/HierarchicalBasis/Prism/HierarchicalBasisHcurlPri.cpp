  ...
  std::vector<std::vector<double> > &bubbleBasis)
{
  ...
  std::vector<double> lobattoW(
    std::max(
      std::max(std::max(_pOrderQuadFace[1][0] + 2, _pOrderQuadFace[1][1] + 2),
               _pOrderQuadFace[1][2] + 2),
      _pb[1] + 2),
    0);
  ...
  for(int iFace = 0; iFace < _numQuadFace; iFace++) {
    ...
    if(_pOrderQuadFace[0][iFace] > 0) {
      for(int n1 = 0; n1 <= _pOrderQuadFace[0][iFace]; n1++) {
        ...
        for(int n2 = 2; n2 <= _pOrderQuadFace[1][iFace] + 1; n2++) {
    ...
    for(int n1 = 2; n1 <= _pOrderQuadFace[0][iFace] + 1; n1++) {
      double phie =
        prod * OrthogonalPoly::EvalKernelFunction(n1 - 2, sub[index]);

      for(int n2 = 0; n2 <= _pOrderQuadFace[1][iFace]; n2++) {
  ...
  for(int iFace = 0; iFace < _numQuadFace; iFace++) {
    ...
    for(int n1 = 2; n1 <= _pb[0]; n1++) {
      for(int n2 = 2; n2 <= _pb[1] + 1; n2++) {
  ...
  for(int n1 = 0; n1 < _pb[0] - 1; n1++) {
    for(int n2 = 0; n2 < _pb[0] - 1 - n1; n2++) {
      double term = product * legendreVector[0][n1] * legendreVector[3][n2];
      for(int n3 = 0; n3 <= _pb[1]; n3++) {
  ...
  std::vector<std::vector<double> > &faceFunctions, std::string typeFunction)
{
  if(faceNumber < 3) {
    if(!(flag1 == 1 && flag2 == 1 && flag3 == 1)) {
      ...
      if(flag3 == 1) {
        for(int it1 = 0; it1 <= _pOrderQuadFace[0][faceNumber]; it1++) {
          for(int it2 = 2; it2 <= _pOrderQuadFace[1][faceNumber] + 1; it2++) {
            int impactFlag1 = 1;
            int impactFlag2 = 1;
        ...
        for(int it1 = 2; it1 <= _pOrderQuadFace[0][faceNumber] + 1; it1++) {
          for(int it2 = 0; it2 <= _pOrderQuadFace[1][faceNumber]; it2++) {
            int impactFlag1 = 1;
            int impactFlag2 = 1;
            if(flag1 == -1 && it1 % 2 != 0) { impactFlag1 = -1; }
            if(flag2 == -1 && it2 % 2 == 0) { impactFlag2 = -1; }
        ...
        if(typeFunction == "HcurlLegendre") {
          ...
          std::vector<double> LSub(_pOrderQuadFace[0][faceNumber]);
          std::vector<double> phi(_pOrderQuadFace[0][faceNumber]);
          ...
          for(int it1 = 0; it1 <= _pOrderQuadFace[1][faceNumber]; it1++) {
            double Lw = OrthogonalPoly::EvalLegendre(it1, wc);
            int impactFlag1 = 1;
            if(flag2 == -1 && it1 % 2 == 0) { impactFlag1 = -1; }
            for(int it2 = 2; it2 <= _pOrderQuadFace[0][faceNumber] + 1; it2++) {
              int impactFlag2 = 1;
          ...
          for(int it1 = 2; it1 <= _pOrderQuadFace[1][faceNumber] + 1; it1++) {
            ...
            for(int it2 = 2; it2 <= _pOrderQuadFace[0][faceNumber]; it2++) {
              int impactFlag2 = 1;
          ...
          std::vector<double> Lsub(_pOrderQuadFace[0][faceNumber]);
          std::vector<double> dLsub(_pOrderQuadFace[0][faceNumber]);
          std::vector<double> phi(_pOrderQuadFace[0][faceNumber]);
          std::vector<double> dphi(_pOrderQuadFace[0][faceNumber]);
          for(unsigned int i = 0; i < Lsub.size(); i++) {
            Lsub[i] = OrthogonalPoly::EvalLegendre(i, sub);
  ...
  std::vector<std::vector<double> > &fTableCopy)
{
  if(faceNumber < 3) {
    ...
    for(int i = 0; i < faceNumber; i++) {
      iterator += (_pOrderQuadFace[0][i] + 1) * _pOrderQuadFace[1][i] +
                  (_pOrderQuadFace[1][i] + 1) * _pOrderQuadFace[0][i];
    }
    int numFaceFunctions =
      (_pOrderQuadFace[0][faceNumber] + 1) * _pOrderQuadFace[1][faceNumber] +
      (_pOrderQuadFace[1][faceNumber] + 1) * _pOrderQuadFace[0][faceNumber];
