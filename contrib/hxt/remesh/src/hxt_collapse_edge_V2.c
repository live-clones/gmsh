...
//***************************************************************************************
//***************************************************************************************
HXTStatus hxtTempPrintCavity_V2 (HXTMesh *mesh,
                              HXTEdges* edges,
                              uint64_t cavSize,
                              uint64_t *cavity,
                              uint64_t *neighs,
...
//*************************************************************************************************
//*************************************************************************************************
int isEdgeOrientedInTriangle_V2  (HXTMesh *mesh, 
                              HXTEdges *edges, 
                              uint64_t ct, 
                              uint32_t ce)
{
...
//***************************************************************************************
//***************************************************************************************
HXTStatus hxtPointGenSort_V2 (HXTEdges *edges, uint32_t num, uint32_t *list, uint32_t *sortedList)
{
  struct str{
    double value;
    uint32_t index;
...
//***************************************************************************************
//***************************************************************************************
static inline uint64_t cavMod_V2 (uint64_t a, uint64_t b) {
...
// FUNCTION 
//
// same as hxtCanWeCollapseBoundaryCavity 
// but for a cavity with just one triangle
//
//***************************************************************************************
//***************************************************************************************
HXTStatus hxtCanWeCollapseBoundaryCavity_Triangle  (HXTMesh *mesh, 
                                          HXTEdges *edges, 
                                          ...
                                          int *canWeCollapse)
{
  ...
  if(cavSize!=1)
    return HXT_ERROR_MSG(HXT_STATUS_ERROR,"This should be a cavity of one triangle");
  if(perimSize!=3)
    return HXT_ERROR_MSG(HXT_STATUS_ERROR,"This should be a cavity of one triangle");


  // Find remaining triangle in position of currentTri
  uint64_t rt = cavity[0];
  for (uint32_t i=0; i<perimSize; i++){
    uint32_t ce = perim[i];
    if (edges->node[2*ce+0] == vd || edges->node[2*ce+1] == vd) continue; 

    rt = edges->edg2tri[2*ce+0] == currentTri ? edges->edg2tri[2*ce+1] : edges->edg2tri[2*ce+0];
  }

  if (rt == currentTri)
    return HXT_ERROR_MSG(HXT_STATUS_ERROR,"Did not find adjacent triangle");

  if (mesh->triangles.color[rt] != mesh->triangles.color[currentTri])
    return HXT_ERROR_MSG(HXT_STATUS_ERROR,"Adjacent triangle of different color");
    ...
    for (uint32_t k=0; k<count; k++){
      ...
      if (edges2lines[ce] == UINT64_MAX){
        ...
        for (uint64_t j=0; j<maxNumTriToLine; j++){
          ...
          // Non-manifold case
          int is0 = isEdgeOrientedInTriangle_V2(mesh,edges,cavity[i],ce);
          int is1 = isEdgeOrientedInTriangle_V2(mesh,edges,ct,ce);
          if (is0 == -1 ){
            printf("%d %d \n", count, ce);
...
//***************************************************************************************
//***************************************************************************************
HXTStatus hxtCanWeCollapseBoundaryCavity_V2  (HXTMesh *mesh, 
                                          HXTEdges *edges, 
                                          HXTPointGenParent *parent,
                                          uint32_t *lines2edges,
                                          uint64_t *edges2lines,
...
//***************************************************************************************
//***************************************************************************************
HXTStatus hxtCanWeCollapseBoundaryEdge_V2  (HXTMesh *mesh, 
                                        HXTEdges *edges, 
                                        HXTPointGenParent *parent,
                                        uint32_t *lines2edges,
                                        uint64_t *edges2lines,
                                        ...
                                        int *canWeCollapse)
{
  ...
  for (uint64_t k=0; k<maxNumTriToLine; k++){
    ...
    if (cavSize == 1){
      ...
      HXT_CHECK(hxtCanWeCollapseBoundaryCavity_Triangle(mesh, 
                                               edges, 
                                               parent, 
                                               lines2edges,
                                               edges2lines,
      ...
      HXT_CHECK(hxtCanWeCollapseBoundaryCavity_V2(mesh, 
                                               edges, 
                                               parent, 
                                               lines2edges,
                                               edges2lines,
    ...
    //printf("collapse for %lu with size %lu is %d \n", k, cavSize, collapse);
    if (collapse == 0){
      *canWeCollapse = 0;
      return HXT_STATUS_OK;
    }
...
//***************************************************************************************
//***************************************************************************************
HXTStatus hxtCanWeCollapseBoundaryVertex_V2 (HXTMesh *mesh,
                                         HXTEdges *edges,
                                         HXTPointGenParent *parent,
                                         uint32_t *lines2edges,
                                         uint64_t *edges2lines,
                                         ...
                                         uint32_t *edgeToCollapse ) // return edge to be collapsed
{
  ...
  for (uint64_t i=0; i<maxNumLinesToVertex; i++){
    ...
    int canWeCollapse = 0;
    HXT_CHECK(hxtCanWeCollapseBoundaryEdge_V2 (mesh, 
                                            edges, 
                                            parent,
                                            lines2edges,
                                            edges2lines,
...
//***************************************************************************************
//***************************************************************************************
HXTStatus hxtCanWeCollapseInteriorEdge_V2 (HXTMesh *mesh, 
                                       HXTEdges *edges, 
                                       HXTPointGenParent *parent,
                                       uint32_t *lines2edges,
                                       uint64_t *edges2lines,
                                       ...
                                       int *canWeCollapse)
{
  ...
  for (uint64_t i=0; i<cavSize; i++){
    uint64_t ct = cavity[i];
    if (ct == t0){
      uint64_t ind0 = cavMod_V2(i-1,cavSize);
      uint64_t ind1 = (i+1)%cavSize;
      if      (t1 == cavity[ind0]){
        tt0 = cavity[(i+1)%cavSize];
        tt1 = cavity[cavMod_V2(ind0-1,cavSize)];
      }
      else if (t1 == cavity[ind1]){
        tt0 = cavity[cavMod_V2(i-1,cavSize)];
...
//***************************************************************************************
//***************************************************************************************
HXTStatus hxtCanWeCollapseInteriorVertex_V2 (HXTMesh *mesh,
                                         HXTEdges *edges,
                                         HXTPointGenParent *parent,
                                         uint32_t *lines2edges,
                                         uint64_t *edges2lines,
                                         ...
                                         uint32_t *edgeToCollapse ) // return edge to be collapsed
{
  ...
  for (uint32_t k=0; k<cavSizeEdges; k++){
    ...
    // TODO for now tests 
    HXT_CHECK(hxtCanWeCollapseInteriorEdge_V2(mesh, 
                                           edges, 
                                           parent, 
                                           lines2edges,
                                           edges2lines,
...
// FUNCTION 
//
// replace boundary cavity of ONE triangle when collapsing boundary edge
//
//***************************************************************************************
//***************************************************************************************
HXTStatus hxtCollapseBoundaryTriangle_V2  (HXTMesh *mesh,
                                           HXTEdges *edges,
                                           HXTPointGenParent *parent,
                                           uint32_t *flagE,
                                           uint32_t *lines2edges,
                                           ...
                                           uint32_t vd,  // node to be removed
                                           uint32_t ce,  // edge to be collapsed
                                           uint64_t ct,  // triangle to be deleted
                                           int *collapsed)  
{
  ...
  *collapsed = 0;
  ...
  // Here this would mean that triangle dt is essentially a whole surface,
  // probably impossible to happen by construction
  ...
  // TODO the only remaining edge is oe 
  ...
  if (re != oe)
    return HXT_ERROR_MSG(HXT_STATUS_ERROR,"That's impossible to happen");
...
// FUNCTION 
//
// replace boundary cavity when collapsing boundary edge
//
//***************************************************************************************
//***************************************************************************************
HXTStatus hxtCollapseBoundaryCavity_V2  (HXTMesh *mesh,
                                         HXTEdges *edges,
                                         HXTPointGenParent *parent,
                                         uint32_t *flagE,
                                         uint32_t *lines2edges,
...
//***************************************************************************************
//***************************************************************************************
HXTStatus hxtCollapseBoundaryEdge_V2  (HXTMesh *mesh,
                                   HXTEdges *edges,
                                   HXTPointGenParent *parent,
                                   uint32_t *flagE,
                                   uint32_t *lines2edges,
  ...
  *collapsed = 0;
  ...
  for (uint64_t i=0; i<maxNumTriToLine; i++){
    ...
    if (cavSize == 1){
      HXT_CHECK(hxtCollapseBoundaryTriangle_V2(mesh,
                                             edges,
                                             parent,
                                             flagE,
                                             lines2edges,
      ...
      HXT_CHECK(hxtCollapseBoundaryCavity_V2(mesh,
                                             edges,
                                             parent,
                                             flagE,
                                             lines2edges,
...
//***************************************************************************************
//***************************************************************************************
HXTStatus hxtCollapseInteriorEdge_V2  (HXTMesh *mesh, 
                                   HXTEdges *edges, 
                                   HXTPointGenParent *parent,
                                   uint32_t *flagE,
                                   uint32_t *lines2edges,
                                  ...
                                  int *collapsed ) 
{
  ...
  HXT_CHECK(hxtCanWeCollapseBoundaryVertex_V2(mesh,
                                           edges,
                                           parent,
                                           lines2edges,
                                           edges2lines,
  ...
  //printf("can be collapsed = %d \n", edgeToCollapse);

  if (edgeToCollapse == UINT32_MAX) return HXT_STATUS_OK;

  HXT_CHECK(hxtCollapseBoundaryEdge_V2(mesh,
                                    edges,
                                    parent,
                                    flagE,
                                    lines2edges,
                                  ...
                                  int *collapsed )
{
  ...
  uint32_t edgeToCollapse = UINT32_MAX;
  HXT_CHECK(hxtCanWeCollapseInteriorVertex_V2(mesh,
                                           edges,
                                           parent,
                                           lines2edges,
                                           edges2lines,
  ...
  HXT_CHECK(hxtCollapseInteriorEdge_V2(mesh,
                                    edges,
                                    parent,
                                    flagE,
                                    lines2edges,
