...
HXTStatus hxtSurfaceQuadSingularitiesOutput(HXTMesh *mesh)
{
  ...
  for (uint32_t i=0; i<mesh->vertices.num; i++){
    ...
    if (counter[i] == 1 && isBoundary[i] != 1) printf("BIG PROBLEM - did not expect this\n");
  }
  ...
  HXT_INFO("     valence 3      : %d", c3);
  HXT_INFO("     valence 5      : %d", c5);
  HXT_INFO("     valence 6+     : %d", c6);
  HXT_INFO("");
  HXT_INFO("     valence 1  bnd : %d", c1b);
  HXT_INFO("     valence 3  bnd : %d", c3b);
  HXT_INFO("     valence 5  bnd : %d", c5b);
  HXT_INFO("     valence 6+ bnd : %d", c6b);
  HXT_INFO("");
  HXT_INFO("--- Feature points      = %d ", mesh->points.num);
  HXT_INFO("--- Singular bnd points = %d ", c1b+c3b+c5b+c6b);
  ...
  /*// ========================================================*/
  /*// Elements with multiple singularities*/
  /*// ========================================================*/
  /*uint64_t *elem;*/
  /*HXT_CHECK(hxtMalloc(&elem,mesh->quads.num*sizeof(uint64_t)));*/
  /*for (uint32_t i=0; i<mesh->quads.num; i++) elem[i] = 0;*/

  /*for (uint64_t i=0; i<mesh->quads.num; i++){*/
    /*uint32_t *v = mesh->quads.node + 4*i;*/
    /*elem[i] = 0;*/
    /*for (uint32_t j=0; j<4; j++){*/
      /*if (counter[v[j]]==3 || counter[v[j]] == 5){*/
        /*elem[i]++;*/
      /*}*/
    /*}*/
  /*}*/

  /*hxtPosNewView(test,"elem0");*/
  /*for (uint64_t i=0; i<mesh->quads.num; i++){*/
    /*uint32_t *v = mesh->quads.node + 4*i;*/
    /*if (elem[i] == 0){*/
      /*hxtPosAddQuad(test,&mesh->vertices.coord[4*v[0]],*/
                         /*&mesh->vertices.coord[4*v[1]],*/
                         /*&mesh->vertices.coord[4*v[2]],*/
                         /*&mesh->vertices.coord[4*v[3]],0);*/
    /*}*/
  /*}*/
  /*hxtPosNewView(test,"elem1");*/
  /*for (uint64_t i=0; i<mesh->quads.num; i++){*/
    /*uint32_t *v = mesh->quads.node + 4*i;*/
    /*if (elem[i] == 1){*/
      /*hxtPosAddQuad(test,&mesh->vertices.coord[4*v[0]],*/
