...
namespace geodesic {
  ...
  public:
    ...
    double &stretch() { return m_stretch; };
    ...
    void local_coordinates(Point3D *point, double &x, double &y)
    {
      ...
      // x = length/2.0 + (d0*d0 - d1*d1)/(2.0*length);
      y = sqrt(std::max(0.0, d0 * d0 - x * x));
      // x = length/2.0 + (d0 - d1)*(d0 + d1) / (2.0 * length);
      // y = sqrt(std::max(0.0, (d0 - x)*(d0 + x)));
      // if (false) {
      // 	double xb = length/2.0 + (d0 - d1)*(d0 + d1) / (2.0 * length);
      // 	// double xb = length/2.0 + (d0*d0 - d1*d1) / (2.0 * length);
      // 	double yb = sqrt(std::max(0.0, (d0 - xb)*(d0 + xb)));
      // 	// std::cout << x << " " << y << " " << xb << " " << yb <<
      // std::endl;
      // 	// std::cout << fabs(x - xb) << " " << fabs(y - yb) << std::endl;
      // 	assert(fabs(x - xb) < 1e-8 && fabs(y - yb) < 1e-8);
      // 	assert(!isnan(xb) && !isnan(yb));
      // 	x = xb;
      // 	y = yb;
      // }
      return;
  ...
  class SurfacePoint
    : public Point3D // point on the surface of the mesh
  {
  public:
  ...
  inline edge_pointer Face::next_edge(edge_pointer e, vertex_pointer v)
  {
    ...
    for(unsigned i = 0; i < 3; ++i) {
      ...
      if(e->id() != next->id() && next->belongs(v)) { return next; }
    }
    assert(0);
    return NULL;
  }
  ...
  {
    if(x.vertex_0 == y.vertex_0) { return x.vertex_1 < y.vertex_1; }
    else {
