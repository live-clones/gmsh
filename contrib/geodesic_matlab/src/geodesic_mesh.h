...
namespace geodesic {
...
#if METRIC
    double minStretch = 1;
    void setEdgeStretch(std::vector<double> &stretch);
#endif
  ...
  inline void Mesh::build_adjacencies()
  {
    ...
    for(unsigned i = 1; i < half_edges.size(); ++i) {
      if(half_edges[i] != half_edges[i - 1]) { ++number_of_edges; }
      else {
        if(i <
           half_edges.size() -
    ...
    for(unsigned i = 0; i < m_faces.size(); ++i) {
      ...
      for(unsigned j = 0; j < 3; ++j) // compute angle adjacent to the vertex j
      {
        ...
        // assert(angle>1e-5);						//algorithm works well with
        // non-degenerate meshes only if (angle < 1e-14) { 	std::cout << "The
        // triangle " << f.id() << " is degenerate! (edge lengths: "
        // 			  << abc[0] << " " << abc[1] << " " << abc[2] << ")" <<
        // std::endl; 	throw std::runtime_error("Degenerate triangle!");
        // }
...
#if METRIC
  inline void Mesh::setEdgeStretch(std::vector<double> &stretch)
  {
    assert(stretch.size() == m_edges.size());
    minStretch = std::numeric_limits<double>::max();
    for(unsigned i = 0; i < m_edges.size(); ++i) {
      Edge &e = m_edges[i];
      e.stretch() = stretch[i];
      if(e.stretch() < minStretch) minStretch = e.stretch();
    }

    // Reset the saddle_or_boundary flag for all vertices and edges
    std::vector<double> total_vertex_angle(m_vertices.size(), 0);
    for(unsigned i = 0; i < m_faces.size(); ++i) {
      Face &f = m_faces[i];
      Edge *es[3] = {f.adjacent_edges()[0], f.adjacent_edges()[1],
                     f.adjacent_edges()[2]};
      double ls[3] = {es[0]->length() * es[0]->stretch(),
                      es[1]->length() * es[1]->stretch(),
                      es[2]->length() * es[2]->stretch()};
      for(unsigned j = 0; j < 3; ++j) {
        vertex_pointer v = f.opposite_vertex(es[j]);
        total_vertex_angle[v->id()] +=
          acos((ls[(j + 1) % 3] * ls[(j + 1) % 3] +
                ls[(j + 2) % 3] * ls[(j + 2) % 3] - ls[j] * ls[j]) /
               (2 * ls[(j + 1) % 3] * ls[(j + 2) % 3]));
      }
    }
    ...
    for(unsigned i = 0; i < m_vertices.size(); ++i) {
      ...
      v.saddle_or_boundary() =
        (total_vertex_angle[v.id()] > 2.0 * M_PI - 1e-10);
    }
