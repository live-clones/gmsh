...
void macroMesh(const std::string & filename,
  ...
  for (auto entity2Tag: entity2Tags) {
    ...
    if (dim == 0) {
      for (auto vTag: vTags) {
	if (tag2Index.find(old2New[vTag]) == tag2Index.end())
	  continue;
	outputEntities << dim << "," << tag << "," << tag2Index[old2New[vTag]] << "\n";
      }
    }
    else if (dim == 1) {
      for (size_t i = 0; i < vTags.size()/2; ++i) {
	outputEntities << dim << "," << tag << "," << tag2Index[old2New[vTags[2*i]]] << ","
		                                   << tag2Index[old2New[vTags[2*i+1]]] << "\n";

      }
    }
    else if (dim == 2) {
      for (size_t i = 0; i < vTags.size()/3; ++i) {
	outputEntities << dim << "," << tag << "," << tag2Index[old2New[vTags[3*i]]] << ","
		                                   << tag2Index[old2New[vTags[3*i+1]]] << ","
	                                           << tag2Index[old2New[vTags[3*i+2]]] << "\n";
