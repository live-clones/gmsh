...
namespace WinslowUntangler {
  ...
  double update_jacobian_matrix(size_t t, UntanglerData& w, const alglib::real_1d_array &X) {
    if (w.dim == 2) {
  ...
  inline void print_array(const alglib::real_1d_array &x, const std::string &name = "")
  {
    std::cout << name << ": ";
  ...
  double compute_energy_and_gradient(UntanglerData &w, 
      const alglib::real_1d_array &X, alglib::real_1d_array &grad) {
    // Initial values
    w.J_det_min = DBL_MAX;
    w.nb_invalid = 0;
    double energy = 0.;
    ...
    const size_t nElements = (w.dim == 2) ? w.triangles.size() : w.tetrahedra.size();
    for(size_t t = 0; t < nElements; t++) {
      ...
      const double f_eps = (w.dim == 2) ? 
        (w.J_mat_2D[t].transpose() * w.J_mat_2D[t]).trace() / chi
        : (w.J_mat_3D[t].transpose() * w.J_mat_3D[t]).trace() / std::pow(chi,2./3.);
      ...
      for (size_t i = 0; i < w.dim; ++i) {
        if (w.dim == 2) { // Planar case
          ...
          Eigen::Vector2d dphi_da = 2./chi * a_i - 1./chi * (f_eps*chip - 2.*w.lambda*det+w.lambda*g_eps*chip) * b_i;
          ...
          Eigen::Vector3d b_i = w.J_mat_3D[t].col((i+1)%3).cross(w.J_mat_3D[t].col((i+2)%3));
          Eigen::Vector3d dphi_da = 2./std::pow(chi,2./3.) * a_i - 1./chi * (2./3.*f_eps*chip - 2.*w.lambda*det+w.lambda*g_eps*chip) * b_i;
          for (size_t k = 0; k < 4; ++k) {
  ...
  { 
    if(triIdealShapes.size() != 0 &&
      ...
      Msg::Error("Winslow untangler 2D: incoherent sizes in triangles / triIdealShapes");
      return false;
    }
    data.dim = 2;
    data.triangles = tris;
    ...
    if (avg_tri_area <= 0) {
      Msg::Warning("Winslow untangler 2D: average triangle area is negative: %.3e", avg_tri_area);
    }
    ...
    for(size_t t = 0; t < data.triangles.size(); t++) {
      ...
      const double f_eps = (data.J_mat_2D[t].transpose() * data.J_mat_2D[t]).trace() / chi;
      const double g_eps = (det*det + 1.) / chi;
  ...
  { 
    if(tetIdealShapes.size() != 0 &&
      ...
      Msg::Error("Winslow untangler 3D: incoherent sizes in tets / tetIdealShapes");
      return false;
    }
    data.dim = 3;
    ...
    if (avg_tet_vol <= 0) {
      Msg::Warning("Winslow untangler 3D: average triangle area is negative: %.3e", avg_tet_vol);
    }
    ...
    for(size_t t = 0; t < data.tetrahedra.size(); t++) {
      ...
      const double chi = coef_chi(det,data.eps);
      const double chip = coef_chip(det,data.eps);
      const double f_eps = (data.J_mat_3D[t].transpose() * data.J_mat_3D[t]).trace() / std::pow(chi,2./3.);
      const double g_eps = (det*det + 1.) / chi;
      const double Ec = f_eps + data.lambda * g_eps;
      data.energy += Ec;
    }
      ...
      double timeMax)
  {
    ...
    double* points = (dim == 2) ? points2D.front().data() : points3D.front().data();
    ...
    for(int iter = 0; iter < iterMaxOuter; iter++) {
      ...
      double dErel = data.energy > 0 ? std::abs(data.energy - E_prev) / data.energy : 0.;

      Msg::Debug(
          "- iter %i: eps = %f, E: %.3e, dE/E=%.3e, min(detJ)=%f, #(det<0)=%i (LBFGS: %i iters)", 
          iter, data.eps, data.energy, dErel, data.J_det_min, data.nb_invalid, rep.iterationscount);
    ...
    if (dim == 2) {
      scaleToInitial(points2D,bbmin2D,bbmax2D);
    } else if (dim == 3) {
  ...
  double timeMax)
{
  ...
  const std::vector<std::array<uint32_t, 4> > tetrahedra;
  const std::vector<std::array<std::array<double, 3>, 4> > tetIdealShapes;
  return untangle_simplex_elements(2, points, points3D, locked, triangles, triIdealShapes, tetrahedra, tetIdealShapes,
      lambda, iterMaxInner, iterMaxOuter, iterFailMax, timeMax);
#else
  Msg::Error("Winslow untangler requires modules Eigen, Alglib and QuadMeshingTools");
  return false;
#endif
}
  ...
  double timeMax)
{
  ...
  const std::vector<std::array<uint32_t, 3> > tris;
  const std::vector<std::array<std::array<double, 2>, 3> > triIdealShapes;
  return untangle_simplex_elements(3, points2D, points, locked, tris, triIdealShapes, tets, tetIdealShapes,
      lambda, iterMaxInner, iterMaxOuter, iterFailMax, timeMax);
#else
  Msg::Error("Winslow untangler requires modules Eigen, Alglib and QuadMeshingTools");
  return false;
