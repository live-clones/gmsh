...
bool buildVerticesAndTriangles(
    GFace* gf,
...
bool buildPlanarTriProblem(
  ...
  double denom = std::pow(transfo.mp.a,2) + std::pow(transfo.mp.b,2) + std::pow(transfo.mp.c,2) ;
  if (denom < 1.e-16) {
  ...
  for (size_t v = 0; v < points.size(); ++v) {
    ...
    points[v] = { dot(p-transfo.pop, transfo.tangent), dot(p-transfo.pop, transfo.binormal) };
...
void invertTrianglesIfNecessary(
  ...
  for (size_t i = 0; i < tris.size(); ++i) {
    area += triangle_area_2d(points[tris[i][0]], points[tris[i][1]], points[tris[i][2]]);
  }
  if (area < 0.) {
...
bool untangleGFaceMeshConstrained(GFace* gf, int iterMax, double timeMax) {
  if (gf->geomType() != GFace::Plane) {
    Msg::Error("- Face %i: untangleGFaceMeshConstrained only implemented for planar faces", gf->tag());
    return false;
  }
  ...
  vector<std::array<uint32_t,3> > triangles;
  TransfoPlanar transfo;
  bool okb = buildPlanarTriProblem(gf, vertices, points, locked, triangles, transfo);
  if (!okb) {
    Msg::Error("- Face %i: untangleGFaceMeshConstrained, failed to build planar problem", gf->tag());
    return false;
  }
  ...
  bool converged = untangle_triangles_2D(points, locked, triangles,
      triIdealShapes, theta, iterMaxInner, iterMax, iterFailMax, timeMax);

  /* Project back on 3D plane, then CAD */
  for (size_t v = 0; v < points.size(); ++v) if (!locked[v]) {
    SPoint3 onPlane = SPoint3(transfo.pop
      + points[v][0] * transfo.tangent 
  ...
  Msg::Info("- Face %i: Winslow untangling, SICN min: %.3f -> %.3f, avg: %.3f -> %.3f (%li vertices, %.3f seconds)",
      gf->tag(), sicnMinB, sicnMinA, sicnAvgB, sicnAvgA, vertices.size(), Cpu()-t0);
...
#else
bool untangleGFaceMeshConstrained(GFace* gf, int iterMax, double timeMax) {
  Msg::Error("Module QuadMeshingTools required for untangleGFaceMeshConstrained");
  return false;
}
#endif
