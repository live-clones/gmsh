 ...
 *        Only apply to triangles. Quads should be split in multiple triangles by
 *        the caller (the triangles may overlap)
 *
 ...
 * @param[in] theta Tradeoff between shape and area, range: [0,1], default: 1/128
 ...
 *        with regularization for inverted elements.
 *        Only apply to tetrahedra. Other cell types should be split in multiple tets
 *        by the caller (the tets may overlap, e.g. use 32 tets per hex)
 *
 * @param[in,out] points Coordinates of the 3D points to optimize
 * @param[in] locked Locked vertices
 * @param[in] tets List of tet indexes in points
 * @param[in] tetIdealShapes Ideal shapes for the tets. Useful for anisotropic targets or
 *                           for decomposition of hex/prism/pyramid into tets. Can be empty.
 * @param[in] theta Tradeoff between shape and volume, range: [0,1], default: 0.5
 * @param[in] iterMaxInner Maximum number of iterations for each LBFGS solve
 * @param[in] iterMaxOuter Maximum number of outer steps
 * @param[in] iterFailMax Stop the solver after iterFailMax step failures
 * @param[in] timeMax Stop the solver after timeMax CPU seconds
