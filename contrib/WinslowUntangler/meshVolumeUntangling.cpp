...
namespace WinslowUntanglerVolume {
  ...
  std::vector<std::array<uint32_t,4> > tetsFromHex(const array<uint32_t,8>& hex, HexDcp dcp = TO_24TETS) {
  ...
  bool buildVerticesAndTetrahedra(
    ...
    for (size_t e = 0; e < gr->getNumMeshElements(); ++e) {
      ...
      } else if (elt->getType() == TYPE_HEX) {
        const HexDcp dcp = HexDcp::TO_32TETS;
        const array<uint32_t,8> hex = {
          vert[0], vert[1], vert[2], vert[3],
          vert[4], vert[5], vert[6], vert[7]};
        vector<array<uint32_t,4> > htets = tetsFromHex(hex,dcp);
        vector<array<array<double,3>,4> > shapes = tetsFromHexIdealShapes(dcp);
      ...
      } else if (elt->getType() == TYPE_PRI) {
      } else if (elt->getType() == TYPE_PYR) {
      }
    }
  ...
  void invertTetsIfNecessary(
    ...
    if (tetIdealShapes.size() > 0) {
      ...
      for (size_t i = 0; i < tetIdealShapes.size(); ++i) {
        volIdealShapes += volume(tetIdealShapes[i][0], tetIdealShapes[i][1], tetIdealShapes[i][2], tetIdealShapes[i][3]);
      }
...
bool untangleGRegionMeshConstrained(GFace* gf, int iterMax, double timeMax) {
  Msg::Error("Module QuadMeshingTools required for untangleGRegionMeshConstrained");
  return false;
}
#endif
