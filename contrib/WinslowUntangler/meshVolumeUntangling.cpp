...
namespace WinslowUntanglerVolume {
  ...
  tetsFromHexIdealShapes(HexDcp dcp = TO_24TETS)
  {
    std::vector<std::array<std::array<double, 3>, 4> > shapes;
    if(dcp == TO_8TETS || dcp == TO_32TETS) {
      for(size_t i = 0; i < 8; ++i) {
        shapes.push_back(
          {unit_cube[hex2tet_corners[i][0]], unit_cube[hex2tet_corners[i][1]],
    ...
    if(dcp == TO_24TETS || dcp == TO_32TETS) {
      for(size_t i = 0; i < 24; ++i) {
        shapes.push_back(
          {unit_cube[hex2tet_24[i][0]], unit_cube[hex2tet_24[i][1]],
