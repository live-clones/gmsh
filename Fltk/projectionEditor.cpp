...
static fourierProjectionFace *createProjectionFaceFromName(const char *name)
{
  ...
  if(!strcmp(name, "plane"))
    ...
    f = new fourierProjectionFace(m, tag,
                                  new FM::TrapezoidProjectionSurface(tag));
  else if(!strcmp(name, "sphere"))
    f = new fourierProjectionFace(m, tag,
                                  new FM::SphericalProjectionSurface(tag));
  else if(!strcmp(name, "paraboloid"))
    f = new fourierProjectionFace(m, tag,
                                  new FM::ParaboloidProjectionSurface(tag));
  else if(!strcmp(name, "cylinder"))
    f = new fourierProjectionFace(m, tag,
                                  new FM::CylindricalProjectionSurface(tag));
  else if(!strcmp(name, "revolvedParabola"))
    f = new fourierProjectionFace(
      m, tag, new FM::RevolvedParabolaProjectionSurface(tag));
  else if(!strcmp(name, "translatedParabola"))
    f = new fourierProjectionFace(
      m, tag, new FM::TranslatedParabolaProjectionSurface(tag));
  else
...
static void project_point(FM::ProjectionSurface *ps, double x, double y,
                          double z, std::vector<double> &u,
                          std::vector<double> &v, std::vector<double> &dist,
...
static void getTangents(const SVector3 n, SVector3 &t1, SVector3 &t2,
                        const double angle)
{
  SVector3 ex(0., 0., 0.);
  if(n[0] == 0.)
...
static void update_cb(Fl_Widget *w, void *data)
{
  ...
  if(p) {
    FM::ProjectionSurface *ps =
      (FM::ProjectionSurface *)p->face->getNativePtr();
    ps->SetOrigin(p->parameters[0]->value(), p->parameters[1]->value(),
                  p->parameters[2]->value());
    SVector3 n(p->parameters[3]->value(), p->parameters[4]->value(),
    ...
    ps->SetScale(p->parameters[7]->value(), p->parameters[8]->value(),
...
static void translate(void *data, int axis, bool plus)
{
  ...
  if(p) {
    FM::ProjectionSurface *ps =
      (FM::ProjectionSurface *)p->face->getNativePtr();
    SVector3 origin(p->parameters[0]->value(), p->parameters[1]->value(),
                    p->parameters[2]->value());
    SVector3 vec;
    if(axis == 0)
...
static void translate_p0_cb(Fl_Widget *w, void *data)
{
  translate(data, 0, true);
}
static void translate_p1_cb(Fl_Widget *w, void *data)
{
  translate(data, 1, true);
}
static void translate_p2_cb(Fl_Widget *w, void *data)
{
  translate(data, 2, true);
}
static void translate_m0_cb(Fl_Widget *w, void *data)
{
  translate(data, 0, false);
}
static void translate_m1_cb(Fl_Widget *w, void *data)
{
  translate(data, 1, false);
}
static void translate_m2_cb(Fl_Widget *w, void *data)
{
  translate(data, 2, false);
}
  ...
  while(1) {
    ...
    if(ib == 'l') {
      if(CTX::instance()->pickElements) {
        ...
        for(unsigned int i = 0; i < FlGui::instance()->selectedVertices.size();
            i++) {
...
static void save_selection_cb(Fl_Widget *w, void *data)
{
  ...
  if(fileChooser(FILE_CHOOSER_CREATE, "Save Selection", "*.{geo,msh}")) {
    ...
    if(ele.size()) {
      ...
      fprintf(fp, "$ENDNOD\n$ELM\n%d\n", nelm);
      for(unsigned int i = 0; i < ele.size(); i++)
        if(ele[i]->getVisibility() == 2) ele[i]->writeMSH(fp, 1.0);
      fprintf(fp, "$ENDELM\n");
    }
    fclose(fp);
  }
...
static void compute_cb(Fl_Widget *w, void *data)
{
  ...
  if(p) {
    ...
    if(e->getPatchType()) {
      ...
      m->getFMInternals()->current()->GetGroup(0)->GetBlendGroup()->AddPatch(
        patch);
      ...
      if(ps->IsUPeriodic()) {
        FM::Patch *patchL = new FM::FPatch(0, ps->clone(), u, v, f, 3, uModes,
                                           vModes, uM, vM, h0, h1, h2, h3);
        patchL->SetMinU(-0.35);
        patchL->SetMaxU(0.35);
        m->getFMInternals()->current()->GetGroup(0)->GetBlendGroup()->AddPatch(
          patchL);
        ...
        // makeGFace(patchL);
        FM::Patch *patchR = new FM::FPatch(0, ps->clone(), u, v, f, 3, uModes,
                                           vModes, uM, vM, h0, h1, h2, h3);
        patchR->SetMinU(0.15);
        patchR->SetMaxU(0.85);
        m->getFMInternals()->current()->GetGroup(0)->GetBlendGroup()->AddPatch(
        ...
        FM::Patch *patch = new FM::FPatch(0, ps->clone(), u, v, f, 3, uModes,
                                          vModes, uM, vM, h0, h1, h2, h3);
        m->getFMInternals()->current()->GetGroup(0)->GetBlendGroup()->AddPatch(
          patch);
        m->getFMInternals()->makeGFace(patch, m);
...
static void blend_cb(Fl_Widget *w, void *data)
{
  ...
  for(GModel::fiter it = m->firstFace(); it != m->lastFace(); it++)
    if((*it)->getNativeType() == GEntity::FourierModel) faces.push_back(*it);

  m->getFMInternals()->current()->GetGroup(0)->GetBlendGroup()->Blend();
  for(int i = 0; i < m->getFMInternals()
                       ->current()
                       ->GetGroup(0)
                       ->GetBlendGroup()
                       ->GetNumPatches();
      i++)
    m->getFMInternals()->makeGFace(
      m->getFMInternals()->current()->GetGroup(0)->GetBlendGroup()->GetPatch(i),
      m);
...
static void action_cb(Fl_Widget *w, void *data)
{
  ...
  if(what == "delete_last" || what == "save_last") {
    ...
    if(ib == 'l')
      faces.insert(faces.end(), FlGui::instance()->selectedFaces.begin(),
  ...
  : face(f)
{
  ...
  { // origin is stored in parameters[0,1,2]
    ...
    for(int i = 0; i < 3; i++) {
      new Fl_Box(x + w - bb / 3 - bb / 6, y + (1 + i) * bh, bb / 8, bh,
                 (i == 0) ? "E0" : (i == 1) ? "E1" : "E2");
      bp[i] = new Fl_Repeat_Button(x + w - bb / 3, y + (1 + i) * bh, bb / 8,
                                   bh / 2, "+");
  ...
  { // normal is stored in parameters[3,4,5]
    ...
    v2->value(0.);
    Fl_Value_Input *v3 =
      new Fl_Value_Input(x + 2 * bb / 3, y + 4 * bh, bb - 2 * bb / 3, bh);
    ...
    v3->label("Normal");
    Fl_Button *b =
      new Fl_Button(x + w - bb / 3, y + 4 * bh + bh / 4, bb / 8, bh / 2, "-");
...
projectionEditor::projectionEditor()
{
  ...
  _window = new paletteWindow(width, height,
...

  {
    ...
    Fl_Button *b2 = new Fl_Button(width - WB - 3 * BB / 2, WB + BH, 3 * BB / 2,
                                  BH, "Save selection");
...

  {
    ...
    Fl_Button *b2 =
      new Fl_Button(WB + brw / 2, 2 * WB + 8 * BH, brw / 2, BH, "Save");
  ...
  _hardEdges[0] = new Fl_Toggle_Button(WB, 3 * WB + 9 * BH + hard, hard, uvh);
  _hardEdges[1] = new Fl_Toggle_Button(width - 4 * WB - hard,
                                       3 * WB + 9 * BH + hard, hard, uvh);
  _hardEdges[2] = new Fl_Toggle_Button(WB + hard, 3 * WB + 9 * BH, uvw, hard);
  _hardEdges[3] =
    new Fl_Toggle_Button(WB + hard, height - 4 * WB - 5 * BH - hard, uvw, hard);
  for(int i = 0; i < 4; i++)
  ...
  _orientation = new Fl_Toggle_Button(
    width - 3 * WB, height - 4 * WB - 5 * BH - hard, 2 * WB, hard);
  ...
  _pselect[0] = new Fl_Round_Button(2 * WB + BB, height - 3 * WB - 5 * BH, BB,
                                    BH, "Continuation");
  _pselect[1] = new Fl_Round_Button(3 * WB + 2 * BB, height - 3 * WB - 5 * BH,
                                    BB, BH, "Windowing");

  for(int i = 0; i < 2; i++) _pselect[i]->type(FL_RADIO_BUTTON);
  ...
  _modes[1] = new Fl_Value_Input(WB + BB / 2, height - 3 * WB - 4 * BH, BB / 2,
                                 BH, "Fourier modes");
  ...
  _modes[3] = new Fl_Value_Input(WB + BB / 2, height - 3 * WB - 3 * BH, BB / 2,
                                 BH, "Chebyshev modes");
  ...
  {
    Fl_Button *b = new Fl_Button(width - WB - BB, height - 3 * WB - 4 * BH, BB,
                                 2 * BH, "Generate\nPatch");
...

  {
    ...
    Fl_Button *b1 =
      new Fl_Button(WB + BB / 2, height - 2 * WB - 2 * BH, bb, BH, "last");
    b1->callback(action_cb, (void *)"delete_last");
    Fl_Button *b2 =
      new Fl_Button(WB + BB / 2 + bb, height - 2 * WB - 2 * BH, bb, BH, "all");
    b2->callback(action_cb, (void *)"delete_all");
    Fl_Button *b3 = new Fl_Button(WB + BB / 2 + 2 * bb,
                                  height - 2 * WB - 2 * BH, bb, BH, "sel.");
...

  {
    ...
    Fl_Button *b1 =
      new Fl_Button(s + BB / 2, height - 2 * WB - 2 * BH, bb, BH, "last");
    b1->callback(action_cb, (void *)"save_last");
    Fl_Button *b2 =
      new Fl_Button(s + BB / 2 + bb, height - 2 * WB - 2 * BH, bb, BH, "all");
  ...
  {
    Fl_Button *b1 = new Fl_Button(WB, height - WB - BH, BB, BH, "Blend");
...
void projectionEditor::load(fourierProjectionFace *face, std::string tag)
{
  ...
  projection *p =
    new projection(face, _paramWin[0], _paramWin[1], _paramWin[2], _paramWin[3],
                   _paramWin[4], _paramWin[5], this);
