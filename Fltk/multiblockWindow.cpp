...
multiblockWindow::multiblockWindow(int deltaFontSize) {
  ...
  { /* finding singularities box */
    y += BH;
    Fl_Box *b =
      new Fl_Box(x - WB, y, width, BH, "Detection of singularities");
    b->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE);
  ...
  { /* Seperator */
   ...
   { /* Cross field box */
     y += BH;
    Fl_Box *b = new Fl_Box(x - WB, y, width, BH, "Cross field computation with prescribed singularities");
    b->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE);
    y += BH; /* ---------------------------- new line ---------------------------------*/
    ...
    y += BH; /* ---------------------------- new line ---------------------------------*/
    check_cf_use_prescribed = new Fl_Check_Button(x, y, width - 4 * WB, BH, "Use prescribed singularities");
    check_cf_use_prescribed->type(FL_TOGGLE_BUTTON);
    check_cf_use_prescribed->value(opt->cross_field_use_prescribed_if_available);
  ...
  { /* cutting */
    y += BH;
    Fl_Box *b = new Fl_Box(x - WB, y, width, BH, "Mesh cutting from obtained separatrices");
    b->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE);

    y += BH;
    Fl_Box *b2 = new Fl_Box(x, y, width - 4 * WB, BH);
    b2->align(FL_ALIGN_LEFT | FL_ALIGN_INSIDE);
    b2->label("- require a cross-field with prescribed singularities");
...
void multiblockWindow::show(bool redrawOnly) {
...
void multiblock_cb(Fl_Widget *w, void *data)
{
  // get the visibility info from the model, and update the browser
  // accordingly
  const char *str = (const char *)data;
  if(str && !strcmp(str, "redraw_only"))
    FlGui::instance()->multiblock->show(true);
  else
    FlGui::instance()->multiblock->show(false);
}
