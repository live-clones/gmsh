...
void HierarchicalBasisH1Quad::generateBasis(
  double const &u, double const &v, double *vertexBasis, double *edgeBasis,
  double *bubbleBasis, int sizeVertexBasis, int sizeEdgeBasis,
  int sizeBubbleBasis)
{
  if(sizeVertexBasis != this->nVertexFunction ||
     sizeEdgeBasis != this->nEdgeFunction ||
     sizeBubbleBasis != this->nBubbleFunction) {
...
void HierarchicalBasisH1Quad::generateGradientBasis(
  ...
  double *gradientBubbleU, double *gradientBubbleV, int sizeVertexBasis,
  int sizeEdgeBasis, int sizeBubbleBasis)
{
  if(sizeVertexBasis != this->nVertexFunction ||
     sizeEdgeBasis != this->nEdgeFunction ||
     sizeBubbleBasis != this->nBubbleFunction) {
  ...
  if(this->pOrderEdge[0] > minpe0pe2) {
    ...
    for(int k = minpe0pe2 + 1; k <= this->pOrderEdge[2]; k++) {
      ...
      gradientEdgeU[k + this->pOrderEdge[0] + this->pOrderEdge[1] - 4] =
        dphie2U;
      gradientEdgeV[k + this->pOrderEdge[0] + this->pOrderEdge[1] - 4] =
        dphie2V;
