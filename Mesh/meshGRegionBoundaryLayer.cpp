...
static GFace * haveTowGEdges (std::vector<GFace *> &faces, GEdge *ge1, GEdge *ge2){
  for (size_t i = 0; i < faces.size(); i++){
	...
	std::find(e.begin(),e.end(),ge2) != e.end() )return faces[i];
  }
  return NULL;
}

static void meshPolygon (GRegion *gr, std::vector<MLine*> &poly, std::vector<MTriangle*> &mesh,
...
public :
  ...
  void computeType (double angle) {
    if (max_angle > angle)_t = EXTERNAL;
    else if (min_angle < -angle)_t = INTERNAL;
    else _t = FLAT;    
  }
  ...
  blyr_ridge (GEdge *ge, GRegion *gr, GFace *f0, GFace *f1) : _ge (ge), _gr(gr), _N_SUBNORMALS (0)
  {
    _f[0] = f0;
    _f[1] = f1;
  }
...
public:
  ...
  // ridge points
  mutable std::map<std::pair<GFace*,GFace*>,
		   std::vector<MVertex*> >   _v_per_ridge;
...
public:
  ...
  blyr_manager (GRegion *gr, std::vector<GFace *> & bls , double t) : _thickness(t) , _gr(gr) , _faces (bls)
  {
    ...
    Msg::Debug("Boundary layer manager computes extruded elements in region %d",gr->tag());
	...
	std::map<MFace,SVector3,Less_Face>::iterator it = t_normals.find(t->getFace(0));
	SVector3 n;
	if (it == t_normals.end())Msg::Error ("BLYR ERROR");
	else n = it->second;	
	for (int k=0;k<3;k++){
  ...
  void classify_ridges () {
	...
	for (size_t k=0;k<it->_triangles.size();k++){
	  ...
	  if ((v0 == l0 && v1 == l1) ||
	      (v0 == l1 && v1 == l0) ){
	    if (gf == f0){N[0] =  it->_normals[k]; op0 = v2;}
	    if (gf == f1){N[1] =  it->_normals[k]; op1 = v2;}
	  }
	  if ((v0 == l0 && v2 == l1) ||
	      (v0 == l1 && v2 == l0) ){
	    if (gf == f0){N[0] =  it->_normals[k]; op0 = v1;}
	    if (gf == f1){N[1] =  it->_normals[k]; op1 = v1;}
	  }
	  if ((v1 == l0 && v2 == l1) ||
	      (v1 == l1 && v2 == l0) ){
	    if (gf == f0){N[0] =  it->_normals[k]; op0 = v0;}
	    if (gf == f1){N[1] =  it->_normals[k]; op1 = v0;}
	  }
	}
	...
	while (_ridges [i].max_angle / (_ridges [i]._N_SUBNORMALS + 1) > _threshold_angle){
  ...
  void add_fan (const blyr_mvertex &v){
    ...
    for (size_t i=0;i<v._gfaces.size();i++){
      if (std::find (_unique.begin(), _unique.end(),v._gfaces[i]) == _unique.end())
    ...
    for (size_t i=0;i<_unique.size();i++){
      ...
      MVertex *new_v = new MVertex (v._v->x()+n.x()*t,
				    v._v->y()+n.y()*t,
				    v._v->z()+n.z()*t, _gr);
	...
	std::pair<GFace*,GFace*> pa = std::make_pair(_unique[i],
						     _unique[j]);
  ...
  void add_external_corner(const blyr_mvertex &v){
    ...
    for (size_t i=0; i<v._v_per_face.size() ; i++){
      aaa.push_back(SPoint3(v._v_per_face[i]->x(),
			    v._v_per_face[i]->y(),
			    v._v_per_face[i]->z()));
    }
    mean_plane plane;
    ...
    std::map<std::pair<GFace*,GFace*>,
	     std::vector<MVertex*> >::iterator it = v._v_per_ridge.begin();
	...
	if (f1 == v._f_per_normal[i])v1 =v._v_per_face[i]; 
      ...
      if (itp != plane_vertices.end())v_plane_0 = itp->second;
      else {v_plane_0 = new MVertex (P0.x(), P0.y(), 0,_gr); plane_vertices[v0] = v_plane_0;}
      MVertex *v_plane_1 = NULL;
      itp = plane_vertices.find(v1);
      if (itp != plane_vertices.end())v_plane_1 = itp->second;
      else {v_plane_1 = new MVertex (P1.x(), P1.y(), 0,_gr); plane_vertices[v1] = v_plane_1;}
	...
	if (i == 0)plane_lines.push_back(new MLine (v_plane_0,v_plane));
	else if (i == verts.size() - 1){
	  plane_lines.push_back(new MLine (plane_lines[plane_lines.size()-1]->getVertex(1),v_plane));
    ...
    for (size_t i=0;i<new_vertices.size();i++){
      ...
      SPoint3 pp (plane.x + plane.plan[0][0]*vv->x() + plane.plan[1][0]*vv->y(),
      ...
      dx.normalize();
      MVertex *newv = new MVertex (v._v->x()+dx.x()*blyr_thickness(v._v->x(),v._v->y(),v._v->z()),
				   v._v->y()+dx.y()*blyr_thickness(v._v->x(),v._v->y(),v._v->z()),
				   v._v->z()+dx.z()*blyr_thickness(v._v->x(),v._v->y(),v._v->z()), _gr);
    ...
    for (std::map<MVertex*,MVertex*>::iterator it = plane_vertices.begin();
	 it != plane_vertices.end();++it)plane_vertices_inv[it->second->getNum()] = it->first;
  ...
  void extrude_vertex_on_both_surfaces_of_a_ridge(const blyr_mvertex &v, blyr_ridge * ridge,
						  std::vector <blyr_mvertex> &new_vertices )
  {
    ...
    std::pair<GFace*,GFace*> pa = std::make_pair(ridge->_f[0],
						 ridge->_f[1]);
    std::vector<MVertex*> fan;
    fan.push_back(vmid);
  ...
  void extrude_vertices_on_faces () {
    ...
    std::set<blyr_mvertex>::iterator it =  _vertices.begin();
    for (; it != _vertices.end() ; ++it){
      //      printf ("%d has %d lines %d tris\n",it->_v->getNum(), it->_lines.size(), it->_triangles.size());
      if (it->_lines.size() == 2){
	...
	if (ridge0 == NULL && ridge1 == NULL) // not a ridge
	  ...
	  if (ridge0->getType() == blyr_ridge::INTERNAL){
	    if (ridge0 == ridge1) {
	      extrude_vertex_on_both_surfaces_of_a_ridge(*it,ridge0,new_vertices);
	    }
	    else{
	      Msg::Warning("there is a seam... and we should extrude on that seam");
	    }
	  }
	}
      }
  ...
  void extrude_one_external () {
	...
	for (size_t iGe = 0 ; iGe < it->_gedges.size() ;iGe++){
	  ...
	  MVertex *o = ml->getVertex(0) == it->_v ? ml->getVertex(1) : ml->getVertex(0);
	...
	if	 (nINTERNAL == (it->_gedges.size()-1) && nEXTERNAL == 1) {
	  ...
	  //  we could add a FAN (commented below) on the face, quite complicated indeed ...
	  //  if users really want it, we may code that feature. Yet, I beleive that fans
	  // should only be "embedded" to close a O mesh.
	  
	  double initialGuess[2] = {0,0};
	  blyr_ridge *ridge = _externals[0];
	  	  printf("average normals --> %d %d\n",ridge->_f[0]->tag(),ridge->_f[1]->tag());
	  ...
	  SPoint3 p (it->_v->x() + n.x(), it->_v->y() + n.y(), it->_v->z() + n.z());
	  GFace *gf = NULL;
	  ...
	  if (!gf)Msg::Error ("topological error in 3D Boundary Layer generation"); 
	  GPoint gp = gf->closestPoint (p,initialGuess);
	  	  printf("adding a point %g %g %g in face %d\n",gf->tag(),n.x(),n.y(),n.z());
	  MVertex *vf = new MFaceVertex (gp.x(),gp.y(),gp.z(),
					 gf,gp.u(),gp.v());
	  ...
	  std::set<blyr_mvertex>::iterator ite = _vertices.find(blyr_mvertex(_externals_v[0]));
	  if (ite != _vertices.end()){
	    ...
	    for (size_t k=0;k<_internals.size();k++){
	      std::pair<GFace*,GFace*> pa = std::make_pair(_internals[k]->_f[0],_internals[k]->_f[1]);
	      it->_v_per_ridge [pa] = fan;
	      std::set<blyr_mvertex>::iterator iti = _vertices.find(blyr_mvertex(_internals_v[k]));
	      MVertex *o = iti->_v_per_face[0];
	      gf->quadrangles.push_back(new MQuadrangle (it->_v, iti->_v, o, vf));
	      if (_externals[0]->_f[0] == _internals[k]->_f[0] ||
		  _externals[0]->_f[0] == _internals[k]->_f[1] ) {
		MVertex*l = iti->extruded_vertex(_externals[0]->_f[0]);
		_externals[0]->_f[0]->quadrangles.push_back(new MQuadrangle (it->_v, ite->_v, l, iti->_v));
	      }
		...
		_externals[0]->_f[1]->quadrangles.push_back(new MQuadrangle (it->_v, ite->_v, l, iti->_v));
	      }
	    }
	  }
    ...
    for (size_t i=0;i<vplus.size();i++) if (vplus[i]._v) _vertices.insert(vplus[i]);
  }
  ...
  void extrude_vertices () {
	...
	if (ridge0 == NULL && ridge1 == NULL){ // not a ridge
	  ...
	  if (ridge0->getType() == blyr_ridge::FLAT) {
	    ...
	    //	    printf("verex %d has 2 lines (%d %d)\n",it->_v->getNum(),ge0->tag(), ge1->tag());
	    if (!it->_triangles.empty())add_one_normal (*it);
	    //	    add_fan (*it);
	  }
	}
	...
	for (size_t iGe = 0 ; iGe < it->_gedges.size() ;iGe++){
	  GEdge *ge = it->_gedges [iGe];
	  blyr_ridge * ridge = getRidge (ge);
	  if (ridge->getType() == blyr_ridge::EXTERNAL)_externals.push_back(ridge);
	  else if (ridge->getType() == blyr_ridge::INTERNAL)_internals.push_back(ridge);	  
	  else if (ridge->getType() == blyr_ridge::FLAT)_flats.push_back(ridge);	  
	}
	size_t nINTERNAL = _internals.size();
	size_t nEXTERNAL = _externals.size();
	...
	// internal corner 
	  ...
	  Msg::Error("corner with %d internal ridges and %d external ridges should be coded",nINTERNAL,nEXTERNAL);
	  printf("EXTERNALS :");
	  for (size_t i=0;i<_externals.size();i++)printf("%d ",_externals[i]->_ge->tag());
	  printf("\n");
	  printf("INTERNALS :");
	  for (size_t i=0;i<_internals.size();i++)printf("%d ",_internals[i]->_ge->tag());
	  printf("\n");
	}
      }      
    }
  ...
  void extrude_vertices_on_edges () {
	...
	blyr_ridge * ridge = getRidge (ge);
	bool create_vertices_on_edge = _externals.empty() || ridge->getType() == blyr_ridge::EXTERNAL;
	if (create_vertices_on_edge){
	  ...
	  else Msg::Error("topological error in boundary layer");
	  ...
	  NR += SVector3(mev->x()-v->x(),mev->y()-v->y(),mev->z()-v->z());
	}
	else {
      ...
      MVertex *vr = NULL;
      if (_externals.empty()){
	vr =  new MVertex (v->x() + NR.x() , v->y() + NR.y() , v->z() + NR.z(), _gr);
	...
	if (vplus[iGe]._v != NULL) {
	  ...
	  for (size_t jGe = 0 ; jGe < it->_gedges.size() ;jGe++){
	    if (jGe != iGe){
	      ...
	      if (gf) {
		MVertex *vf = NULL;
		//		printf("s %d %d %d -- %d %d -- %d -- %p %p\n",v->getNum(),iGe,jGe,gei->tag(),gej->tag(),gf->tag(),vplus[iGe]._v,vplus[jGe]._v);

		SVector3 dx1 (vplus[iGe]._v->x() - v->x(), vplus[iGe]._v->y() - v->y(),vplus[iGe]._v->z() - v->z());
		if ((vplus[jGe]._v != NULL) && (jGe > iGe)){		  

		  SVector3 dx2 (vplus[jGe]._v->x() - v->x(), vplus[jGe]._v->y() - v->y(), vplus[jGe]._v->z() - v->z());
		  SPoint3 PNEW (v->x()+dx1.x()+dx2.x(), v->y()+dx1.y()+dx2.y(), v->z()+dx1.z()+dx2.z());
		  ...
		  if (ridgei->getType() == blyr_ridge::EXTERNAL &&
		    ...
		    std::vector<MVertex*> fan;
		    fan.push_back(vf);
		    std::pair<GFace*,GFace*> pa = std::make_pair(_internals[0]->_f[0],
								 _internals[0]->_f[1]);
		    it->_v_per_ridge [pa] = fan;		      
		    ...
		    gf->mesh_vertices.push_back(vf);
		    gf->quadrangles.push_back(new MQuadrangle (v,vplus[iGe]._v, vf,vplus[jGe]._v));
		  ...
		  //		  printf("-------------------------------------- >  %d -- %d %d -- %d %d\n",gf->tag(), gei->tag(), gej->tag(),iGe,jGe);
		  vf = vplus[iGe]._v;
		}
		if (!vf || vf->onWhat() != _gr){
		  if (!vf)printf("a bon\n");
	...
	if (vr){
	  ...
	  for (size_t iGe = 0 ; iGe < it->_gedges.size() ;iGe++){
	    if (vplus[iGe]._v){
	      GEdge *gei = it->_gedges [iGe];
	      blyr_ridge * ridgei = getRidge (gei);	    
	      std::pair<GFace*,GFace*> pa = std::make_pair(ridgei->_f[0],
							   ridgei->_f[1]);
	...
	if (itev != e2v.end()){
	  ...
	  if (ml->getVertex(0) == v)ml->setVertex(0,mev); 
	  else ml->setVertex(1,mev);	    
      ...
      for (size_t i=0;i<vplus.size();i++) if (vplus[i]._v) additional_vertices.push_back(vplus[i]);
      if(vplusf.size())additional_vertices.insert(additional_vertices.end(),vplusf.begin(),vplusf.end());
    }
  ...
  void extrude_triangles () {
	...
	//	printf("%p %p %p\n",o0,o1,o2);
	if (o0 && o1 && o2)_gr->prisms.push_back(new MPrism (v0,v1,v2,o0,o1,o2));
      }
    }
    //    printf("a\n");
  }
  ...
  void extrude_ridges () {
	...
	for (size_t j = 0 ; j < r._ge->lines.size();j++){
	  ...
	  std::map<std::pair<GFace*,GFace*>,
		   std::vector<MVertex*> >::iterator it = it0->_v_per_ridge.find(pa_pos);
	  if (it != it0->_v_per_ridge.end())fan0 = it->second;
	  else {
	  ...
	  if (it != it1->_v_per_ridge.end())fan1 = it->second;
	  else {
	  ...
	  if (fan0.size() == r._N_SUBNORMALS &&
	      fan1.size() == r._N_SUBNORMALS ){
  ...
  void extrude_external_corners () {
	...
	_gr->prisms.push_back(new MPrism (v._v,v._v,v._v,
					  t->getVertex(0),
					  t->getVertex(1),
					  t->getVertex(2)));	    	
      }
...
bool createBoundaryLayerOneLayer (GRegion *gr, std::vector<GFace *> & bls){
  ...
  gr->tetrahedra.clear();
  //  for (size_t i=0;i<gr->mesh_vertices.size();i++)delete gr->mesh_vertices[i];
