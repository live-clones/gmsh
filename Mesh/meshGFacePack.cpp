...
static void jac_points_p2 ( double *xa, double *xb, double *xc, double *xab, double *xbc, double *xca,
			    double *xis, double *etas, double detJ[6], double J[6][4])
{
  double *x[6] = {xa, xb, xc, xab, xbc, xca};  
  for ( int i=0; i<6; i++ ){
    double u = xis[i];
...
static double p2triangle_alignement_quality_measure(double *xa, double *xb, double *xc,
						    double *xab, double *xbc, double *xca, int VIEW_TAG){
  ...
  for( uint32_t j=0; j<6; j++ ) {
    ...
    x[j][0] = interpolateTriangleP2( xi, eta, xa[0], xb[0], xc[0], xab[0], xbc[0], xca[0] );
    x[j][1] = interpolateTriangleP2( xi, eta, xa[1], xb[1], xc[1], xab[1], xbc[1], xca[1] );
    ...
    if (j == 0)
      THETAM[j] = atan2(S,C);
    else THETAM[j] = closest (THETAM[j-1], atan2(S,C));
    
    if (j == 0)
      THETA[j] = atan2(dydu,dxdu);
    else THETA[j] = closest (THETA[j-1], atan2(dydu,dxdu));
  }
...
static double p1triangle_quality_metric( int VIEW_TAG,  PolyMesh::Vertex *v0, PolyMesh::Vertex *v1, PolyMesh::Vertex *v2 )
{
  ...
  std::vector<double> val(9);
  gmsh::view::probe(VIEW_TAG, (xa[0]+xb[0]+xc[0])/3, (xa[1]+xb[1]+xc[1])/3, 0.0, val);
  double M[3] = { val[0], val[4], val[1] }; val.clear();
			     ...
			     int VIEW_TAG)
{
  *lengthInMetricField = 0.0;
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    double x[2]  = { -0.5*t*(1-t)*lagrangianPoints[0][0] + (1+t)*(1-t)*lagrangianPoints[1][0] + 0.5*t*(1+t)*lagrangianPoints[2][0],
                     -0.5*t*(1-t)*lagrangianPoints[0][1] + (1+t)*(1-t)*lagrangianPoints[1][1] + 0.5*t*(1+t)*lagrangianPoints[2][1]};

    double dx = -0.5*(1-2*t)*lagrangianPoints[0][0] - 2*t*lagrangianPoints[1][0] + 0.5*(1+2*t)*lagrangianPoints[2][0];
    double dy = -0.5*(1-2*t)*lagrangianPoints[0][1] - 2*t*lagrangianPoints[1][1] + 0.5*(1+2*t)*lagrangianPoints[2][1]; 

    std::vector<double> val(9);
    gmsh::view::probe(VIEW_TAG, x[0], x[1], 0.0, val);
    if (val.empty()){
      *lengthInMetricField = 1.e22;
    }
    else {
      double M[3] = {val[0], val[4], val[1]};
      *lengthInMetricField = *lengthInMetricField
...
double objectiveFunction_L( double x0, double y0, double xt, double yt, double x1, double y1,
			      int VIEW_TAG)
{
...
double goldenSection( double x0, double y0, double &xt, double &yt, double x1, double y1,
                   double astart, double bstart,
                   double &gamma1, double direction[2], //rename direction instead of jF?
                   int VIEW_TAG)
{
  double tol = 1.e-7;
  ...
  f1 = objectiveFunction( x0, y0, xt-xi1*direction[0], yt-xi1*direction[1], x1, y1, VIEW_TAG );
  f2 = objectiveFunction( x0, y0, xt-xi2*direction[0], yt-xi2*direction[1], x1, y1, VIEW_TAG );
  ...
  //  double Lend = objectiveFunction_L( x0, y0, xt-xi*direction[0], yt-xi*direction[1], x1, y1, VIEW_TAG);
  //  if (Lstart < 1.2) printf("xi = %12.5E fstart %12.5E f %12.5E\n",xi, Lstart, Lend);
  return f;
}


int shortestParabola( double *xstart , double *xend, 
		      int VIEW_TAG, double *xmid, double *lengthGeodesicsTwoPoints ) 
{
  ...
  //  goldenSection(x0, y0, xmid[0], xmid[1], x1, y1, astart, bstart, gamma1, direction, VIEW_TAG);

  
  xmid[0] = xmid[0] -  gamma1 * direction[0];
  xmid[1] = xmid[1] -  gamma1 * direction[1];

  *lengthGeodesicsTwoPoints = objectiveFunction_L( x0, y0, xmid[0], xmid[1], x1, y1, VIEW_TAG);
  
  return 0;

  
  //  printf("%12.5E %12.5E %12.5E %12.5E %12.5E\n",gamma1, direction[0],direction[1],xmid[0],xmid[1]);
  //  return 0;
}
...
static int delaunayEdgeCriterionAnIsotropic(PolyMesh::HalfEdge *he, int VIEW_TAG)
{
  ...
  if (!check_triangle_validity_2d( v3->position, v1->position, v2->position) ||
      !check_triangle_validity_2d( v3->position, v2->position, v0->position) )return -1;
...
double triangleValidityP2 (PolyMesh::HalfEdge* hea, std::map<PolyMesh::HalfEdge*, SPoint2> *midPoints = nullptr){
  ...
  SPoint2 ab (0.5*(hea->v->position.x()+heb->v->position.x()),0.5*(hea->v->position.y()+heb->v->position.y()));
  SPoint2 bc (0.5*(heb->v->position.x()+hec->v->position.x()),0.5*(heb->v->position.y()+hec->v->position.y()));
  SPoint2 ca (0.5*(hec->v->position.x()+hea->v->position.x()),0.5*(hec->v->position.y()+hea->v->position.y()));
  if (midPoints){
    ab = (*midPoints)[hea];
    bc = (*midPoints)[heb];
    ca = (*midPoints)[hec];
  }
  return validity_p2triangle_formula(hea->v->position, heb->v->position, hec->v->position,ab,bc,ca);  
}
...
double triangleQualityP2 (int VIEW_TAG, PolyMesh::HalfEdge* hea, std::map<PolyMesh::HalfEdge*, SPoint2> *midPoints = nullptr){
  ...
  if (FF){
    ...
    gmsh::view::probe(VIEW_TAG, hea->v->position.x(), hea->v->position.y(), 0.0, val0);
    gmsh::view::probe(VIEW_TAG, heb->v->position.x(), heb->v->position.y(), 0.0, val1);
    gmsh::view::probe(VIEW_TAG, hec->v->position.x(), hec->v->position.y(), 0.0, val2);
    ...
    fprintf(FF, "ST2(%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,0,%g,%g,0,%g,%g,0){%g,%g,%g,%g,%g,%g};\n",
	    ...
	    ab.x(),ab.y(),
	    bc.x(),bc.y(),
	    ca.x(),ca.y(), val0[8],val1[8],val2[8],val3[8],val4[8],val5[8]);
  }    

  
  double validity = triangleValidityP2(hea, midPoints);
  if (validity < 0)return validity; 
  return p2triangle_alignement_quality_measure(hea->v->position, heb->v->position, hec->v->position,ab,bc,ca,VIEW_TAG);  
}
...
double triangleValidityP1 (PolyMesh::HalfEdge* hea){
  PolyMesh::HalfEdge* heb = hea->next;
  PolyMesh::HalfEdge* hec = heb->next;
  return -robustPredicates::orient2d(hea->v->position, heb->v->position, hec->v->position);
}
...
double LENGTH( PolyMesh::HalfEdge *he, std::map<PolyMesh::HalfEdge*, SPoint2> &midPoints, int VIEW_TAG, bool straight = false) {
  ...
  SPoint2 mid =  straight ? SPoint2(0.5*(x0+x1),0.5*(y0+y1)): midPoints[he];
...
static double bestParabola( PolyMesh::HalfEdge *he, int VIEW_TAG, std::map<PolyMesh::HalfEdge*, SPoint2> &midPoints, double ximax  = .2) {
  ...
  return QMIN;
  //  printf("%12.5E %12.5E %12.5E %12.5E\n",ximin,QMIN,triangleQualityP2 (VIEW_TAG, he, &midPoints), triangleQualityP2 (VIEW_TAG, he->opposite, &midPoints));
}


static double bestParabola( PolyMesh::HalfEdge *he, std::map<PolyMesh::HalfEdge*, SPoint2> &midPoints,
			    double er (double *,double *,double *,double *,double *,double *), double ximax = 0.2) {
  ...
  SPoint2 CUR =  midPoints[he->opposite];
  double Q1 = er (he->v->position,he->next->v->position,he->next->next->v->position,
		  midPoints[he],
		  midPoints[he->next],
		  midPoints[he->next->next]);
  double Q2 = er (he->opposite->v->position,he->opposite->next->v->position,he->opposite->next->next->v->position,
		  midPoints[he->opposite],
		  midPoints[he->opposite->next],
		  midPoints[he->opposite->next->next]);
...
int computePointsUsingScaledCrossFieldPlanarP2 (const char *modelForMetric,
  ...
  while(!_q.empty()){
	...
	if (dist > _LIMIT){
	  ...
	  _points.push_back(pp);
	  fprintf(f,"SP(%22.15E,%22.15E,0){%lu};\n",pp.x(),pp.y(),_points.size());
	  
	}
	else {
	  //	  fprintf(f,"SL(%22.15E,%22.15E,0,%22.15E,%22.15E,0){%g,%g};\n",close.x(),close.y(),pp.x(),pp.y(),dist, dist);
	}
      }
      //    if (_points.size() > 1900)break;
      if (_points.size()%100 == 0) printf("q size %lu p size %lu\n",_q.size(), _points.size());
    }
  }
  fprintf(f,"};\n");
  fclose(f);
  ...
  while(iter++ < 6){
	...
	if (er && iter > 14)bestParabola( he,  midPoints, er, ximax) ;
	else bestParabola( he,  VIEW_TAG, midPoints, ximax) ;
  ...
  while(iter++ < 3){
    int count =0;
    for (auto he : newMesh->hedges){
      if (!he->f || he->opposite == NULL || he->opposite->data != he->data || he->f->data < 0 || he->opposite->f->data < 0)continue;
      double LA = LENGTH(he,midPoints,VIEW_TAG);
      SPoint2 old = midPoints[he];
      if (LA > 1){
	double A  = er ? bestParabola( he,  midPoints, er)  : bestParabola( he,  VIEW_TAG, midPoints) ;
	newMesh->swap_edge(he);
	double B = er ? bestParabola( he,  midPoints, er) : bestParabola( he,  VIEW_TAG, midPoints) ;
	...
	if (invalid){
	  ...
	  newMesh->swap_edge(he);
	  if (er)bestParabola( he,  midPoints, er) ;
	  else bestParabola( he,  VIEW_TAG, midPoints) ;
	}	
	else {
	  count++;
	  printf("%12.5E %12.5E %12.5E %12.5E %12.5E \n",A, LA, B, LB, LENGTH(he,midPoints,VIEW_TAG));
	}
      }
    }
    printf("count %3d\n",count);
