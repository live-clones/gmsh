...
size_t HilbertCoordinates(double x, double y,
			  double x0, double y0, 
  ...
  for (int i = 0; i <16; i++) {
    ...
    else Msg::Warning("Hilbert failed %d %d",coordRed,coordBlue);
    BIG /= 4;
  }
  return RESULT;
}
...
int GFace2PolyMesh (int faceTag, PolyMesh **pm){
  ...
  *pm = new PolyMesh;
...

  {
    ...
    gmsh::model::mesh::getNodes(nodeTags, coord, parametricCoord, 2, faceTag, true, true );
    
    size_t j = 0;
    for (size_t i = 0; i< nodeTags.size() ; i++, j+=2){
      (*pm)->vertices.push_back
	(new PolyMesh::Vertex(parametricCoord[j],parametricCoord[j+1],0.0,nodeTags[i]));
...

  {
    ...
    gmsh::model::mesh::getElements(elementTypes, elementTags, nodeTags, 2, faceTag);
	...
	(*pm)->hedges.push_back
      ...
      he0->prev = he2;  he0->next = he1;
      he1->prev = he0;  he1->next = he2;
      he2->prev = he1;  he2->next = he0;
      he0->v->he = he0;
      he1->v->he = he1;
      he2->v->he = he2;
      ...
      std::pair<size_t,size_t> p0 = std::make_pair(nodeTags[0][i+1],nodeTags[0][i]);
      std::pair<size_t,size_t> p1 = std::make_pair(nodeTags[0][i+2],nodeTags[0][i+1]);
      std::pair<size_t,size_t> p2 = std::make_pair(nodeTags[0][i],nodeTags[0][i+2]);
...
static int delaunayEdgeCriterionPlaneIsotropic(PolyMesh::HalfEdge *he, void *){
  ...
  double result = -robustPredicates::incircle(v0->position, v1->position, v2->position, v->position);
  
  return (result > 0) ? 1 : 0;
}

static void faceCircumCenter (PolyMesh::HalfEdge *he, GFace *gf,
...
static double faceQuality (PolyMesh::HalfEdge *he, GFace *gf){
  ...
  GPoint p1 = gf->point (v1->position.x(),v1->position.y()); 
  GPoint p2 = gf->point (v2->position.x(),v2->position.y()); 
  return qmTriangle::gamma (p0.x(),p0.y(),p0.z(),p1.x(),p1.y(),p1.z(),p2.x(),p2.y(),p2.z());  
}
...
static int qualityCriterion3D(PolyMesh::HalfEdge *he, void *p){
  ...
  double q1 = qmTriangle::gamma (p0.x(),p0.y(),p0.z(),p1.x(),p1.y(),p1.z(),p2.x(),p2.y(),p2.z());
  double q2 = qmTriangle::gamma (p2.x(),p2.y(),p2.z(),p3.x(),p3.y(),p3.z(),p0.x(),p0.y(),p0.z());
  
  double o1 = qmTriangle::gamma (p1.x(),p1.y(),p1.z(),p2.x(),p2.y(),p2.z(),p3.x(),p3.y(),p3.z());
  double o2 = qmTriangle::gamma (p3.x(),p3.y(),p3.z(),p0.x(),p0.y(),p0.z(),p1.x(),p1.y(),p1.z());
...
static PolyMesh::Face * Walk (PolyMesh::Face *f , double x, double y){
  ...
  while (1){
    ...
    else if (s0 <= 0 && s1 >=0 && s2 >=0) he = he->opposite;
    else if (s1 <= 0 && s0 >=0 && s2 >=0) he = he->next->opposite;
    else if (s2 <= 0 && s0 >=0 && s1 >=0) he = he->next->next->opposite;
    else if (s0 <= 0 && s1 <=0) he = s0 > s1 ? he->opposite : he->next->opposite;
    else if (s0 <= 0 && s2 <=0) he = s0 > s2 ? he->opposite : he->next->next->opposite;
    else if (s1 <= 0 && s2 <=0) he = s1 > s2 ? he->next->opposite : he->next->next->opposite;
    else {
      printf("Face %g %g %g / %g %g %g / %g %g %g \n",
	     v0->position.x(), v0->position.y(), v0->position.z(),
	     v1->position.x(), v1->position.y(), v1->position.z(),
...
static int intersect (PolyMesh::Vertex *v0, PolyMesh::Vertex *v1, PolyMesh::Vertex *b0, PolyMesh::Vertex *b1){
  double s0 = robustPredicates::orient2d(v0->position, v1->position, b0->position);
  double s1 = robustPredicates::orient2d(v0->position, v1->position, b1->position);
  if (s0*s1 >= 0) return 0;
  double t0 = robustPredicates::orient2d(b0->position, b1->position, v0->position);
  double t1 = robustPredicates::orient2d(b0->position, b1->position, v1->position);
  if (t0*t1 >= 0) return 0;
  return 1;
}

int recover_edge (PolyMesh *pm, PolyMesh::Vertex *v_start, PolyMesh::Vertex *v_end){
  ...
  while (!_list.empty()){
    ...
    if (intersect (he->v, he->next->v, he->next->next->v, he->opposite->next->next->v)){
      ...
      int still_intersect = intersect (v_start, v_end, he->next->next->v, he->opposite->next->next->v);
      //      printf("swapping %d %d\n",he->v->data,he->next->v->data);
      pm->swap_edge(he);
      //      pm->print4debug(K++);
      if (still_intersect) _list.push_back(he);
    }
    else _list.push_back(he);
  }
...
struct nodeCopies {
  ...
  size_t closest (double _u, double _v){
    ...
    size_t I = 0;
    for (size_t i=0; i<nbCopies ; i++){
      double dist = sqrt ((_u-u[i])*(_u-u[i])+
			  (_v-v[i])*(_v-v[i]));
      if (dist < minD){
	minD = dist;
	I = i;
...
static void getNodeCopies (GFace *gf,
  ...
  for (auto e : edges) {
    if(!e->isMeshDegenerated()) {
      ...
      if(e->isSeam(gf)) {
	direction = 0;
	if (touched.find(e) == touched.end())touched.insert(e);
	else direction = 1;
      }      
	...
	if (direction != -1){
	  double t=0;
	  if (v->onWhat()->dim() == 0)reparamMeshVertexOnEdge(v, e, t);
	  else if (v->onWhat()->dim() == 1) v->getParameter(0, t);
	  else Msg::Error("a seam edge without CAD ?");
...
PolyMesh * GFaceInitialMesh (int faceTag, int recover ){
  ...
  for (auto e : edges){      
	...
	if (c0 == copies.end() || c1 == copies.end())
	  Msg::Error("unable to find %lu %lu %d %d",l->getVertex(0)->getNum(),l->getVertex(1)->getNum(),
	...
	for (size_t j=0;j<c0->second.nbCopies;j++){
	  ...
	  if (result < 0){
	    Msg::Warning("Impossible to recover edge %lu %lu (error tag %d)",l->getVertex(0)->getNum(),
			 l->getVertex(0)->getNum(),result);
	  }
	  else {
  ...
  // the first 4 vertices are "infinite vertices" --> color them with tag -2 meaning exterior
