    ...
    int N)
{
  ...
  {
    Msg::Info("- quadqs sizemap method: %s (%i), expect midpoint subdivision: %i, scaling on edge length: %f", 
  ...
  bool invertNormalsForQuality, SurfaceProjector *sp = nullptr)
{
  ...
  std::vector<MElement *> elements;
  {
    ...
    for(MVertex *v : unq) {
      auto it = adj.find(v);
      if(it != adj.end()) { 
        auto it2 = std::find(gf->mesh_vertices.begin(),gf->mesh_vertices.end(),v);
        if (it2 != gf->mesh_vertices.end()) {
  ...
  std::vector<MElement*> faceElements = dynamic_cast_vector<MQuadrangle*,MElement*>(gf->quadrangles);
  ...
  DQStats &stats)
{
  ...
  int CORNER_PATCH = 2; /* add the neighbors of neighbors of the extruded vertex if valence 2 */
  ...
  for(int pass : {CORNER_PATCH, CORNER_PATCH_2, LARGER_PATCH, SMALL_PATCH}) {
    for(const auto &kv : cornerAndIdeal) {
      ...
      GFaceMeshPatch patch;
      if((pass == CORNER_PATCH || pass == CORNER_PATCH_2) && quads.size() == 2) {
      ...
      if(sdq == 0) {
        ...
        if (PARANO_VALIDITY) {
          errorAndAbortIfInvalidSurfaceMesh(gf,"improveCornerValences | after execute");
        }
        if(ok) {
  ...
  DQStats &stats)
{
  ...
  for(const auto &kv : curveVertexAndIdeal) {
    ...
    if(sdq == 0) {
      ...
      bool ok = diff.execute(true); /* warning: GFace mesh changes here */
      if (PARANO_VALIDITY) {
        errorAndAbortIfInvalidSurfaceMesh(gf,"improveCurveValences | after execute");
      }
      if(ok) {
        updateAdjacencies(patchBefore, patchAfter, adj);
        append(smoothingPool, patchAfter.intVertices);
  ...
  DQStats &stats)
{
  ...
  while(Q.size() > 0) {
    ...
    if(sdq == 0) {
      ...
      bool ok = diff.execute(true); /* warning: GFace mesh changes here */
      if (PARANO_VALIDITY) {
        errorAndAbortIfInvalidSurfaceMesh(gf,"improveInteriorValences | after execute");
      }
      if(ok) {
        updateAdjacencies(patchBefore, patchAfter, adj);
        append(smoothingPool, patchAfter.intVertices);
...
bool patchIsValidForDiskRequadrangulation(const GFaceMeshPatch& patch,
    const unordered_map<MVertex *, std::vector<MElement *> > &adj) {
  ...
  for (MElement* e: patch.elements) {
    auto it = std::find(
        patch.gf->quadrangles.begin(),
        patch.gf->quadrangles.end(),
        e);
...
bool getIrregularPatchesAroundVertices(
  ...
  for (MVertex* v: vert) {
    ...
    if (val > 5) {
      ...
      for (MElement* q: quads) for (size_t le = 0; le < 4; ++le) {
      ...
      for (MElement* q: quads) {
        ...
        if (it0 == adj.end() || it1 == adj.end() 
            || it2 == adj.end() || it3 == adj.end()) continue;
        std::array<size_t,4> vals = {it0->second.size(),
  ...
  DQStats &stats)
{
  ...
  auto comp = []( const Ir_and_Patch& a, const Ir_and_Patch& b ) { 
    return a.first > b.first 
      || (a.first == b.first && a.second.elements.size() < b.second.elements.size());
  };
  std::priority_queue<Ir_and_Patch, std::vector<Ir_and_Patch>,
  ...
  while(Q.size() > 0) {
    ...
    if(sdq == 0) {
      ...
      bool ok = diff.execute(true); /* warning: GFace mesh changes here */
      if (PARANO_VALIDITY) {
        errorAndAbortIfInvalidSurfaceMesh(gf,"improveInteriorValencesV2 | after execute");
      }
      if(ok) {
        updateAdjacencies(patchBefore, patchAfter, adj);
        append(smoothingPool, patchAfter.intVertices);
         ...
         * add them to the queue */
        std::vector<std::pair<double,GFaceMeshPatch> >  patches;
        getIrregularPatchesAroundVertices(patchAfter.bdrVertices.front(),adj,qValIdeal,patches);
        for (size_t k = 0; k < patches.size(); ++k) {
          Q.push(patches[k]);
        }
      }
...
int RefineMeshWithBackgroundMeshProjectionSimple(GModel *gm)
{
  ...
  for(size_t e = 0; e < edges.size(); ++e) {
    ...
    for (size_t i = 0; i < toProj.size();++ i) {
      MVertex* v = toProj[i];
      double tGuess = tMin + (tMax-tMin) * double(i+1)/double(toProj.size());
  ...
  for(size_t f = 0; f < faces.size(); ++f) {
    ...
    // if (gf->haveParametrization() && !gf->periodic(0)  && !gf->periodic(1)) evalOnCAD = true;
...
int RefineMeshWithBackgroundMeshProjection(GModel *gm)
{
  ...
  if (USE_CAD_PROJECTION) {
    ...
    appendQuadMeshStatistics(gm, stats, "Mesh_");
    printStatistics(
      stats, "Quad mesh after subdivision with CAD projection:");
...
int optimizeQuadMeshWithDiskQuadrangulationRemeshing(GFace *gf)
{
  ...
  if (SHOW_DQR) {
    for (auto& kv: qValIdeal) {
      GeoLog::add(kv.first->point(),std::round(kv.second),"f_"+std::to_string(gf->tag())+"_val_ideal");
    }
    GeoLog::flush();
  }
    ...
    const unordered_map<MVertex *, std::vector<MElement *> >& adj) 
{
  ...
  for (size_t i = 0; i < loop.size(); ++i) {
    ...
    MVertex* v2 = new MFaceVertex(pos.x(),pos.y(),pos.z(),gf,uv.x(),uv.y());
...
int ensureEvenNumberOfEdgesOnCurvesAfterInitialSurfaceMesh(GModel *gm) 
{
  ...
  for (size_t e = 0; e < edges.size(); ++e) {
    ...
    if (ge->lines.size() % 2 != 0) {
      Msg::Warning("- Curve %i has %li lines (due to boundary recovery), splitting one", ge->tag(),ge->lines.size());
      ...
      double lc = -1.;
      if (mev1 && mev2) {
        lc = 0.5 * (mev1->getLc()+mev2->getLc());
      } else if (mev1) {
        lc = mev1->getLc();
      } else if (mev2) {
        lc = mev2->getLc();
      }
      MEdgeVertex *newv = new MEdgeVertex(proj.x(), proj.y(), proj.z(), ge, t, 0, lc);
  ...
  for(size_t f = 0; f < faces.size(); ++f) {
    ...
    if(CTX::instance()->mesh.meshOnlyVisible && !gf->getVisibility()) continue;
    if(CTX::instance()->debugSurface > 0 && gf->tag() != CTX::instance()->debugSurface) continue;
    if (gf->meshAttributes.method == MESH_TRANSFINITE && gf->edges().size() == 4) {
      std::vector<size_t> sizes;
      for (GEdge* ge: gf->edges()) {
        sizes.push_back(ge->lines.size());
      }
      std::sort(sizes.begin(),sizes.end());
      if (sizes.size() != 4 || sizes[0] != sizes[1] || sizes[2] != sizes[3])  {
        Msg::Warning("- Face %i: transfinite no longer possible, remove the attribute", gf->tag());
        gf->meshAttributes.method = MESH_UNSTRUCTURED;
      }
    }
  }
...
#pragma omp parallel for schedule(dynamic)
  ...
  for(size_t f = 0; f < faces.size(); ++f) {
    ...
    if(PARANO_VALIDITY) {
      errorAndAbortIfInvalidSurfaceMesh(gf,"after disk quadrangulation remeshing");
    }
  }
...
#pragma omp parallel for schedule(dynamic)
  ...
  for(size_t f = 0; f < faces.size(); ++f) {
    ...
    if (!okg) {
      if (true) {
        Msg::Warning("TODOMX DISABLED DEBUGGING getSingularitiesFromNewCrossFieldComputation");
      } else {
...
QuadqsContextUpdater::QuadqsContextUpdater()
{
  ...
  backups_int.push_back(new RestoreValueAtEndOfLife<int>(&CTX::instance()->abortOnError));
