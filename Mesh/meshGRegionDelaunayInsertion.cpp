                         ...
                         faceXtet &toExtend)
{
  ...
  for (int i=0;i<4;i++){
    ...
    std::vector<faceXtet>::iterator it = std::find(shell.begin(),shell.end(),fxt);
...
static bool verifyShell (MVertex *v, MTet4*t, std::vector<faceXtet> & shell){
  ...
  for (int i=0;i<4 ; i++){
    ...
    if (!starShaped){
      std::vector<faceXtet>::iterator its = std::find(shell.begin(),shell.end(),fxt);
                   ...
                   std::set<MTet4*,compareTet4Ptr> &allTets)
{
  ...
  while (it != shell.end()){
    ...
    if (distance(it->v[0],v) < lc * .05 ||
        distance(it->v[1],v) < lc * .05 ||
        distance(it->v[2],v) < lc * .05) onePointIsTooClose = true;
  ...
  const std::set<MFace, Less_Face> &allEmbeddedFaces)
{
  ...
  for (std::vector<faceXtet>::const_iterator it = shell.begin(); it != shell.end(); it++){
    shellFaces.push_back(MFace((*it).unsorted[0], (*it).unsorted[1], (*it).unsorted[2]));
  }

  for (std::vector<MTet4*>::const_iterator itc = cavity.begin(); itc != cavity.end(); ++itc){
    for (int j = 0; j<4; j++){
      MFace f = (*itc)->tet()->getFace(j);
      if ((std::find(shellFaces.cbegin(), shellFaces.cend(), f) == shellFaces.end()) &&
