...
static inline double compat_orientation_extrinsic (const double *o0, const double *n0,
						   const double *o1, const double *n1,
						   double *a1, double *b1){
  double t0[3] = {n0[1]*o0[2]-n0[2]*o0[1],
		  n0[2]*o0[0]-n0[0]*o0[2],
		  n0[0]*o0[1]-n0[1]*o0[0]};
  double t1[3] = {n1[1]*o1[2]-n1[2]*o1[1],
		  n1[2]*o1[0]-n1[0]*o1[2],
		  n1[0]*o1[1]-n1[1]*o1[0]};
  
  const size_t permuts[8][2] = {{0,0},{1,0},{2,0},{3,0},{0,1},{1,1},{2,1},{3,1}};
  const double A[4][3] = {
    {o0[0],o0[1],o0[2]},
    {t0[0],t0[1],t0[2]},
    {-o0[0],-o0[1],-o0[2]},
...
static inline double compat_orientation_extrinsic (const SVector3 &o0, const SVector3 &n0,
						   const SVector3 &o1, const SVector3 &n1,
...
public:
  ...
  void finish2(){
	...
	da[i] = atan2(dot(_tgt2,_cneighbors[i]->_tgt),dot(_tgt,_cneighbors[i]->_tgt));
	_C[i] = cos(4*da[i]);
	_S[i] = sin(4*da[i]);
      }
    }
  ...
  double grad()
  {
    if(!inBoundary) {
      ...
      double a[4] = {_cneighbors[0]->_a, _cneighbors[1]->_a, _cneighbors[2]->_a,
                     _cneighbors[3]->_a};
      for (int i=0;i<4;i++){a[i] += da[i];normalize(a[i]);}	
...
static void closest (const cross2d &c1, const cross2d &c2, double &a2, double &diff) {
  ...
  SVector3 d3 = c2._tgt * cos (c2._atemp+2*P) + c2._tgt2 * sin (c2._atemp+2*P);
  SVector3 d4 = c2._tgt * cos (c2._atemp+3*P) + c2._tgt2 * sin (c2._atemp+3*P);
...
static void computeLifting(cross2d *first, int branch,  std::set<MEdge,Less_Edge> &cutG,
  ...
  void print(FILE *f) {
    for (size_t i=0;i<crosses.size();i++){
      fprintf(f,"SL(%g,%g,%g,%g,%g,%g){%d,%d};\n",
	      ...
	      crosses[i]->_e.getVertex(0)->y(),
	      crosses[i]->_e.getVertex(0)->z(),
	      crosses[i]->_e.getVertex(1)->x(),
	      crosses[i]->_e.getVertex(1)->y(),
	      crosses[i]->_e.getVertex(1)->z(),groupId,groupId);
    }
    for (size_t i=0;i<side.size();i++){
      fprintf(f,"ST(%g,%g,%g,%g,%g,%g,%g,%g,%g){%d,%d,%d};\n",
	      side[i]->getVertex(0)->x(),
	      side[i]->getVertex(0)->y(),
	      side[i]->getVertex(0)->z(),
	      side[i]->getVertex(1)->x(),
	      side[i]->getVertex(1)->y(),
	      side[i]->getVertex(1)->z(),
	      side[i]->getVertex(2)->x(),
	      side[i]->getVertex(2)->y(),
...
static void duplicateNodesInCutGraph2(std::vector<GFace *> &f,
				      ...
				      std::vector<groupOfCross2d> &G)
{
  for (size_t i=0;i<G.size();i++){
	...
	std::map<MVertex*,MVertex*>::iterator it0 = old2new.find(e.getVertex(0));
	std::map<MVertex*,MVertex*>::iterator it1 = old2new.find(e.getVertex(1));
	if (it0 != old2new.end() && it1 != old2new.end())
      ...
      for (size_t j=0;j<3;j++){
	std::map<MVertex*,MVertex*>::iterator it = old2new.find(G[i].side[k]->getVertex(j));
...
void print_H_and_Cross (GModel *gm, std::vector<GFace *> &f,
  ...
  for (std::set<MVertex *, MVertexLessThanNum>::iterator it = vs.begin(); it != vs.end(); ++it){
    double a;
    dof.getDofValue(*it, 0, 1, a);
    fprintf(_f,"%lu %12.5E\n",(*it)->getNum(),a);
  }
  fprintf(_f,"%lu",singularities.size());
  for (std::set<MVertex*>::iterator it = singularities.begin(); it != singularities.end(); ++it){
    fprintf(_f,"%lu\n",(*it)->getNum());
  }
  ...
  for (std::map<MEdge, cross2d, Less_Edge>::iterator it = C.begin();
    ...
    fprintf(_f,"%lu %lu %12.5E %12.5E %12.5E %12.5E %12.5E %12.5E\n",
	    it->first.getVertex(0)->getNum(),
	    it->first.getVertex(1)->getNum(),
...
static void computeUniqueVectorPerTriangle (GModel *gm, std::vector<GFace *> &f,
					    std::map<MEdge, cross2d, Less_Edge> &C,
					    int dir,
      ...
      for(int k = 0; k < 3; k++) {
        ...
        if(it != C.end()) {
          if(!it->second.inCutGraph) {
            n++;
	    double C = (dir == 1) ? cos(it->second._atemp) :  sin(it->second._atemp);
	    double S = (dir == 1) ? sin(it->second._atemp) : -cos(it->second._atemp) ;
...
static void LagrangeMultipliers2(dofManager<double> &myAssembler,
				 int NUMDOF,
				 std::map<MEdge, cross2d, Less_Edge> &C,
				 ...
				 bool assemble)
{
  for (size_t i=0;i<groups.size();i++){
	...
	for (int k=0;k<2;k++){
	  ...
	  if (vk != v){
	    if (!assemble){
	      myAssembler.numberVertex(vk, 0, 5);
	    }
	    else {
...
static void createDofs(dofManager<double> &myAssembler,
		       int NUMDOF,
		       std::set<MVertex *> &vs)
{
...
static double distance (MTriangle *t, std::set<MVertex*> & boundaries){
  ...
  double dmin = 1.e22;
  for (std::set<MVertex*>::iterator it = boundaries.begin(); it != boundaries.end();++it){
...
static bool isSingular(MVertex *v, std::vector<cross2d*> &adj, double &MAX){
  ...
  n.normalize();
  if (v->getNum() == TEST)printf("VERTEX %lu %12.5E %12.5E %12.5E\n",v->getNum(),n.x(),n.y(),n.z());
  SVector3 t0,b0;
  std::vector<double> angles;
  for (size_t i=0;i<adj.size();i++){
    if (i == 0){
      SVector3 t = (adj[i]->_e.getVertex(0) == v) ? -adj[i]->_tgt : adj[i]->_tgt;
      t -= n*(dot(t,n));
      t.normalize();
    ...
    if (v->getNum() == TEST){
      ...
      printf("o %12.5E %12.5E %12.5E\n",adj[i]->o_i.x(),adj[i]->o_i.y(),adj[i]->o_i.z());
      printf("ANGLE = %12.5E %12.5E\n",angle*180/M_PI,lifting(angles[0],angles[i])*180/M_PI);
    }
  }
  ...
  if (v->getNum() == TEST)printf("vertex %lu %lu edges %12.5E\n",v->getNum(),adj.size(),MAX);
  return MAX > .5;
}
...
static void computeSingularities(std::map<MEdge,cross2d,Less_Edge > &C,
  ...
  for (std::map<MEdge,cross2d,Less_Edge >::iterator it = C.begin(); it !=C.end(); ++it){
    std::pair<MVertex*,cross2d*> p = std::make_pair(it->first.getVertex(0),&it->second);
    conn.insert(p);
...
static void cutGraph (std::map<MEdge,cross2d,Less_Edge > &C , dofManager<double> & dof,
		      std::set<MEdge,Less_Edge> &cutG, std::set<MVertex*> & singularities,
  ...
  while(!_distances.empty()){
	...
	if (touched.find(tt) == touched.end()){
	  std::pair<double,MTriangle*> pp = std::make_pair(distance(t,_all),tt);
	  _distances.insert(pp);
	  touched.insert(tt);
  ...
  for (; it != C.end(); ++it){
    for (int i=0;i<2;i++){
      std::map<MVertex*,std::vector<MEdge> >::iterator it0 = _all_graph.find(it->first.getVertex(i));
      if (it0 == _all_graph.end()){
	std::vector<MEdge> ee ; ee.push_back(it->first); _all_graph[it->first.getVertex(i)] = ee;
      }
    ...
    /*
  {
	...
	MVertex *o = it->second[i].getVertex(0) == v ? it->second[i].getVertex(1) : it->second[i].getVertex(0);
	double hI;
	dof.getDofValue(o, 0, 1, hI);
	if (hI > hV)isMax = false;
	if (hI < hV)isMin = false;
	}
      if ((isMax || isMin)){
  ...
  
  {
    ...
    for (; it != cutG.end(); ++it){
      ...
      fprintf(fff,"SL(%g,%g,%g,%g,%g,%g){%d,%d};\n",
	      ...
	      e.getVertex(1)->x(),e.getVertex(1)->y(),e.getVertex(1)->z(),1,1);
    }
  }
  fprintf(fff,"};\n");
  fclose(fff);
...
static void groupBoundaries (GModel *gm, std::map<MEdge, cross2d, Less_Edge> &C,
  ...
  for (std::set<MVertex*>::iterator it = boundaries.begin(); it != boundaries.end();++it){
    ...
    for (std::multimap<MVertex*, cross2d*>::iterator it2 = conn.lower_bound(v);
	 it2 != conn.upper_bound(v) ; ++it2){
      if (it2->second->inBoundary){
	bnd.push_back(it2->second);
      }
      if (it2->second->inCutGraph){
	countCutGraph++;
      }      
    }
    if (bnd.size() == 2){
      if (fabs(cos (bnd[0]->_atemp)-cos (bnd[1]->_atemp)) > .25){cutgraph.insert(v);}
  ...
  for (int AA = 0; AA < 4 ; AA++){
	...
	do{
	  ...
	  for (std::multimap<MVertex*, cross2d*>::iterator it2 = conn.lower_bound(v);
	       it2 != conn.upper_bound(v) ; ++it2){
	    if ((_all.find(it2->second) == _all.end()) && (group.empty() || group[group.size()-1] != it2->second) &&
		it2->second->inCutGraph){
	      group.push_back(it2->second);
	      vnew = (it2->second->_e.getVertex(0) == v) ? it2->second->_e.getVertex(1):it2->second->_e.getVertex(0);
	      fprintf(f,"SL(%g,%g,%g,%g,%g,%g){%lu,%lu};\n",
		      it2->second->_e.getVertex(0)->x(),it2->second->_e.getVertex(0)->y(),it2->second->_e.getVertex(0)->z(),
		      it2->second->_e.getVertex(1)->x(),it2->second->_e.getVertex(1)->y(),it2->second->_e.getVertex(1)->z(),
	  ...
	  if (vnew == NULL)break;
	  v = vnew;
	}while((boundaries.find(v)== boundaries.end()) && (singularities.find(v)== singularities.end()));
	if (group.size()){ 
	...
	if (cutgraph.find(v) != cutgraph.end()){
	  ...
	  do{
	    ...
	    if (vnew == NULL)break;
	    v = vnew;
	    //	printf("NEXT POINT %lu %d\n",v->getNum(),cutgraph.find(v) == cutgraph.end());
...
void fastImplementationExtrinsic (std::map<MEdge, cross2d, Less_Edge> &C){
  ...
  counter = 0;
  for (; it !=C.end();++it){
    graph[4*counter+0] = graph[4*counter+1] =
...
static dofManager<double>* computeH(GModel *gm,
			    std::vector<GFace *> &f,
			    std::set<MVertex*> &vs,
			    std::map<MEdge, cross2d, Less_Edge> &C){
  ...
  for(size_t i = 0; i < f.size(); i++) {
    for(size_t j = 0; j < f[i]->triangles.size(); j++) {
      ...
      double A1 = it1->second._a+ atan2(dot(it0->second._tgt2,it1->second._tgt),
					dot(it0->second._tgt,it1->second._tgt));
      it0->second.normalize(A1);
      double a1 = it0->second.lifting(A1);
      double A2 = it2->second._a+ atan2(dot(it0->second._tgt2,it2->second._tgt),
					dot(it0->second._tgt,it2->second._tgt));
      it0->second.normalize(A2);
      double a2 = it0->second.lifting(A2);

      SVector3 x0,x1,x2,x3;
      SVector3 t_i = crossprod(xx,it0->second._tgt); t_i -= xx * dot(xx,t_i); t_i.normalize();
      SVector3 o_i = it0->second.o_i; o_i -= xx * dot(xx,o_i); o_i.normalize();
      SVector3 o_1 = it1->second.o_i; o_1 -= xx * dot(xx,o_1); o_1.normalize();
      SVector3 o_2 = it2->second.o_i; o_2 -= xx * dot(xx,o_2); o_2.normalize();
      compat_orientation_extrinsic (o_i,xx,o_1,xx,x0,x1);
      compat_orientation_extrinsic (o_i,xx,o_2,xx,x2,x3);
      
      //      compat_orientation_extrinsic (it0->second.o_i,it0->second._nrml,it1->second.o_i,it1->second._nrml,x0,x1);
      //      compat_orientation_extrinsic (it0->second.o_i,it0->second._nrml,it2->second.o_i,it2->second._nrml,x2,x3);
      //      a0 = atan2(dot(it0->second._tgt2,it0->second.o_i),
      //		 dot(it0->second._tgt,it0->second.o_i));
      a0 = atan2(dot(t_i,o_i),
...
static void computeIso (MVertex *vsing,
			...
			std::map<MEdge,std::pair<std::map<MVertex*,double> *, double>, Less_Edge> &cutGraphEnds,
			...
			FILE *f, int COUNT)
{
  ...
  if (d1.find(e) != d1.end()){
    ...
    std::pair<std::map<MVertex*,double> * , double > aa = std::make_pair(&pot,VAL);
    std::pair<MEdge,std::pair<std::map<MVertex*,double> *, double> > p = std::make_pair(e,aa);
  ...
  for (size_t i=0;i<lst.size();i++){
	...
	(lst[i]->getVertex(0) == v1 && lst[i]->getVertex(1)== v0))vs[count++]=lst[i]->getVertex(2);
    if ((lst[i]->getVertex(0) == v0 && lst[i]->getVertex(2)== v1)||
	(lst[i]->getVertex(0) == v1 && lst[i]->getVertex(2)== v0))vs[count++]=lst[i]->getVertex(1);
    if ((lst[i]->getVertex(1) == v0 && lst[i]->getVertex(2)== v1)||
	(lst[i]->getVertex(1) == v1 && lst[i]->getVertex(2)== v0))vs[count++]=lst[i]->getVertex(0);    
  }
  ...
  for (int i=0;i<2;i++){
	...
	fprintf(f,"SL(%g,%g,%g,%g,%g,%g){%d,%d};\n",
		p.x(),p.y(),p.z(),pp.x(),pp.y(),pp.z(),COUNT,COUNT);
	computeIso(vsing, adj, VAL, v0, vs[i],pp,pot,visited,cutGraphEnds,d1,G,f, COUNT);
      }
	...
	fprintf(f,"SL(%g,%g,%g,%g,%g,%g){%d,%d};\n",
		p.x(),p.y(),p.z(),pp.x(),pp.y(),pp.z(),COUNT,COUNT);
	computeIso(vsing, adj,VAL,v1, vs[i],pp,pot,visited,cutGraphEnds,d1,G,f, COUNT);
      }
    }
  }    
}
			...
			FILE *f, int COUNT)
{
  ...
  //  printf("computing startiung at %lu %lu val %12.5E\n",v0->getNum(),v1->getNum(),VAL);
  
  std::set<MEdge,Less_Edge> visited;
  std::map<MEdge,std::pair<std::map<MVertex*,double> *, double> , Less_Edge> cutGraphEnds;
  computeIso (vsing, adj,VAL,v0,v1,p,*potU,visited,cutGraphEnds,d1,G,f, COUNT);
  //  return;
  int XX = 1;
  while(!cutGraphEnds.empty()){
    ...
    //  printf("cutgaphends %lu xi %12.5E p %12.5E %12.5E\n",cutGraphEnds.size(),xi,p[0],p[1]);
    ...
    bool found = false;
    for (size_t i = 0 ; i < G.size(); i++){
      if (std::find(G[i].left.begin(), G[i].left.end(), o.getVertex(0)) != G[i].left.end() ||
	  std::find(G[i].left.begin(), G[i].left.end(), o.getVertex(1)) != G[i].left.end()){
	//	printf("coucouc %12.5E\n",G[i].mat[0][0]);
	found = true;
	ROT += fabs(G[i].mat[0][0]) > .6 ? 0:1;
      }
      else if (std::find(G[i].left.begin(), G[i].left.end(), e.getVertex(0)) != G[i].left.end() ||
	       std::find(G[i].left.begin(), G[i].left.end(), e.getVertex(1)) != G[i].left.end()){
	//	printf("coucouc -- %12.5E\n",G[i].mat[0][0]);
	found = true;
	ROT += fabs(G[i].mat[0][0]) > .6 ? 0:1;
      }
    ...
    computeIso (vsing, adj,VAL,o.getVertex(0),o.getVertex(1),p,*POT,visited,cutGraphEnds,d1,G,f, COUNT);
    if (XX > 1200)break;
  }
}
			...
			std::vector<groupOfCross2d> &G, int COUNT)
{
  ...
  for (size_t i=0;i<faces.size();i++){
	...
	//computeIso (adj,VV,v0,v1,pp,potV,potU,visited,f,duplicateEdges,duplicateEdges_back);
	...
	//computeIso (adj,VV,v0,v2,pp,potV,potU,visited,f,duplicateEdges,duplicateEdges_back);
	...
	//computeIso (adj,VV,v1,v2,pp,potV,potU,visited,f,duplicateEdges,duplicateEdges_back);
...
static void computeIsos (GModel *gm,
  ...
  if (1){
    ...
    std::map<MVertex *, MVertex *>::iterator it = new2old.begin();
    for (; it  != new2old.end();++it){
      computeIso (it->second,adj,potU[it->second],potU,potV,f,d1,G,it->second->getNum());
      computeIso (it->second,adj,potV[it->second],potV,potU,f,d1,G,it->second->getNum());
      computeIso (it->first,adj,potU[it->first],potU,potV,f,d1,G,it->second->getNum());
      computeIso (it->first,adj,potV[it->first],potV,potU,f,d1,G,it->second->getNum());
    }
    fprintf(f,"};\n");
    fclose(f);
  }
...
void getAllConnectedTriangles (cross2d* start, std::vector<cross2d*> & group,
  ...
  for (size_t i=0;i<group.size();i++){
    if (isolated_singularities.find(group[i]->_e.getVertex(0)) == isolated_singularities.end())
      all.insert(group[i]->_e.getVertex(0));
    if (isolated_singularities.find(group[i]->_e.getVertex(1)) == isolated_singularities.end())
      all.insert(group[i]->_e.getVertex(1));
  }

  if (allTrianglesConsidered.find(start->_t[0]) != allTrianglesConsidered.end()){      
    if (!start->_cneighbors[0]->inCutGraph)start = start->_cneighbors[0];
    else if (!start->_cneighbors[1]->inCutGraph)start = start->_cneighbors[1];
    else printf("error\n");
  }
  else if ( start->_cneighbors.size() == 4 && allTrianglesConsidered.find(start->_t[1]) != allTrianglesConsidered.end()){
    if (start->_cneighbors.size() == 4 && !start->_cneighbors[2]->inCutGraph)start = start->_cneighbors[2];
    else if (start->_cneighbors.size() == 4 && !start->_cneighbors[3]->inCutGraph)start = start->_cneighbors[3];
    else printf("error\n");
  }
  else {
    if (!start->_cneighbors[0]->inCutGraph)start = start->_cneighbors[0];
    else if (!start->_cneighbors[1]->inCutGraph)start = start->_cneighbors[1];
    else if (start->_cneighbors.size() == 4 && !start->_cneighbors[2]->inCutGraph)start = start->_cneighbors[2];
    else if (start->_cneighbors.size() == 4 && !start->_cneighbors[3]->inCutGraph)start = start->_cneighbors[3];
    else printf("error\n");
  }
...
static void computeLeftRight(groupOfCross2d &g, MVertex **left, MVertex **right){
  for (size_t i=0;i<g.side.size();i++){
    if (g.side[i]->getVertex(0) == *right ||
	g.side[i]->getVertex(1) == *right ||
    ...
    if (g.side[i]->getVertex(0) == *left ||
	g.side[i]->getVertex(1) == *left ||
...
void analyzeGroup (std::vector<cross2d*> & group, groupOfCross2d &g,
  ...
  std::set<MTriangle*> t;
  std::set<MVertex *>  all;
  getAllConnectedTriangles (group[0], group,isolated_singularities, all, t, allTrianglesConsidered);
  ...
  for (int j=0;j<2;j++){
    for (int k=0;k<2;k++){
      if (g.mat[j][k] > .7)g.mat[j][k] = 1; 
      else if (g.mat[j][k] < -.7)g.mat[j][k] = -1; 
      else g.mat[j][k] = 0; 
    }
  }
}
                             ...
                             const char *outputName)
{
  ...
  if(_BDRY){
	...
	for (size_t k=0;k<e[j]->lines.size();k++){
	  MLine *l = e[j]->lines[k];
	  MEdge e = l->getEdge(0);
	  std::map<MEdge, cross2d, Less_Edge>::iterator it =
	    C.find(e);
	  if (it !=C.end()) {
	    it->second.inBoundary = true;
	  }
	}
      }
    }
  ...
  for(it = C.begin(); it != C.end(); ++it) {
    ...
    //    fact = 1;
    SVector3 d1 = (it->second._tgt * cos (a0)    + it->second._tgt2*sin(a0))*fact;
    SVector3 d2 = (it->second._tgt * (-sin (a0)) + it->second._tgt2*cos(a0))*fact;
    SVector3 d3 = (it->second._tgt * (-cos (a0)) - it->second._tgt2*sin(a0))*fact;
    SVector3 d4 = (it->second._tgt * sin (a0)    - it->second._tgt2*cos(a0))*fact;
  ...
  for (size_t i=0;i<groups_cg.size();i++){
    groupOfCross2d g(i);
    analyzeGroup (groups_cg[i], g, d0, d1, adj, isolated_singularities, boundaries, allTrianglesConsidered);
    g.print(_f);
    G.push_back(g);    
  }
  fprintf(_f, "};\n");
  ...
  std::map<MVertex *, MVertex *> new2old;
  std::map<MVertex *, MVertex *> old2new;
  duplicateNodesInCutGraph(f,C, new2old, old2new,duplicateEdges, singularities, adj, G);
  ...
  for(size_t i = 0; i < f.size(); i++) {
    for(size_t j = 0; j < f[i]->triangles.size(); j++) {
      ...
      SVector3 a = d0[t];
      SVector3 b = d1[t];
      fprintf(_f, "VT(%g,%g,%g,%g,%g,%g,%g,%g,%g){%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g};\n",
	      t->getVertex(0)->x(),t->getVertex(0)->y(),t->getVertex(0)->z(),
	      t->getVertex(1)->x(),t->getVertex(1)->y(),t->getVertex(1)->z(),
	      t->getVertex(2)->x(),t->getVertex(2)->y(),t->getVertex(2)->z(),
  ...
  computePotential(gm, f, fn.c_str(), fn2.c_str(),
		   *myAssembler, C, new2old, groups, duplicateEdges, d0, d1, G, potU, potV);

  
  computeIsos (gm,f,singularities,C,new2old,duplicateEdges,groups,groups_cg,potU,potV,cutG,G);
