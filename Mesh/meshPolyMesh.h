#ifndef _MESH_POLYMESH_
#define _MESH_POLYMESH_
...
public:
  ...
  class Vertex {
  public :
    Vertex (double x, double y, double z, int _d = -1) : position (x,y,z) , he(NULL), data(_d) {}
    SVector3 position;
  ...
  class HalfEdge {
  public :
    HalfEdge (Vertex *vv) : v(vv) , f(NULL), prev(NULL),
      next(NULL), opposite(NULL), data(-1) {}
    Vertex  * v ; // origin
  ...
  void print4debug(const int debugTag){
    ...
    for (auto it : faces){
      ...
      fprintf(f,"ST(%g,%g,0,%g,%g,0,%g,%g,0){%d,%d,%d};\n",he0->v->position.x(),he0->v->position.y(),
	      he1->v->position.x(),he1->v->position.y(),he2->v->position.x(),he2->v->position.y(),
	...
	fprintf(f,"SL(%g,%g,0,%g,%g,0){%d,%d};\n",
		...
		he->opposite->v->position.x(),he->opposite->v->position.y(),
		he->data,he->data);
      }
    }
  ...
  inline void initialize_rectangle (double xmin, double xmax, double ymin, double ymax){
    reset();
    ...
    HalfEdge *MM_Mm = new  HalfEdge (v_MM);
    HalfEdge *Mm_mm = new  HalfEdge (v_Mm);
    hedges.push_back(mm_MM);hedges.push_back(MM_Mm);hedges.push_back(Mm_mm);
  ...
  inline int split_triangle (double x, double y, double z, Face *f,
			     ...
			     void* data = NULL, std::vector <HalfEdge*> *_t = NULL){
    ...
    hedges.push_back(hev0);hedges.push_back(hev1);hedges.push_back(hev2);
    hedges.push_back(he0v);hedges.push_back(he1v);hedges.push_back(he2v);
    
    hev0->opposite = he0v ; he0v->opposite = hev0;
    hev1->opposite = he1v ; he1v->opposite = hev1;
    hev2->opposite = he2v ; he2v->opposite = hev2;
    ...
    faces.push_back(f1);faces.push_back(f2);
	...
	if (doSwap(he,data) == 1){
	  ...
	  for (int k=0;k<2;k++){
	    ...
	    if (std::find(_touched.begin(), _touched.end() , heb) == _touched.end() &&
		std::find(_touched.begin(), _touched.end() , hebo) == _touched.end()){
	      _stack.push(heb);
	    }
	    ...
	    if (std::find(_touched.begin(), _touched.end() , hec) == _touched.end() &&
		std::find(_touched.begin(), _touched.end() , heco) == _touched.end()){
	      _stack.push(hec);
	    }
	  }
	}
