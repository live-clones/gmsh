...
static void printSizeField (const char *fn,
  ...
  for (auto t : triangles){
    fprintf(f,"ST(%12.5E,%12.5E,%12.5E,%12.5E,%12.5E,%12.5E,%12.5E,%12.5E,%12.5E){%12.5E,%12.5E,%12.5E};\n",
	    t->getVertex(0)->x(),t->getVertex(0)->y(),t->getVertex(0)->z(),
	    t->getVertex(1)->x(),t->getVertex(1)->y(),t->getVertex(1)->z(),
	    t->getVertex(2)->x(),t->getVertex(2)->y(),t->getVertex(2)->z(),
	    sizeField[t->getVertex(0)],sizeField[t->getVertex(1)],sizeField[t->getVertex(2)]);
  }
  fprintf(f,"};\n");
  fclose(f);
}
...
int createSizeFieldFromExistingMesh (GModel *gm, bool computeCrosses) {
  ...
  for (auto itf = gm->firstFace() ; itf != gm->lastFace() ; ++itf){
    ...
    if ((*itf)->tag() < 100000)
      triangles.insert(triangles.end(), (*itf)->triangles.begin(),(*itf)->triangles.end());
  }
  ...
  for (auto it : v2v){
    ...
    else size = BGM_MeshSize(v->onWhat(), 0., 0., v->x(), v->y(), v->z());
    sizeField[v] = size;
  }
