...
bool id_loop_from_closed_pairs(const std::vector<std::array<int,2>>& pairs, std::vector<int>& loop) {
  ...
  while (loop.size() != pairs.size()) {
    int cv = loop.back();
    bool found = false;
    for (std::size_t i = 0; i < pairs.size(); ++i) if (!done[i]) {
  ...
  if (angle_threshold_rad > 0.) {
    ...
    for (std::size_t i = 0; i < loop.size(); ++i) {
      ...
      if (agl > angle_threshold_rad) {
        Msg::Debug("quadrangular face %i rejected for automatic transfinite because corner angle = %.3f deg > threshold = %.3f deg\n", 
            gf->tag(), 180./M_PI * agl, 180./M_PI * angle_threshold_rad);
        return false;
      }
    }
  }
...
void build_chords(const std::set<GFace*>& faces,
  ...
  Msg::Debug("build chords: %li faces, %li edges", faces.size(), edge2faces.size());
...
bool MeshSetTransfiniteFacesAutomatic(std::set<GFace*>& candidate_faces, 
  ...
  Msg::Debug("transfinite automatic: building chords from %li quadrangular faces ...", faces.size());
  std::vector<std::set<GEdge*> > chords;
