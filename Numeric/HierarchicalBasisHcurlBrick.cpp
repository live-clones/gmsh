...
void HierarchicalBasisHcurlBrick::generateBasis(double const &u, double const &v, double const &w,
  ...
  legendreVector[1]=std::vector<double> (_pb2+1);
    ...
    for(int iEdge = 0; iEdge < _nedge; iEdge++) {
      ...
      for(int indexEdgeFunc = 0; indexEdgeFunc < _pOrderEdge[iEdge] + 1;
          indexEdgeFunc++) {
        for(int j=0;j<3;j++){
          edgeBasis[indexEdgeBasis][j] =direction[j]*
            legendreVector[uvw][indexEdgeFunc] * product[iEdge];
        }
        indexEdgeBasis++;
      }
    }
    ...
    for(int iFace = 0; iFace < _nfaceQuad; iFace++) {
      ...
      for(int index1 = 0; index1 < _pOrderFace1[iFace] + 1; index1++) {
        for(int index2 = 0; index2 < _pOrderFace2[iFace] ; index2++) {
          for(int j=0;j<3;j++){
            faceBasis[indexFaceFunction][j] = lambda[indexLambda] *
    ...
    for(int ipb1 = 0; ipb1 < _pb1 + 1; ipb1++) {
      for(int ipb2 = 0; ipb2 < _pb2 ; ipb2++) {
        for(int ipb3 = 0; ipb3 < _pb3; ipb3++) {
          bubbleBasis[indexBubbleBasis][0] =
            legendreVector[0][ipb1] * lobattoVector[1][ipb2] * lobattoVector[2][ipb3];
      ...
      for(int ipb2 = 0; ipb2 < _pb2+1 ; ipb2++) {
        for(int ipb3 = 0; ipb3 < _pb3; ipb3++) {
          bubbleBasis[indexBubbleBasis][0] =0;
            bubbleBasis[indexBubbleBasis][1] =lobattoVector[0][ipb1] * legendreVector[1][ipb2] * lobattoVector[2][ipb3];
            bubbleBasis[indexBubbleBasis][2] =0;
          indexBubbleBasis++;
        }
      }
    ...
    for(int ipb1 = 0; ipb1 < _pb1; ipb1++) {
      for(int ipb2 = 0; ipb2 < _pb2 ; ipb2++) {
        for(int ipb3 = 0; ipb3 < _pb3+1; ipb3++) {
          bubbleBasis[indexBubbleBasis][0] =0;
            bubbleBasis[indexBubbleBasis][1] =0;
            bubbleBasis[indexBubbleBasis][2] =
              lobattoVector[0][ipb1] * lobattoVector[1][ipb2] * legendreVector[2][ipb3];
          indexBubbleBasis++;
        }
      }
    }
...
void HierarchicalBasisHcurlBrick::orientFace(double const &u, double const &v, double const &w,
  ...
  std::vector<std::vector<double>> &faceFunctions,std::string typeFunction)
{
    if(!(flag1 == 1 && flag2 == 1 && flag3 == 1)) {
      ...
      for(int i = 0; i < faceNumber; i++) {
        iterator += (_pOrderFace1[i] + 1)* _pOrderFace2[i] +(_pOrderFace2[i] + 1)* _pOrderFace1[i]  ;
      }
      if(flag3 == 1) {
        ...
        for(int it1 = 0; it1 < _pOrderFace2[faceNumber]; it1++) {
          for(int it2 = 0; it2 < _pOrderFace1[faceNumber]+1; it2++) {
            ...
            for(int itVector = 0; itVector < 3; itVector++) {
                ...
                lambda *legendreVector1[it2]*lkVector1[it1]*
                impactFlag1 * impactFlag2*direction1[itVector];
            }
            iterator++;
          }
        }
        ...
        for(int it1 = 0; it1 < _pOrderFace2[faceNumber]+1; it1++) {
          for(int it2 = 0; it2 < _pOrderFace1[faceNumber]; it2++) {
            ...
            for(int itVector = 0; itVector < 3; itVector++) {
                faceFunctions[iterator][itVector] =
                lambda *legendreVector2[it1]*lkVector2[it2]*
                impactFlag1 * impactFlag2*direction2[itVector];
            }
            iterator++;
          }
        }
          ...
          for(int iFace = 0; iFace < _nfaceQuad; iFace++) {
            ...
            for(int it1 = 0; it1 < _pOrderFace2[faceNumber]; it1++) {
              for(int it2 = 0; it2 < _pOrderFace1[faceNumber]+1; it2++) {
                ...
                for(int j=0;j<3;j++){
                  if(j==jl1){
                  faceFunctions[indexFaceFunction][j] = vec1[j]*
                                                    legendreVector1[it2] *
                                                lkVector1[it1]*
          ...
          for(int it1 = 0; it1 < _pOrderFace2[faceNumber]+1; it1++) {
            for(int it2 = 0; it2 < _pOrderFace1[faceNumber]; it2++) {
              ...
              if(flag1 == -1 && it2 % 2 != 0) { impactFlag2 = -1; }
                for(int j=0;j<3;j++){
                  if(j==jl2){
                faceFunctions[indexFaceFunction][j] =  vec2[j]*
                                                  lkVector2[it2] *
                                                 legendreVector2[it1]*
                                                 impactFlag1 * impactFlag2;
                  }
...
void HierarchicalBasisHcurlBrick::generateCurlBasis(double const &u, double const &v, double const &w,
                           std::vector<std::vector<double>> &edgeBasis,
                           std::vector<std::vector<double>> &faceBasis,
                          std::vector<std::vector<double>> &bubbleBasis)
{
...
std::vector<double> dlambda(6, 0);
    ...
    for(int iEdge = 0; iEdge < _nedge; iEdge++) {
      ...
      for(int indexEdgeFunc = 0; indexEdgeFunc < _pOrderEdge[iEdge] + 1;
          indexEdgeFunc++) {
        for(int j=0;j<3;j++){
          edgeBasis[indexEdgeBasis][j] =curlProduct[iEdge][j]*
            legendreVector[uvw][indexEdgeFunc];
        }
        indexEdgeBasis++;
      }
    }
   ...
   int indexBubbleBasis = 0;
    for(int ipb1 = 0; ipb1 < _pb1 + 1; ipb1++) {
      for(int ipb2 = 0; ipb2 < _pb2 ; ipb2++) {
        for(int ipb3 = 0; ipb3 < _pb3; ipb3++) {
          bubbleBasis[indexBubbleBasis][0] =0;
            bubbleBasis[indexBubbleBasis][1] =    legendreVector[0][ipb1] * lobattoVector[1][ipb2] * dlobattoVector[2][ipb3];
            bubbleBasis[indexBubbleBasis][2] =-legendreVector[0][ipb1] * dlobattoVector[1][ipb2] * lobattoVector[2][ipb3];
          indexBubbleBasis++;
        }
      }
    }
    for(int ipb1 = 0; ipb1 < _pb1 ; ipb1++) {
      for(int ipb2 = 0; ipb2 < _pb2+1 ; ipb2++) {
        for(int ipb3 = 0; ipb3 < _pb3; ipb3++) {
          bubbleBasis[indexBubbleBasis][0] =-lobattoVector[0][ipb1] * legendreVector[1][ipb2] * dlobattoVector[2][ipb3];
            bubbleBasis[indexBubbleBasis][1] =0;
            bubbleBasis[indexBubbleBasis][2] =dlobattoVector[0][ipb1] * legendreVector[1][ipb2] * lobattoVector[2][ipb3];
          indexBubbleBasis++;
        }
      }
    }
    for(int ipb1 = 0; ipb1 < _pb1; ipb1++) {
      for(int ipb2 = 0; ipb2 < _pb2 ; ipb2++) {
        for(int ipb3 = 0; ipb3 < _pb3+1; ipb3++) {
          bubbleBasis[indexBubbleBasis][0] = lobattoVector[0][ipb1] * dlobattoVector[1][ipb2] * legendreVector[2][ipb3];
            bubbleBasis[indexBubbleBasis][1] =-dlobattoVector[0][ipb1] * lobattoVector[1][ipb2] * legendreVector[2][ipb3];
            bubbleBasis[indexBubbleBasis][2] =0;
