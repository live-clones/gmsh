...
namespace {
  ...
  void calcMapFromIdealElement(int type, T &gSMatX, T &gSMatY, T &gSMatZ)
  {
    ...
    case TYPE_HEX:
    case TYPE_PYR:
      { // Quad, hex, pyramid -> square with side of length 1
        gSMatX.scale(2.);
        gSMatY.scale(2.);
        break;
      }
    default:
      { // Tri, tet, prism: equilateral tri with side of length 1
    ...
    case TYPE_HEX:
    case TYPE_PRI:
      { // Hex, prism -> side of length 1 in z
        gSMatZ.scale(2.);
        break;
      }
    case TYPE_PYR:
      { // Pyramid -> height sqrt(2.)/2
        static const double cPyr = sqrt(2.);
        gSMatZ.scale(cPyr);
        break;
      }
    case TYPE_TET:
      {  // Tet: take into account (x, y) scaling to obtain regular tet
...
void GradientBasis::getAllIdealGradientsFromNodes(const fullMatrix<double> &nodes,
                                                  fullMatrix<double> &dxyzdXYZ) const
{
  fullMatrix<double> prox;
  prox.setAsProxy(dxyzdXYZ, 0, 3);
                                       ...
                                       fullMatrix<double> &result) const
{
  ...
  const double norm0 = sqrt(result(0,0)*result(0,0)+
  ...
  const double norm1 = sqrt(result(1,0)*result(1,0)+
                            result(1,1)*result(1,1)+
                            result(1,2)*result(1,2));
  result(1,0) /= norm1; result(1,1) /= norm1; result(1,2) /= norm1;

  return sqrt(dxyzdXbar(0)*dxyzdXbar(0)+
              dxyzdXbar(1)*dxyzdXbar(1)+
                                      ...
                                      fullMatrix<double> &result, bool ideal) const
{
  ...
  const fullVector<double> &gSY = ideal ? primIdealGradShapeBaryY : primGradShapeBaryY;
  ...
  double norm0 = sqrt(result(0, 0)*result(0, 0)+
                      result(0, 1)*result(0, 1)+
                      result(0, 2)*result(0, 2));
                                   ...
                                   bool ideal) const
{
  const fullVector<double> &gSX = ideal ? primIdealGradShapeBaryX : primGradShapeBaryX;
  const fullVector<double> &gSY = ideal ? primIdealGradShapeBaryY : primGradShapeBaryY;
  const fullVector<double> &gSZ = ideal ? primIdealGradShapeBaryZ : primGradShapeBaryZ;
  ...
  case 1 :
    {
      ...
      if(scaling){
        ...
        normals(0,0) *= scale; normals(0,1) *= scale; normals(0,2) *= scale;
      }
...
void JacobianBasis::getSignedIdealJacAndGradientsGeneral(int nJacNodes,
                                                         ...
                                                         fullMatrix<double> &JDJ) const
{
  getSignedJacAndGradientsGeneral(nJacNodes, gSMatX, gSMatY, gSMatZ,
