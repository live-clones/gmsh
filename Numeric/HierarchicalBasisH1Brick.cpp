...
double HierarchicalBasisH1Brick::_affineCoordinate(const int &j,
  ...
  case (6):
    return 0.5 * (1 - w);
  default:
    throw std::string("j must be : 1<=j<=6");
  }
}
inline void HierarchicalBasisH1Brick::_someProduct(
    double const &u, double const &v, double const &w,
    std::vector<double> &product, std::vector<double> &lambda) {
...
void HierarchicalBasisH1Brick::generateGradientBasis(
  ...
  for (int it = 0; it < 3; it++) {
    ...
    gradientVertex[6][it] = gradientLambda[0][it] * product[11] +
                            gradientProduct[11][it] * lambda[0];
    gradientVertex[7][it] = gradientLambda[1][it] * product[11] +
                            gradientProduct[11][it] * lambda[1];
  }
  std::vector<double> lkVectorU(_pb1 - 1);
  ;
  std::vector<double> lkVectorV(_pb2 - 1);
