...
void HierarchicalBasisHcurlTria::generateBasis(
  ...
  for (int i = 0; i < 3; i++) {
    psie_0[0][i] = -sqrt(0.5) * lambda3 * n2[i] + lambda2 * n3[i];
    psie_0[1][i] = -lambda1 * n3[i] / sqrt(2) + lambda3 * n1[i] / sqrt(2);
    psie_0[2][i] = lambda2 * n1[i] - sqrt(0.5) * lambda1 * n2[i];
    psie_1[0][i] = -sqrt(0.5) * lambda3 * n2[i] - lambda2 * n3[i];
    psie_1[1][i] = -lambda1 * n3[i] / sqrt(2) - lambda3 * n1[i] / sqrt(2);
    psie_1[2][i] = lambda2 * n1[i] + sqrt(0.5) * lambda1 * n2[i];
  }
  ...
  legendreVector[0] =
      std::vector<double>(std::max(std::max(_pOrderEdge[0] - 1, _pf - 2), 0));
  legendreVector[1] =
      std::vector<double>(std::max(std::max(_pOrderEdge[1] - 1, _pf - 2), 0));
  legendreVector[2] =
      std::vector<double>(std::max(std::max(_pOrderEdge[2] - 1, _pf - 2), 0));
  ...
  for (int i = 0; i < _nedge; i++) {
    ...
    if (_pOrderEdge[i] >= 1) {
      ...
      for (int iedge = 2; iedge <= _pOrderEdge[i]; iedge++) {
        for (int j = 0; j < 3; j++) {
          edgeBasis[edgeIt][j] =
              jacob * ((2 * iedge - 1) / iedge * legendreVector[i][iedge - 1] *
                           psie_1[i][j] -
                       (iedge - 1) / iedge * legendreVector[i][iedge - 2] *
                           psie_0[i][j]);
        }
        edgeIt++;
      }
    ...
    for (int n1 = 2; n1 <= _pf; n1++) {
      for (int j = 0; j < 3; j++) {
        faceBasis[faceIt][j] =
            jacob * product * legendreVector[i][n1 - 2] * (*normal)[j];
      }
      faceIt++;
    }
  }
...
void HierarchicalBasisHcurlTria::generateCurlBasis(
  ...
  for (int i = 0; i < _nedge; i++) {
    edgeBasis[edgeIt][2] = curlpsie_0[i];
    edgeIt++;
    if (_pOrderEdge[i] >= 1) {
      edgeBasis[edgeIt][2] = curlpsie_1[i];
      edgeIt++;
            ...
            det * ((2 * iedge - 1) / iedge *
                       (dsubtraction[i][0] * dlegendreVector[i][iedge - 1] *
                        ...
                        dsubtraction[i][1] * dlegendreVector[i][iedge - 1] *
                            psie_1[i][0]) -
                   (iedge - 1) / iedge *
                       (edgeBasis[i][2] * legendreVector[i][iedge - 2] +
    ...
    if (typeFunction == "HcurlLegendre") {
      ...
      if (!(flag1 == 0 && flag2 == 1)) {
       ...
       for (int it = 0; it < _pf - 2; it++) {
          LSub2[it] = OrthogonalPoly::EvalLobatto(it, sub2);
        }
        for (int n1 = 0; n1 < _pf - 2; n1++) {
          double LSub1 = OrthogonalPoly::EvalKernelFunction(n1, sub1);
          for (int n2 = 0; n2 < _pf - 2 - n1; n2++) {
      ...
      dProduct[0] = 0.5 * (lambda[2] * lambda[0] - pl3l1);
      dProduct[1] = 0.5 * (lambda[0] * lambda[1] - pl3l1);
      double product = lambda[0] * lambda[1] * lambda[2];
      if (flag1 == 1 && flag2 == -1) {
        double copy = lambda[0];
        lambda[0] = lambda[1];
      ...
      for (int it = 0; it < _pf - 2; it++) {
        LSubAC[it] = OrthogonalPoly::EvalLobatto(it, subAC);
        dLSubAC[it] = OrthogonalPoly::EvalDLobatto(it, subAC);
      }
      std::vector<double> LSubBA(_pf - 2);
      std::vector<double> dLSubBA(_pf - 2);
      for (int it = 0; it < _pf - 2; it++) {
        LSubBA[it] = OrthogonalPoly::EvalLobatto(it, subBA);
        dLSubBA[it] = OrthogonalPoly::EvalDLobatto(it, subBA);
      }

      for (int n1 = 0; n1 < _pf - 2; n1++) {
        for (int n2 = 0; n2 < _pf - 2 - n1; n2++) {
          faceFunctions[faceIt][2] =
              -det * (dProduct[1] * LSubAC[n1] * LSubBA[n2] +
                      product * dsubBA[1] * dLSubAC[n1] * LSubBA[n2] +
                      product * dsubAC[1] * LSubAC[n1] * dLSubBA[n2]);
      ...
      for (int n1 = 0; n1 < _pf - 2; n1++) {
        for (int n2 = 0; n2 < _pf - 2 - n1; n2++) {
          faceFunctions[faceIt][2] =
              det * (dProduct[0] * LSubAC[n1] * LSubBA[n2] +
                     product * dsubBA[0] * dLSubAC[n1] * LSubBA[n2] +
                     product * dsubAC[0] * LSubAC[n1] * dLSubBA[n2]);
          faceIt++;
        }
      }
