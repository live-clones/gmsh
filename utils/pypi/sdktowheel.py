...
dist_info_dir = glob.glob(sdk_dir + '/lib/gmsh*.dist-info')[0]
dist_info_dir = dist_info_dir.removeprefix(sdk_dir + '/lib/')
...
with tempfile.TemporaryDirectory() as wheel_dir:
    data_dir = os.path.join(wheel_dir, 'gmsh-' + version + '.data')
    ...
    if os.path.isfile(os.path.join(sdk_dir, 'lib', 'gmsh-' + libversion + '.dll')):
        shutil.copy(os.path.join(sdk_dir, 'lib', 'gmsh-' + libversion + '.dll'),
    ...
    elif os.path.isfile(os.path.join(sdk_dir, 'lib', 'libgmsh.so.' + libversion)):
        shutil.copy(os.path.join(sdk_dir, 'lib', 'libgmsh.so.' + libversion),
                    os.path.join(data_dir, 'data', 'lib'))
    elif os.path.isfile(os.path.join(sdk_dir, 'lib', 'libgmsh.' + libversion + '.dylib')):
        shutil.copy(os.path.join(sdk_dir, 'lib', 'libgmsh.' + libversion + '.dylib'),
                    os.path.join(data_dir, 'data', 'lib'))
    else :
    ...
    for dirpath, dirs, files in os.walk(wheel_dir):
        for fname in files:
            ...
            with open(fpath, "rb") as f:
                ...
                length = len(content)
                records.append(','.join([
                    os.path.relpath(fpath, wheel_dir),
                    "sha256=" + checksum,
                    str(len(content))]))

    with open(os.path.join(wheel_dir, dist_info_dir, 'RECORD'), 'a') as f:
        for r in records:
            f.write(r+"\n")
        f.write(os.path.join(wheel_dir, dist_info_dir, 'RECORD') + ',,\n')

    # zip
    shutil.make_archive(wheel_name, 'zip', wheel_dir)
    shutil.move(wheel_name + '.zip', wheel_name + '.whl')
    print(wheel_name + '.whl successfully built.')
