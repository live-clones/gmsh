...
inline MEdge maxEdge(MElement *e)
{
  ...
  for(int i = 0; i < e->getNumEdges(); i++)
    if(maxEd.length() < e->getEdge(i).length()) maxEd = e->getEdge(i);
  return maxEd;
}
...
// compute dummy attributes of any conformal (linear~straighted-sided)
// triangulation
class triangulation {
public:
  // attributes
  ...
  std::map<double, std::vector<MVertex *> >
    bord; // border(s) #todo |-> std::vector
  std::set<MEdge, Less_Edge> borderEdg; // border edges #todo |-> std::vector

  std::list<GEdge *> my_GEdges;
  std::set<int>
    fillingHoles; // obsolete ? /!\ used within discreteDiskFace class
  ...
  void assignVert()
  {
    for(unsigned int i = 0; i < tri.size(); ++i) {
      ...
      for(int j = 0; j < t->getNumVertices(); j++) {
        ...
        if(j < 3) P[j].setPosition(tv->x(), tv->y(), tv->z());
  ...
  void assignBord()
  {
    ...
    for(std::set<MEdge>::iterator ie = borderEdg.begin(); ie != borderEdg.end();
        ++ie) {
      MEdge ed = *ie;
      ...
      std::map<MVertex *, std::vector<MVertex *> >::iterator im =
        firstNode2Edge.find(first);
    ...
    while(!firstNode2Edge.empty() && !seamPoint) {
      ...
      std::map<MVertex *, std::vector<MVertex *> >::iterator in =
        firstNode2Edge.begin();
  ...
  void print(char *name, int elementary) const
  {
    ...
    fprintf(f, "View \"\"{\n");
    for(unsigned int i = 0; i < tri.size(); i++) {
      tri[i]->writePOS(f, true, false, false, false, false, false, 1.0,
                       elementary);
    }
    fprintf(f, "};\n");
    fclose(f);
...
public:
  discreteDiskFace(GFace *parent, triangulation *diskTriangulation, int p = 1,
                   std::vector<GFace *> *CAD = NULL);
  virtual ~discreteDiskFace();
  void getTriangleUV(const double u, const double v,
                     discreteDiskFaceTriangle **mt, double &_u,
                     double &_v) const;
  using GFace::point;
