...
Range<double> ACISEdge::parBounds(int i) const { return Range<double>(s0, s1); }
...
SPoint2 ACISEdge::reparamOnFace(const GFace *face, double epar, int dir) const
{
  ...
  SPApar_pos fpar =
    ((FACE *)(face->getNativePtr()))->geometry()->equation().param(vpos);
  SPoint2 pt2(fpar.u, fpar.v);
  face->moveToValidRange(pt2);
  return pt2;
}

GPoint ACISEdge::closestPoint(const SPoint3 &qp, double &param) const { throw; }
...
bool ACISEdge::isSeam(const GFace *face) const
{
  ...
  if(!(((FACE *)face->getNativePtr())->geometry()->equation().periodic_u()) &&
    ...
    return 0;
  else if((((FACE *)face->getNativePtr())
             ->geometry()
             ->equation()
             .periodic_u()) &&
          !(((FACE *)face->getNativePtr())
              ->geometry()
              ->equation()
              .periodic_v())) {
    ...
    if(((p1_par.u == sur_rang_u.start_pt() ||
         p1_par.u == sur_rang_u.end_pt()) &&
    ...
    if(((p1_par.v == sur_rang_v.start_pt() ||
         p1_par.v == sur_rang_v.end_pt()) &&
    ...
    if(((p1_par.u == sur_rang_u.start_pt() ||
         p1_par.u == sur_rang_u.end_pt()) &&
        (p2_par.u == sur_rang_u.start_pt() ||
         p2_par.u == sur_rang_u.end_pt())) ||
       ((p1_par.v == sur_rang_v.start_pt() ||
         p1_par.v == sur_rang_v.end_pt()) &&
        (p2_par.v == sur_rang_v.start_pt() || p2_par.v == sur_rang_v.end_pt())))
      return 1;
    else
...
bool ACISEdge::degenerate(int) const { return _e->length() == 0.0; }
...
GPoint ACISEdge::point(double par) const
{
  ...
  if(&equ) {
    SPAposition pos;
    equ.eval(par, pos);
    return GPoint(pos.coordinate(0), pos.coordinate(1), pos.coordinate(2), this,
                  par);
  }
...
SVector3 ACISEdge::firstDer(double par) const
{
  ...
  return SVector3(first_deriv.component(0), first_deriv.component(1),
                  first_deriv.component(2));
}
