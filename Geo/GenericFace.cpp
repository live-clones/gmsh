...
GenericFace::ptrfunction_int_refvector_refvector GenericFace::FaceUVFromXYZ =
  NULL;
GenericFace::ptrfunction_int_refvector_refvector_refvector
  GenericFace::FaceClosestPoint = NULL;
GenericFace::ptrfunction_int_refvector_refbool
  GenericFace::FaceContainsPointFromXYZ = NULL;
GenericFace::ptrfunction_int_refvector_refbool
  GenericFace::FaceContainsPointFromUV = NULL;
GenericFace::ptrfunction_int_refvector_refvector GenericFace::FaceXYZFromUV =
  NULL;
GenericFace::ptrfunction_int_int_refdouble_refdouble
  GenericFace::FaceParBounds = NULL;
GenericFace::ptrfunction_int_refvector_refvector_refvector_refdouble_refdouble
  GenericFace::FaceCurvatures = NULL;
GenericFace::ptrfunction_int_refvector_refvector GenericFace::FaceEvalNormal =
  NULL;
GenericFace::ptrfunction_int_refvector_refvector_refvector
  GenericFace::FaceFirstDer = NULL;
GenericFace::ptrfunction_int_refvector_refvector_refvector_refvector
  GenericFace::FaceSecondDer = NULL;
GenericFace::ptrfunction_int_refbool_refbool_refdouble_refdouble
  GenericFace::FacePeriodicInfo = NULL;

GenericFace::GenericFace(GModel *m, int num, int _native_id)
  : GFace(m, num), id(_native_id)
{
  if(!FaceParBounds)
    Msg::Fatal("Genericface::ERROR: Callback FaceParBounds not set");
...
SVector3 GenericFace::normal(const SPoint2 &param) const
{
  ...
  if(!FaceEvalNormal)
    Msg::Fatal("Genericface::ERROR: Callback FaceEvalNormal not set");
...
{
  if(!FaceFirstDer)
    Msg::Fatal("Genericface::ERROR: Callback FaceFirstDer not set");
...
void GenericFace::secondDer(const SPoint2 &param, SVector3 &dudu,
                            SVector3 &dvdv, SVector3 &dudv) const
{
  ...
  for(int i = 0; i < 2; i++) par[i] = param[i];
  if(!FaceSecondDer)
    Msg::Fatal("Genericface::ERROR: Callback FaceSecondDer not set");
...
GPoint GenericFace::point(double par1, double par2) const
{
  ...
  if(!FaceXYZFromUV)
    Msg::Fatal("Genericface::ERROR: Callback FaceXYZFromUV not set");
...
GPoint GenericFace::closestPoint(const SPoint3 &qp,
                                 const double initialGuess[2]) const
{
  ...
  if(!FaceClosestPoint)
    Msg::Fatal("Genericface::ERROR: Callback FaceClosestPoint not set");
  bool ok =
    FaceClosestPoint(id, queryPoint, xyzres, uvres); // orthogonal projection
  if(!ok) Msg::Error("GenericFace::ERROR from FaceClosestPoint ! ");

  // check if the projected point lies on the face...
  bool on_the_face;

  if(!FaceContainsPointFromUV)
    Msg::Fatal("Genericface::ERROR: Callback FaceContainsPointFromUV not set");
  ok = FaceContainsPointFromUV(id, uvres, on_the_face);
  if(!ok) Msg::Error("GenericFace::ERROR from FaceContainsPointFromUV ! ");
  if(!on_the_face)
    Msg::Warning("GenericFace::closestPoint::Warning (using UV)  !!!! The "
                 "returned point does not lies on the face ! ");
...
SPoint2 GenericFace::parFromPoint(const SPoint3 &qp, bool onSurface) const
{
  ...
  if(onSurface) {
    if(!FaceUVFromXYZ)
      Msg::Fatal("Genericface::ERROR: Callback FaceUVFromXYZ not set");
  ...
  if((!onSurface) || (!ok)) { // if not on surface
    ...
    if(!ok) Msg::Error("GenericFace::ERROR from FaceClosestPoint ! ");
    bool on_the_face;
    if(!FaceContainsPointFromXYZ)
      Msg::Fatal(
        "Genericface::ERROR: Callback FaceContainsPointFromXYZ not set");
    ok = FaceContainsPointFromXYZ(
      id, xyzres,
      on_the_face); // check if the projected point lies on the face...
    if(!ok)
      Msg::Error(
        "GenericFace::parFromPoint::ERROR from FaceContainsPointFromXYZ ! ");
    if(!on_the_face)
      Msg::Warning("GenericFace::parFromPoint::Warning !!!! The returned point "
                   "does not lies on the face ! ");
  }
...
GEntity::GeomType GenericFace::geomType() const
{
  std::string s;
  if(!FaceGeomType)
    Msg::Fatal("Genericface::ERROR: Callback FaceGeomType not set");
...
double GenericFace::curvatureMax(const SPoint2 &param) const
{
  ...
  for(int i = 0; i < 2; i++) par[i] = param[i];
  if(!FaceCurvatures)
    Msg::Fatal("Genericface::ERROR: Callback FaceCurvatures not set");
...
double GenericFace::curvatures(const SPoint2 &_param, SVector3 &_dirMax,
                               SVector3 &_dirMin, double &curvMax,
...
bool GenericFace::containsPoint(const SPoint3 &pt) const
{
  ...
  if(!FaceContainsPointFromXYZ)
    Msg::Fatal("Genericface::ERROR: Callback FaceContainsPointFromXYZ not set");
  bool ok = FaceContainsPointFromXYZ(id, queryPoint, res);
  if(!ok)
    Msg::Error(
      "GenericFace::containsPoint::ERROR from FaceContainsPointFromXYZ ! ");
  return res;
}
...
{
  if(!FacePeriodicInfo)
    Msg::Fatal("Genericface::ERROR: Callback FacePeriodicInfo not set");
  bool ok =
    FacePeriodicInfo(id, _periodic[0], _periodic[1], _period[0], _period[1]);
