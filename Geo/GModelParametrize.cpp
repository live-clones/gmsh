...
int parametrizeGFace (discreteFace *gf)
{
  ...
  for(int iv=0; iv<nv; iv++){
    ...
    gf->stl_vertices_xyz[iv] = SPoint3(m->vertices.coord[4*iv+0],
...
{
  Msg::Error("Gmsh should be compiled against HXT for being able to compute discrete parametrizations");
  return -1;
}
#endif
...
int isTriangulationParametrizable(const std::vector<MTriangle*> &t, int Nmax, double ar){
  ...
  for (size_t i=0;i<t.size();++i){
    ...
    for (int j=0;j<3;j++){
      ...
      if (it == e.end())e[t[i]->getEdge(j)] = 1;
      else it->second ++;
    }
  }
  std::map<MEdge,int,Less_Edge>::iterator it = e.begin();
  std::vector<MEdge> _bnd;
  for (; it != e.end() ;++it)if (it->second == 1) _bnd.push_back(it->first);
  std::vector<std::vector<MVertex *> > vs;
  //  printf("%d triangles %d edges %d vertices %d edges on boundaries\n",t.size(),v.size(),e.size(),_bnd.size());
  ...
  int poincare = t.size() - (2*(v.size()-1) - _bnd.size() + 2*(vs.size()-1));
...
void computeEdgeCut(GModel *gm, std::vector<MLine*> &cut, int max_elems_per_cut){
  ...
  for (GModel::fiter it = gm->firstFace();it !=gm->lastFace();++it){
	...
	for (size_t j=0;j<3;++j)vs.insert((*it)->triangles[i]->getVertex(j));
      }
      (*it)->mesh_vertices.insert((*it)->mesh_vertices.begin(),vs.begin(),vs.end());
      partitions.pop();
      int np = isTriangulationParametrizable((*it)->triangles, max_elems_per_cut, 5.0);
      if (np == 1){
	for (size_t i=0;i<(*it)->triangles.size();i++)global[(*it)->triangles[i]] = part;
	part++;
      }
      else {
	...
	if(!PartitionFace (*it,np,p)){
	  ...
	  for (size_t i=0;i<(*it)->triangles.size();i++)t[(*it)->triangles[i]->getPartition()].push_back((*it)->triangles[i]);
	  for (int i = 0;i<np;i++){
	    partitions.push(t[i]);
	  }
	}
	...
	if (it3 == cuts.end())cuts[ed]=it2->second;
	else {
	  if (it3->second != it2->second)cut.push_back(new MLine(ed.getVertex(0),ed.getVertex(1)));	  
	}
      }
    }
...
void computeNonManifoldEdges(GModel *gm, std::vector<MLine*> &cut, bool addBoundary){
  ...
  for (GModel::fiter it = gm->firstFace();it !=gm->lastFace();++it){
    for (size_t i=0;i<(*it)->triangles.size();i++){
      for (int j=0;j<3;j++){
	std::map<MEdge,int,Less_Edge>::iterator it2 = m.find((*it)->triangles[i]->getEdge(j));
	if (it2 == m.end())m[(*it)->triangles[i]->getEdge(j)] = 1;
	else it2->second++;
      }
    }
  }
  {
    ...
    for (; it != m.end();++it){
      if (it->second > 2){
	cut.push_back(new MLine (it->first.getVertex(0),it->first.getVertex(1)));
	countNM ++;
      }
    ...
    if (countNM+countBND > 0)Msg::Info ("Model has %d non manifold mesh edges and %d boundary mesh edges",countNM, countBND);
  }  
  makeMLinesUnique (cut);
}
