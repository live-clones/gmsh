...
GenericEdge::ptrfunction_int_double_refvector GenericEdge::EdgeEvalXYZFromT =
  NULL;
GenericEdge::ptrfunction_int_refdouble_refdouble
  GenericEdge::EdgeEvalParBounds = NULL;
...
GenericEdge::ptrfunction_int_double_refvector GenericEdge::EdgeEvalFirstDer =
  NULL;
GenericEdge::ptrfunction_int_double_refdouble GenericEdge::EdgeEvalCurvature =
  NULL;
GenericEdge::ptrfunction_int_refvector_refdouble_refvector_refbool
  GenericEdge::EdgeClosestPoint = NULL;
GenericEdge::ptrfunction_int_refbool GenericEdge::EdgeIs3D = NULL;
GenericEdge::ptrfunction_int_int_double_int_refvector
  GenericEdge::EdgeReparamOnFace = NULL;

GenericEdge::GenericEdge(GModel *m, int num, int _native_id, GVertex *v1,
                         GVertex *v2, bool _isseam)
  : GEdge(m, num, v1, v2), id(_native_id), is_seam(_isseam)
{
  if((!EdgeEvalParBounds) || (!EdgeEvalXYZFromT))
    Msg::Error("GenericEdge::ERROR: Callback not set");
...
SPoint2 GenericEdge::reparamOnFace(const GFace *face, double par, int dir) const
{
  ...
  if(!EdgeReparamOnFace)
    Msg::Error("GenericEdge::ERROR: Callback EdgeReparamOnFace not set");
  bool ok = EdgeReparamOnFace(id, face->getNativeInt(), par, dir, res);
  if(!ok) {
    Msg::Error("GenericEdge::ERROR from EdgeReparamOnFace ! Edge Native id %d",
               getNativeInt());
  }
...
GPoint GenericEdge::closestPoint(const SPoint3 &qp, double &param) const
{
  ...
  bool is_on_edge;
  if(!EdgeClosestPoint)
    Msg::Error("GenericEdge::ERROR: Callback EdgeClosestPoint not set");
  bool ok = EdgeClosestPoint(id, queryPoint, param, res, is_on_edge);
  if(!ok) Msg::Error("GenericEdge::ERROR from EdgeClosestPoint ! ");
  if(!is_on_edge)
    Msg::Warning("WARNING:GenericEdge::closestPoint closest point NOT on edge, "
                 "out of bounds ! ");
  return GPoint(res[0], res[1], res[2], this, param);
}

bool GenericEdge::isSeam(const GFace *face) const { return is_seam; }

GPoint GenericEdge::point(double par) const
{
  std::vector<double> res(3, 0.);
  if(!EdgeEvalXYZFromT)
    Msg::Error("GenericEdge::ERROR: Callback EdgeEvalXYZFromT not set");
...
SVector3 GenericEdge::firstDer(double par) const
{
  std::vector<double> res(3, 0.);
  if(!EdgeEvalFirstDer)
    Msg::Error("GenericEdge::ERROR: Callback EdgeEvalFirstDer not set");
...
GEntity::GeomType GenericEdge::geomType() const
{
  std::string s;
  if(!EdgeGeomType)
    Msg::Error("GenericEdge::ERROR: Callback EdgeGeomType not set");
  bool ok = EdgeGeomType(id, s);
  if(!ok) {
    Msg::Error("GenericEdge::ERROR from EdgeGeomType ! Edge Native id %d",
               getNativeInt());
    return Unknown;
  }
  ...
  Msg::Warning("GenericEdge::geomType:: unknown type from callback: %s",
               s.c_str());
...
{
  double res;
  if(!EdgeEvalCurvature)
    Msg::Error("GenericEdge::ERROR: Callback EdgeEvalCurvature not set");
...
bool GenericEdge::degenerate(int) const
{
  bool res = false;
  if(!EdgeDegenerated)
    Msg::Error("GenericEdge::ERROR: Callback EdgeDegenerated not set");
...
LinearSeamEdge::LinearSeamEdge(GModel *m, int num, GVertex *v1, GVertex *v2)
  : GEdge(m, num, v1, v2)
{
  s0 = 0.;
...
SVector3 LinearSeamEdge::firstDer(double par) const { return first_der; }

GEntity::GeomType LinearSeamEdge::geomType() const { return Line; }

double LinearSeamEdge::curvature(double par) const { return 0.; }

bool LinearSeamEdge::is3D() const { return false; }

bool LinearSeamEdge::degenerate(int) const { return false; }
