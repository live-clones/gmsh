                                      ...
                                      std::vector<cgsize_t> &elt) const
{
  ...
  for(std::size_t i = 0; i < nb; i++) {
    ...
    elt[i] = ijk2Ind<DIM>(ijk, nbEltIJK());;
  }
}

  
template<int DIM>
void CGNSZoneStruct<DIM>::nodeFromRange(const cgsize_t *range,
                                        std::vector<cgsize_t> &node) const
{
  node.resize(nbNodeInRange(range));
...
template<int DIM>
void CGNSZoneStruct<DIM>::nodeFromList(const std::vector<cgsize_t> &list,
                                       std::vector<cgsize_t> &node) const
{
  std::size_t nb = list.size() / DIM;
  node.resize(nb);
  ...
  for(std::size_t i = 0; i < nb; i++) {
    cgsize_t ijk[3];
    for(int d = 0; d < DIM; d++) ijk[d] = list[i*DIM+d] - 1;
    node[i] = ijk2Ind<DIM>(ijk, nbNodeIJK());;
  }
}
