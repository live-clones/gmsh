...
void writeElementsMATLAB(FILE *fp, const GEntity* entity,
			 int filetype, bool binary, int physical)
{
  if(filetype !=0) return;
  for(unsigned int j = 0; j < entity->getNumMeshElements(); j++){
    if(entity->getMeshElement(j)-> getTypeForMSH())
      entity->getMeshElement(j)->writeMATLAB(fp, filetype, entity->tag(), physical, binary);
  }
  return;
}


void storePairMATLAB(const GEntity* entity, int physical,
		     std::vector<std::vector<std::pair<MElement*, int > > > &elems)
{
  for(unsigned int j = 0; j < entity->getNumMeshElements(); j++){
    int elemtype = entity->getMeshElement(j)->getTypeForMSH();
    if(elemtype)
      elems[elemtype - 1].push_back(std::make_pair(entity->getMeshElement(j), physical));
  }
  return;
}


int GModel::writeMATLAB(const std::string &name, bool binary, bool saveAll, double scalingFactor, int filetype)
{
  //File type
  ...
  if(filetype == SIMPLE)
	...
	for(unsigned int j = 0; j < entities[i]->mesh_vertices.size(); j++)
	  entities[i]->mesh_vertices[j]->writeMATLAB(fp, filetype, binary, scalingFactor);
      fprintf(fp, "];\n");
      //triangles
      fprintf(fp, "\%\%\n");
      fprintf(fp, "gTri=[\n");
	...
	if(saveAll)
	  ...
	  for (unsigned k = 0; k < entities[i]->getPhysicalEntities().size(); k++){
  ...
  if(filetype == LOAD_GMSH2)
	...
	for(unsigned int j = 0; j < entities[i]->mesh_vertices.size(); j++)
      ...
      //We have to store every element in an array and then store them in the file.
	...
	if(saveAll)
	  ...
	  for (unsigned int iphys = 0; iphys < entities[i]->getPhysicalEntities().size(); iphys++){
	...
	{
	  if(elems[elemtype-1].empty())
	    continue;
	  fprintf(fp, "msh.%s =[\n", getMATLABName(elemtype).c_str());
	  //writes
	  for (unsigned int i = 0; i < elems[elemtype-1].size(); i++){
	    elems[elemtype-1][i].first->writeMATLAB(fp, filetype, elems[elemtype-1][i].second, binary);
	  }
	  fprintf(fp, "];\n");
	}
    }
