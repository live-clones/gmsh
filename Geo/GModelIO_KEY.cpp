                             ...
                             bool saveAll)
{
  if(elements.size() && (saveAll || ge->physicals.size())){
    ...
    if(typ){
      ...
      physicalName(ge->model(), ge->dim(), ge->tag()).c_str(), ge->tag());
...
int GModel::writeKEY(const std::string &name, int saveAll, int saveGroupsOfNodes,
  ...
  if(saveGroupsOfNodes & 0x1){
    for(int dim = 1; dim <= 3; dim++){
      for(std::map<int, std::vector<GEntity*> >::iterator it = groups[dim].begin();
        ...
        fprintf(fp, "*SET_NODE_LIST\n$# %s\n%d", physicalName(this, dim, it->first).c_str(),++setid);
        int n = 0;
